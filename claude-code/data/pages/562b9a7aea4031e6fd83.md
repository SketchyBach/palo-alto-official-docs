---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-0/pan-os-release-notes/pan-os-9-0-addressed-issues/pan-os-9-0-11-addressed-issues.html
fetched_at: 2026-09-16T13:55:55Z
source: palo-alto-main
---

# PAN-OS 9.0.11 Addressed Issues Clear
...
Updated on 
 ...
 An enhancement was made to provide an option
...
did not retrieve firewall logs from Cortex Data Lake. 
 ...
 Fixed an issue where certain GPRS tunneling
...
certain GTPv1 Update PDP Context packets. 
 ...
 Fixed an issue where upgrading the capacity
...
configuration ( Device > Server Profiles > Netflow ). 
 ...
 Fixed an issue where the candidate configuration
...
removed due to miscommunication between internal processes. 
 ...
 Fixed an issue where a custom report query
...
events did not return any events . 
 ...
 Fixed an issue where stream control transmission
...
on certificate-based authentication ( CVE-2020-2050 ). 
 ...
 Fixed an issue where conversion from Panorama
...
which prevented access to the appliance after conversion. 
 ...
 Fixed an issue where memory usage on a process ( useridd )
...
crafted TCP packets ( CVE-2020-1999 ). 
 ...
 Fixed an issue on an M-600 appliance where
...
to reach maximum capacity. 
 ...
 Fixed an issue with the automated correlation
...
(ID 6005). 
 ...
 Fixed an issue on Panorama where a custom
...
content of the external dynamic list (EDL) on the Panorama web interface. 
 ...
 Fixed an issue where Log Collectors had
problems ingesting older logs for previous days received at a high
rate. 

 PAN-143796 

 Fixed an issue where commits failed on the
...
command. 
 ...
 Fixed an issue where random member ports
...
FQDN contained CAPITAL letters. 
 ...
 Fixed an issue where IP address-to-tag mapping
...
during device serial number checks. 
 ...
 Fixed an issue where the Host Evasion Threat
...
failure on the Log Processing Card (LPC). 
 ...
 Fixed an issue where a large number of groups
...
oversubscription rate was set to 2. 
 ...
 Fixed an issue where the web interface and
...
to display on the web interface: Timed out while getting config lock . 
 ...
 Fixed an issue where dynamic route updates
...
disruption. 
 ...
 Fixed an issue where HIP reports failed
...
responding. 
 ...
 An enhancement was made to reduce the memory
...
were able to reset the rule hit counter for disabled device groups. 
 ...
 Fixed an issue where pushing a configuration
...
App-ID and rule shadowing warnings populated after a commit. 
 ...
 Fixed an intermittent issue where the first
...
issue is fixed only for existing template stacks. 
 ...
 Fixed an issue where IP tags were not evaluated
in the filter evaluation criteria when Dynamic Address Groups were
configured. 

 PAN-134745 

 Fixed an issue where Panorama commits failed
...
to Panorama for User-ID redistribution. 
 ...
 Fixed an issue where AdminStatus for HA1
...
where H.225 VOIP signaling packets dropped. 
 ...
 Fixed an intermittent issue where user-to-IP
...
by hardware after being requested to do so. 
 ...
 Fixed an issue on Panorama where the show system logdb-quota CLI
command took more time than expected, which caused the configuration
lock to time out. 

 PAN-128048 

 Fixed an issue where certificate-based authentication
with IKEv2 IPSec tunnels failed to establish with some third-party
vendors. 

 PAN-125218 

 A fix was made to address an information
...
device when the Panorama administrator performed a context switch ( CVE-2020-2022 ). 
 ...
 Fixed an issue where only the current day's
logs were visible on Panorama. 

 PAN-124331 

 Fixed an issue where the LDAP query took
...
information was requested through the web interface. 
 ...
 Fixed an issue with the session browser
search where using more than 32 characters caused an error. 

 PAN-121944 

 Fixed an issue where the Device Connectivity status
...
with the logging service was successful. 
 ...
 Fixed an issue on Panorama where WildFire ® cloud
content download failed for content deployment to the WF-500 appliance. 

 PAN-119982 

 Fixed an issue where template variable view
...
cloud. 
 ...
 Fixed an issue where firewall policy configurations
displayed [object Object] instead of the
object names. 

 PAN-115896 

 Fixed an issue where the static route path
...
and failed with the following error message: failed to execute op command . 
 ...
 Fixed an issue where removing a cipher from
an SSL/TLS profile did not take effect if it was attached to the
management interface. 

 PAN-112449 

 Fixed an issue that caused a process ( snmpd )
