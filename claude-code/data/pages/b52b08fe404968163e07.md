...
# Configure Pre-Logon Support for Prisma Agent Clear
...
Updated on 
 ...
 Learn how to configure pre-logon for Prisma Agent , enabling secure tunnel
 ...
 Configure pre-logon for Prisma Agent to establish a secure tunnel before a user
 ...
 service like Microsoft Certificate Services. Prisma Agent uses these
 certificates for device authentication during the pre-logon process. 

 Configure the Prisma Agent settings to enable the pre-logon feature. 
 ...
 Prisma Agent only in pre-logon mode. Specifically, the
 ...
 Throughout the pre-logon process, the Windows login screen will show the Prisma Agent status. If the device has never been enrolled (meaning the agent has
 ...
 Starting in Prisma Agent version 26.2, pre-logon device tunnel enhancements
