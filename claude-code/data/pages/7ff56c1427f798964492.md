---
url: https://docs.paloaltonetworks.com/ngfw/help/10-2/network/network-routing-routing-profiles/network-routing-routing-profiles-bgp
...
to a peer group or peer. You can apply a Redistribution profile
...
(_), or hyphen (-), and contain zero or more alphanumeric characters,
...
an alphanumeric character, underscore (_), or hyphen (-), and contain zero
or more alphanumeric characters, underscore (_) or hyphen(-). No
dot (.) or space is allowed. 
...
routes being redistributed into BGP (range is 1 to 65,535). 
 ...
 includes a Metric Action along with a Metric Value, that value will
 be applied to the redistributed route. If no Metric Value is defined
 within the route map, the firewall will use default metric values.
 This means that routes redistributed via BGP will retain the same
 metric value that they had on the originating device. For example,
...
routes being redistributed into BGP (range is 1 to 65,535). 
 ...
 within the route map, the firewall will use default metric values.
 This means that routes redistributed via BGP will retain the same
 metric value that they had on the originating device. 
...
being redistributed into BGP (range is 1 to 65,535). 
 ...
 a Metric Value, that value will be applied to the redistributed
 route. If no Metric Value is defined within the route map, the
 firewall will use default metric values. This means that routes
 redistributed via BGP will retain the same metric value that they
 had on the originating device. For example, if an OSPF route is
...
being redistributed into BGP (range is 1 to 65,535). 
 ...
 Select a Route Map to specify the match criteria that determine which
 RIPv2 routes to redistribute. Default is
 None . If the route map Set configuration
 includes a Metric Action along with a Metric Value, that value will
 be applied to the redistributed route. If no Metric Value is defined
 within the route map, the firewall will use default metric values.
