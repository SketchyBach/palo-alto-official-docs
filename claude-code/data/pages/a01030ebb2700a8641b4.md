---
url: https://cortex-docs.paloaltonetworks.com/cortex-xdr-5.x/detect-investigate-and-respond-to-threats/investigate-and-respond-to-cases/automation/engines/install-an-engine/podman/troubleshoot-podman
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Resolve common Podman issues on Cortex XDR engines. 
 ...
 This can be caused by duplicate lines for Cortex XDR in /etc/subuid and /etc/subgid . 

 To fix this issue: 

 Check if the /etc/subuid file contains multiple lines that start with the Cortex XDR username (usually demisto). For example: 
 ...
 If this is the case, edit the file as root, and remove the extra line(s) for Cortex XDR. The line you should keep is the one that ends with 200000:65536. Continuing with the above example, here is the end result: 
 ...
 When encountering errors in Cortex XDR that are Podman related, such as: 
 ...
 When installing a Cortex XDR engine on a RHEL system (version 8 or later), or when running an integration on such an engine, you get a permission error for a path under /run (for example /run/user/0 or /run/libpod ). 
