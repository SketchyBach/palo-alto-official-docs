---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-30/vm-series-plugin-304/addressed-issues-in-vm-series-plugin-304.html
fetched_at: 2026-09-16T12:58:57Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 3.0.4 Clear
...
Updated on 
 ...
 Fixed an issue that prevented bootstrapping
of the VM-Series firewall on OpenStack when deployed with SR-IOV
enabled on the dataplane interfaces. 

 PLUG-11441 

 Fixed an issue that prevented a successful commit or installation of Antivirus or
 CTD updates due to lack of space. 

 PLUG-11317 

 Fixed an issue that prevented the VM-Series firewall on OCI from accepting an SSH key and
 first 

 upon first login and, therefore, require the default username and password. 

 PLUG-11022 

 Fixed an issue that caused all secondary
IP addresses to be lost on the VM-Series firewall for Azure when
an HA failover event occurs. 

 PLUG-10743 

 Fixed an issue that prevented the init-cfg.txt
file from being applied to a public cloud VM-Series firewall at
boot up if the bootstrap package software folder contains a PAN-OS
image file that does not match the version used to deploy the firewall. 
