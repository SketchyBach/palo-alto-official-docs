---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-vmware-nsx/vmware-nsx-plugin-103/addressed-issues-in-panorama-plugin-for-vmware-nsx-103.html
fetched_at: 2026-09-16T13:00:16Z
source: palo-alto-main
---

# Addressed Issues in Panorama Plugin for VMware NSX 1.0.3 Clear
...
in the Panorama plugin for VMware NSX 1.0.3. 

 PLUG-306 

 Fixed an issue where the Panorama plugin
for VMware NSX stopped responding when Panorama received a multi-line
response or an empty container set from the NSX Manager and it could
not update the Dynamic Address Group properly; the issue is now
resolved. 

 PLUG-302 

 Fixed two issues with how the Panorama
plugin for VMware NSX handles the deletion of a security group and
the creation of an empty security group on the NSX Manager. With
this fix, when the NSX Manager sends a delete notification for a
security group, the plugin handles the update gracefully. It also
addresses the issue where Dynamic Address Groups were not displaying
in the match criteria list on Panorama when you created an empty
security group on NSX Manager. 
