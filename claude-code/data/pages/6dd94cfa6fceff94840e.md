---
url: https://docs.paloaltonetworks.com/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-9-addressed-issues
fetched_at: 2026-08-13T17:13:54Z
source: palo-alto-main
---

# PAN-OS 8.1.9 Addressed Issues Clear
...
and resulted in the following error message: Server error : Failed to execute op command . 
 ...
 Fixed an issue where after you changed the
...
firewall displayed the following error message: Unknown user group cns\Proxy Full . 
 ...
 Fixed an issue where an internal path monitoring
failure due to a buffer leak caused the firewall to reboot. 

 PAN-117729 

 Fixed an issue where the firewall incorrectly
displayed application dependency warnings ( Policies Security ) after you initiated
a commit. 

 PAN-117149 

 Fixed an issue on firewalls configured with
...
policy when sessions were redirected or denied. 
 ...
 Fixed an issue where users were unable to
...
VPN Security policy rule ( Policies Security <GP-VPN-Security-policy-rule> User ). 

 PAN-116848 

 Fixed an issue where multiple device group
...
fix, you must run the debug rasmgr delay-nh-update CLI command. 
 ...
 Fixed an issue where a session created from
...
with the "in" and "not in" configurations ( Objects Log Forwarding Add Add Filter Filter Builder )
and resulted in the following error message: Invalid filter <Log Forwarding profile name> match-list -> <match list profile-name> -> filter is invalid . 
 ...
 Fixed a rare issue where a commit caused
...
gateway objects for managed firewalls. 
 ...
 Fixed an issue where all the log collectors
did not get queued when you configured more than 32 collector groups. 

 PAN-114548 

 Fixed an issue where the firewall discarded
...
configured the Device Group with large group hierarchies. 
 ...
 Fixed an issue where the firewall created
...
of the GlobalProtect pool. 
 ...
 Fixed an issue with a memory leak on Panorama
...
which caused the MP to restart. 
 ...
 A security-related fix was made to correct
...
which caused the management plane to stop responding. 
 ...
 Fixed an issue where an escape ( \ ) character
was added to HTTP logs when a log contained a comma. 

 PAN-112577 

 Fixed an issue on a VM-Series firewall in
...
viewed a scheduled report that required data anonymization. 
 ...
 Fixed an issue where IPv4 BGP routes were
...
( Commit Push to Devices Edit Selections or Commit Commit and Push Edit Selections ) was disabled
...
error. 
 ...
 Fixed an issue in Panorama where you were
...
that did not support it. 
 ...
 Fixed an issue where you were unable to
generate a custom report ( Monitor Manage Custom Report <device-name> Report Setting ). 

 PAN-111084 

 Fixed an issue where an out-of-memory condition
...
the neighboring firewall. 
 ...
 Fixed an intermittent issue where heartbeats
...
to stop responding and displayed the following error message: Dataplane is down: controlplane exit failure . 
 ...
 Fixed an issue where the firewall and Panorama
...
which caused the web browser to timeout. 
 ...
 Fixed an issue where a log collector settings
...
or high availability (HA) sync. 
 ...
 Fixed an issue where group-based policy
...
responding and caused a failover event. 
 ...
 Fixed an issue where, after you upgrade
...
to groups. 
 ...
 Fixed an issue on GlobalProtect where Security
...
you added a new address object to an existing address group. 
 ...
 Fixed an issue where device administrators
were unable to manually upload signature files ( Device Dynamic Updates )
...
took longer than expected to update the Managed Collectors ( Panorama Managed Collectors )
status. 

 PAN-104144 

 Fixed an intermittent issue where the management
...
more than one user was mapped to a single IP address. 
 ...
 Fixed a memory buffer allocation issue that
...
error message: File not found . 
 ...
 ( Japanese language only ) Fixed
an issue where the Interface Mgmt ( Network Network Profiles Interface Mgmt ) and Management Interface
Settings ( Device Setup Interfaces Management ) web interfaces
...
formats did not display consistently across different PAN-OS releases. 
 ...
 Fixed an issue where the FTP data connection
...
User-ID Agent as a Syslog Listener. 
 ...
 Fixed an issue where an external dynamic
list with an invalid IPv6 address range caused commits to fail. 

 PAN-89820 

 Fixed an intermittent issue where the Data
Filtering ( Monitor Data Filtering )
and Threat Log ( Monitor Threat )
did not display file names when you transferred multiple files into
a single session. 

 PAN-88987 

 Fixed an issue on the PA-5220 firewall with
