---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/overview-reporting/t-create-report
...
Updated on 
 ...
 You must have the Superuser role to create custom reports. 

 Check the workspace switcher before you start. The new report belongs to whatever is selected there, and that is also what it reports on by default: create it under Tenant to cover the whole tenant, or select a workspace first to cover that workspace alone. Working under Tenant additionally lets you narrow a report to particular workspaces, which is described in Step 2. 

 Create a Custom Report 

 Click Insights > Certificate Insights > Custom Reports . 
 ...
 Under Report Template , choose one: 

 Standard Reports to start with predefined filter criteria. 

 Custom Reports to define your own filter criteria. 

 In Filter Certificates By , click Add Filter to add or modify criteria. 

 In Columns Selector , choose the columns to include. 

 In Report Preview , review the first 20 results and selected columns. Adjust filters or columns as needed. 

 Click Continue . 

 Report on Specific Workspaces 

 When you build a report under Tenant , Filter Certificates By offers an extra filter category for the workspace a certificate belongs to. Its values are your workspace names, and you can select more than one, so a single report can cover a chosen group of workspaces rather than either one workspace or all of them. 

 This category is offered only under Tenant . Inside a workspace it is hidden, because the report already covers that workspace and nothing else. 

 Note: This filter selects certificates by the workspace that owns them. It does not change who owns the report, which is still decided by the workspace you had selected when you created it. 
 ...
 Access the custom reports inventory 
