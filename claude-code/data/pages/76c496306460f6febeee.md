---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-0/pan-os-release-notes/pan-os-9-0-addressed-issues/pan-os-9-0-6-addressed-issues.html
fetched_at: 2026-09-16T13:55:58Z
source: palo-alto-main
---

# PAN-OS 9.0.6 Addressed Issues Clear
...
improper communications to the LFC (CVE-2019-17440/PAN-SA-2019-0040). 
 ...
 Fixed an issue where a race condition caused pan_task and pan_com to
exit unexpectedly. 

 PAN-133491 

 Fixed an issue where Internet Protocol (IP)
to user mappings were not synced from the HUB virtual system (vsys)
to the non-hub vsys. 

 PAN-133448 

 Fixed an issue where the mprelay process
could crash during commit if the devsrvr process was restarted
before or during the commit. 

 PAN-133443 

 Fixed an issue where an XML API call incorrectly
masked the response, which prevented role based administrators from
running the response. 

 PAN-132501 

 Fixed an issue where after you switched
...
crashed or system logs were not written). 
 ...
 Fixed a rare timing window that caused an
Internal packet path monitoring failure. 

 PAN-129127 

 Fixed an issue where log export from maintenance
...
though the management interface Internet Protocol (IP) address was configured. 
 ...
 Fixed an issue where the disk usage calculation
...
temporary directory in PAN-OS ( CVE-2020-2016 ). 
 ...
 Fixed an issue where a purge script stopped
responding, which caused a process ( logrcvr ) to discard
incoming logs. 

 PAN-127089 

 Fixed an intermittent issue where the default
route did not redistribute to an OSPF Not-So-Stubby Area (NSSA). 

 PAN-126882 

 A security fix was made to address an OpenSSL
...
proxy sessions previously configured for GlobalProtect. 
 ...
 Fixed an intermittent issue where after
...
VM-Series where the configd process would crash. 
 ...
 Fixed an issue where a process ( openssl )
...
certificate validation. 
 ...
 Fixed an issue where multiple No valid URL filtering license warning
...
on a Panorama appliance had a memory leak during commit operations. 
 ...
 Fixed an issue where the real-time clock
(RTC) battery voltage exceeded the maximum threshold and triggered
alerts in the system log. 

 PAN-125157 

 Fixed an issue on the firewalls where the rasmgr process
...
Fixed an issue where logs were unexpectedly discarded. 
 ...
 Fixed an issue where a null point (policy)
dereference was causing a crash. 

 PAN-124882 

 Fixed an issue where traffic logs that contained
...
device group that included more than 200 rules. 
 ...
 A fix was made to address a missing XML
...
management server and two or more log collectors. 
 ...
 Fixed an issue where App-ID™ signatures
failed to match when there were more than 12 partial App-ID matches
within the same session. 

 PAN-123667 

 Fixed an issue where the snmpd process
...
child sessions. 
 ...
 Fixed an issue where the Dashboard did not
display the release dates for Application Version, Threat Version,
and Antivirus Version. 

 PAN-123167 

 Fixed an issue where a process ( mprelay ) stopped
responding. 
...
receive stage. 
 ...
 ( PA-800 Series and PA-220 firewalls
only ) Fixed an issue where the hrProcessorLoad.2 OID displayed
incorrect values. 

 PAN-121827 

 Fixed an issue where allow lists and auth
...
RTSP applications to perform less than the expected achievable performance. 
 ...
 Fixed an issue where downloading the GlobalProtect
...
tags with timeouts generated alert messages. 
 ...
 Fixed an issue where the varrcvr process crashed
...
web interfaces ( CVE-2020-2017 ). 
 ...
 Fixed an issue involving unexpected behavior
...
stopped responding when you configured virtual interfaces. 
 ...
 Fixed an issue where certificate revocation
...
session if certificate status is unknown . 
 ...
 Fixed an issue to improve the validation
...
passive firewall. 
 ...
 Fixed an issue where the Security Parameter
...
packet. 
 ...
 Fixed an issue in Panorama where the show config diff command
was not working correctly and produced unexpected output. 

 PAN-119822 

 Fixed an issue where you were not redirected
...
Routers > More Runtime Stats ). 
 ...
 Fixed an issue on Panorama M-Series and
virtual appliances where communication between two processes ( mgmtsrvr and logd )
stopped responding. 

 PAN-119545 

 Fixed an issue where updates (including
WildFire, antivirus, and so on) were intermittently failing. 

 PAN-119452 

 An enhancement was made to improve subsequent
...
the incorrect receive date. 
 ...
 Fixed an issue where a daemon ( dnsproxy ) incorrectly
handled TCP requests, which caused the daemon ( dnsproxy )
to stop responding. 

 PAN-119047 

 Fixed an issue where local user group names
...
to load user groups names with upper case characters. 
 ...
 Fixed an issue where moving multiple rules
...
caused the rules to move in a reversed order. 
 ...
 Fixed an issue in Panorama where on a high
...
components of Palo Alto Networks PAN-OS ( CVE-2020-2002 ). 
 ...
 Fixed an issue where the BGP Conditional
...
> Setup > Services > Global ). 
 ...
 Fixed an issue where traffic logs and URL
Filtering logs did not display the URL for decrypted traffic. 

 PAN-118207 

 Fixed an issue where the Security Assertion
...
the SAML IdP server profile. 
 ...
 Fixed an issue where an API call against
...
caused Panorama to reboot after you executed the command. 
 ...
 Fixed an issue where application dependency
...
advertised. 
 ...
 Fixed an issue where some packets had incorrect
...
> GlobalProtect > Gateways > <gateway-name> > Agent > Video Traffic ). 
 ...
 An enhancement was made to enable administrators
...
from Panorama were overridden on the local firewall. 
 ...
 Fixed an issue in Panorama where Packet Capture was
...
on a DHCP configured interface. 
 ...
 Fixed an issue where GlobalProtect authentication
...
to stop responding when you do a local commit on the active-secondary firewall. 
 ...
 Fixed an issue where vsysadmins were unable
...
to. To view the locks in CLI run the new show commit-locks vsys and show config-locks vsys CLI commands. 
 ...
 Fixed an issue where a process ( configd ) exceeded
...
for newly added groups in the authentication profile Allow List. 
 ...
 Fixed an issue with Panorama on AWS where
...
( devsrvr ) to stop responding. 
 ...
 Fixed an issue where trunk interfaces were
...
peer groups. 
 ...
 Fixed an intermittent issue where a process ( useridd )
...
responding when another process ( masterd ) sent a signal
interruption after you upgraded from a PAN-OS 9.0 release to a PAN-OS
9.1 release. 
...
Devices > Health ) for the firewall. 
 ...
 Fixed an issue where an old GPRS tunneling
...
vulnerability in PAN-OS ( CVE-2020-1998 ). 
 ...
 Fixed an issue where a firewall had a race
...
buffer. 
 ...
 An enhancement was made to enable you to
...
dataplane process crashes, making the crash analysis difficult. 
 ...
 Fixed an issue on Panorama where the ACC tab
