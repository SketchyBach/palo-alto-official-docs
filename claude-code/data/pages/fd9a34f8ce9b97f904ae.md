---
url: https://cortex-docs.paloaltonetworks.com/cortex-cloud-runtime-security/onboard-and-configure/deployment-steps-and-checklist/cloud-service-provider-csp-onboarding/microsoft-azure-cloud-onboarding/how-to-onboard-microsoft-azure
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Allow connection to private serverless functions: Turn on this optional setting during Microsoft Azure onboarding, or at any time afterward, to extend Cortex Cloud scan coverage to private Azure Functions. By default, Azure Functions configured with Virtual Network (VNet) integration or Private Endpoints block external access. While Cortex Cloud still successfully discovers these functions, it cannot access their underlying code to inspect for vulnerabilities, misconfigurations, or compliance violations. Turning on Allow connection to private serverless functions gives Cortex Cloud the authorized access needed to inspect the code of these isolated Azure Functions without exposing them to the public internet. This setting does not modify VNet integration, Private Endpoint configuration, or any Azure network rule.

 Important: Enabling Allow connection to private serverless functions requires additional Azure permissions. Download and run the Terraform template to grant the permissions required to scan private serverless functions—functions reachable only over a private network. Until you apply the template, Cortex Cloud does not scan private serverless functions. 
