---
url: https://docs.paloaltonetworks.com/ngfw/help/11-2/objects/objects-decryption-profile/settings-to-control-decrypted-ssl-traffic
...
options to control server certificates for decrypted traffic. 
 ...
 Terminate the TLS connection if the server
certificate is expired. This prevents users from accepting expired
certificates and continuing with an TLS session. 
...
prevent access to potentially insecure sites. 
 ...
 Terminate the TLS session if the server
...
attack. 
 ...
 Terminate the TLS session if a server returns
...
timeout 
 ...
 Terminate the TLS session if the certificate
...
(OCSP) if you are concerned about timing out valid certificates. 
 ...
 Limits the certificate extensions used in
the dynamic server certificate to key usage and extended key usage. 

 Restrict certificate extensions if your deployment
requires no other certificate extensions. 

 Append certificate's CN value to SAN extension 

 Enable the firewall to add a Subject Alternative
...
even if a server certificate contains only a CN. 
...
options to control unsupported TLS applications. 
 ...
 Terminate sessions if PAN-OS does not support
...
weaker protocol. 
 ...
 Terminate the session if the cipher suite
specified in the TLS handshake if it is not supported by PAN-OS. 

 Block sessions that use cipher suites you
...
Don’t allow users to connect to sites with weak cipher suites. 
 ...
 Terminate sessions with client authentication
...
unreachable. 
 ...
 Terminate sessions if a hardware security
module (HSM) is not available to sign certificates. 
...
handle encrypted traffic if the HSM isn’t available. 
 ...
 Terminate the session if system resources
are not available to process the TLSv1.3 handshake (instead of downgrading
to TLSv1.2). 

 Whether to block sessions when resources aren’t
...
to TLSv1.2. 
 ...
 The firewall processes and inspects HTTP/2
...
weaker protocol. 
 ...
 Terminate the session if the cipher suite
used is not supported by PAN-OS. 
...
unreachable. 
 ...
 Terminate sessions if a hardware security
module (HSM) is not available to decrypt the session key. 
...
for TLS session traffic. 
 ...
 Enforce the use of minimum and maximum protocol
versions for the TLS session. 

 Min Version 

 Set the minimum protocol version that can
be used to establish the TLS connection. 
...
users). 
 ...
 Set the maximum protocol version that can
...
selected minimum version are supported. 
...
applications so that you can enable TLSv1.3 for all other traffic. 
 ...
 Enforce the use of the selected key exchange
algorithms for the TLS session. 

 All three algorithms ( RSA , DHE ,
...
Forward Proxy or Inbound Inspection decryption. 
 ...
 Enforce the use of the selected encryption
algorithms for the TLS session. 
...
a Decryption policy rule for just those sites. 
 ...
 Enforce the use of the selected authentication
algorithms for the TLS session. 
