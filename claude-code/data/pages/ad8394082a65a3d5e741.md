---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/device/device-setup-services/global-services-settings
fetched_at: 2026-08-13T16:51:35Z
...
Global Services Settings 
 ...
 Represents the IP address or host name of the server
 ...
 If you enable this option, the firewall or Panorama
 ...
 Primary and secondary DNS servers to provide domain name
 resolution. 

 A DNS proxy configured on the firewall as an alternative to
 configuring DNS servers. If you enable a DNS proxy, you must
 enable Cache and EDNS
 Cache Responses ( Network DNS Proxy Advanced ). 

 Primary DNS Server 

 Enter the IP address of the primary DNS server for DNS
 ...
 ( Optional ) Enter the IP address of a
 ...
 Set a limit on how fast the firewall refreshes FQDNs
 ...
 Specify the length of time (in minutes) that the
 ...
 If the firewall needs to use a proxy server to reach
 ...
 Enter the username for the administrator to enter when
 ...
 Enter the IP address or hostname of an NTP server that
 ...
 You can enable the firewall to authenticate time
 updates from an NTP server. For each NTP server, select the type of
 authentication for the firewall to use: 

 None (default)—Select this option to
 disable NTP Authentication. 

 Symmetric Key —Select this option for
 the firewall to use symmetric key exchange (shared secrets)
 to authenticate time updates from the NTP server. If you
 select Symmetric Key, continue by specifying the following
 values: 

 Key ID —Enter the Key ID
 (1–65534). 

 Algorithm —Select the MD5, SHA1, SHA256, or SHA512 algorithm to use for NTP
 authentication. 

 Authentication Key/Confirm Authentication
 Key —Enter and confirm the
 authentication key for the authentication
 algorithm. 

 Autokey —Select this option for the
 firewall to use autokey (public key cryptography) to
 authenticate time updates from the NTP server. 
