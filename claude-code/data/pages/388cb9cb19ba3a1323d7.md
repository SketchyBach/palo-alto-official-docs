---
url: https://docs.paloaltonetworks.com/pan-os/9-1/pan-os-release-notes/pan-os-9-1-addressed-issues/pan-os-9-1-10-addressed-issues
fetched_at: 2026-08-13T17:14:07Z
source: palo-alto-main
---

# PAN-OS 9.1.10 Addressed Issues Clear
...
not found in the cloud. 
 ...
 Fixed an issue where custom URL categories
...
firewall. 
 ...
 Fixed an issue where commit jobs failed
...
the web interface due to incorrect dataplane ownership. 
 ...
 Fixed an issue where DHCP leases were not
...
condition occurred due to a memory leak related to a process ( logrcvr ). 
 ...
 Fixed an issue where, when using the CLI
...
instead of failing or displaying a warning. 
...
dataplane systems. 
 ...
 Fixed an issue on Panorama where a user
...
address assignment. 
 ...
 Fixed an issue where the Device Name field
...
as aa memory leak issue related to the sslvpn process
that occurred when the firewall was configured as a GlobalProtect
satellite. 

 PAN-161112 

 Fixed an issue where a process ( useridd ) repeatedly
exceeded the virtual memory limit, which caused the process to stop
responding. 

 PAN-161025 

 Fixed an issue in Panorama where an administrator
...
> Device Deployment > GlobalProtect ). 
 ...
 Fixed an issue where the metadata from the
...
you must upgrade both Panorama and the firewall. 
 ...
 ( ZTP-capable firewalls only ) Fixed
...
firewall running PAN-OS 9.0 or later. 
 ...
 Fixed an issue in Panorama where a process ( configd )
stopped responding due to a race condition in the mongodb process. 

 PAN-159973 

 Fixed an issue where a local commit in the
Panorama management server caused the status to get out of sync
on the managed WildFire appliance. 

 PAN-159592 

 Fixed an issue where a Japanese keyword
...
to fill up. 
 ...
 Fixed an memory leak issue related to a
...
on spyware_state , which caused latency. 
 ...
 Fixed an issue where the PBF monitor was
...
dropped GPRS tunneling protocol (GTP-U) traffic with the message TEID=0x00000000 . 
 ...
 Fixed an issue where device deployment from
...
command had been issued in the past. 
 ...
 Fixed an issue on the firewall where a process ( useridd )
...
the following error message: data_plane_X: Exited 1 times, must be manually recovered. . 
 ...
 Fixed an issue where a process ( flow_mgmt ) repeatedly
...
following trace: flow_mgmt:pan_flow_dos_ager_invoke pan_sw_timer_100ms pan_sw_timer_invoke . 
 ...
 Fixed an issue where, if the OK button
...
are removed. 
 ...
 ( ZTP firewalls only ) Fixed an issue
...
template stack. 
 ...
 ( 7000-Series firewalls only ) Fixed
...
OOM condition, multiple processes stopped responding. 
 ...
 Fixed an issue where the Radius EAP authentication
...
push dynamic user groups to the managed firewalls. 
 ...
 Fixed an issue where the inactivity logout
...
to be high. 
 ...
 Fixed an issue where using XML special characters
...
Portals > Agent > App ) disrupted portal connectivity. 
 ...
 Fixed an issue where the firewall treated
...
for dynamic address groups or templates from Panorama did not work. 
 ...
 Fixed an issue where the software QoS shaping
queue processing was not properly applied on multicast traffic. 

 PAN-153228 

 Fixed an issue where, when IPSec tunnels
...
and thresholds. 
 ...
 Modified the diff algorithm for when a configuration
...
Format (LEEF) and common event format (CEF). 
 ...
 Fixed an issue where it was possible via
...
the Panorama management server. 
 ...
 Fixed an issue where, when printing External
...
from PAN-OS 9.0). With this fix, the value is set correctly. 
 ...
 Fixed an issue where, even when tunnel interface
...
window did not show correct information. 
 ...
 Fixed an issue with the Panorama web interface
...
and processed. 
 ...
 Fixed an issue where a satellite firewall
...
schedule ( Device > Dynamic Updates ). 
 ...
 Fixed an issue where merged configurations
were unable to be exported from Panorama-managed firewalls using
the PAN-OS XML API. 

 PAN-144057 

 Fixed a rare issue where, when aggregate
...
> Device > Setup > Management ). 
 ...
 Fixed an issue where period Windows Management Instrumentation
(WMI) probing did not work until a process ( useridd )
was restarted. 

 PAN-138869 

 Fixed an issue where some threat logs in
Panorama were not displayed when filtered by Threat-ID name. 

 PAN-136635 

 Fixed an issue where HIP-related objects
