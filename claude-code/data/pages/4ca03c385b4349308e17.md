---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/network/network-interfaces-cellular
...
Network > Interfaces > Cellular 
 ...
 Enable to forward DHCP broadcasts from LAN-side devices to a
 centralized DHCP server across the cellular WAN. Enabling or
 disabling this setting reboots the modem and terminates active
 cellular data connections. Applies to parent interfaces only, not to
 subinterfaces. Based on your network deployment, consider disabling
 NAT on the cellular interface when you enable this setting. 

 Make sure that you review your network
 deployments and any dependencies before disabling NAT on the
 Cellular interface. 
 ...
 APN Profile Specify the APN/DNN profile that
 you want the SIM card to use by default. The SIM slot uses this APN/DNN
 profile unless you override it with a custom profile. If you don't
 specify an APN/DNN profile, the firewall or Panorama uses the default
 auto APN/DNN profile. If you need to
 specify a custom APN profile, you can do so by selecting Cellular Interface Advanced APN/DNN Profile . 

 APN Profile 

 Cellular Advanced APN/DNN Profile 

 Add the default APN/DNN
 Profile . 
