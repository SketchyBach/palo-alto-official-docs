---
url: https://docs.paloaltonetworks.com/ngfw/help/11-2/policies/policies-tunnel-inspection/building-blocks-in-a-tunnel-inspection-policy
fetched_at: 2026-08-13T16:49:02Z
source: palo-alto-main
---

# Building Blocks in a Tunnel Inspection Policy Clear
...
Building Blocks in a Tunnel Inspection Policy 
 ...
 Select Policies Tunnel Inspection to add a
...
a Tunnel Inspection Policy 
 ...
 Enter a name for the Tunnel Inspection policy beginning
...
for the Tunnel Inspection policy. 
 ...
 ( Optional ) Enter one or more tags for
...
you want the firewall to inspect: 
 ...
 GRE —Firewall
inspects packets that use Generic Route Encapsulation in the tunnel. 

 GTP-U —Firewall inspects packets that
use the General Packet Radio Service (GPRS) tunneling protocol for
user data (GTP-U) in the tunnel. 

 Non-encrypted IPSec —Firewall inspects
packets that use non-encrypted IPSec (Null Encrypted IPSec or transport
mode AH IPSec) in the tunnel. 

 VXLAN —Firewall inspects a VXLAN payload
...
remove a protocol from your list, select the protocol and Delete it. 
 ...
 Specify whether the firewall will inspect One
...
as inspection only occurs on the outer layer. 
 ...
 ( Optional ) Drop packets that contain
...
a protocol inside the tunnel that the firewall cannot identify. 
 ...
 ( Optional ) Enable this option to return
...
and destination zones of the outer tunnel. 
 ...
 If you Enable Security Options ,
select a tunnel zone that you created, and the inner content will
use this source zone for the purpose of policy enforcement. 
...
zone apply to the inner content source zone also. 
 ...
 If you Enable Security Options ,
select a tunnel zone that you created, and the inner content will
use this destination zone for the purpose of policy enforcement. 
...
destination zone apply to the inner content destination zone also. 
 ...
 ( Optional ) Enter a monitor name
to group similar traffic together for monitoring the traffic in
logs and reports. 

 Monitor Tag (number) 

 ( Optional ) Enter a monitor tag number
...
use the VXLAN Network Identifier (VNI) from the VXLAN header. 
 ...
 ( Optional ) Select this option to generate
...
Start setting in the Security Policy rule that applies to the session. 
...
you will never see that there is an active GRE tunnel in the ACC. 
 ...
 ( Optional ) Select this option to capture
...
End setting in the Security Policy rule that applies to the session. 
 ...
 ( Optional ) Select a Log Forwarding profile
...
is inspected. 
 ...
 If you configure a VXLAN ID you can use it as a
matching criteria to restrict traffic inspection to specific VNIs. 

 ( Optional ) A name beginning with an
