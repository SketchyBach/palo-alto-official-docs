---
url: https://docs.paloaltonetworks.com/ngfw/help/10-2/device/device-certificate-management-ssltls-service-profile
...
must deploy that CA certificate to the CTL in client browsers. 
 ...
 To add a profile, click Add , complete
...
profile is available only for the vsys selected in the Device tab, Location drop-down. 

 Certificate 

 Select, import, or generate a server certificate
to associate with the profile (see Manage
Firewall and Panorama Certificates ). 

 Do
not use certificate authority (CA) certificates for SSL/TLS services;
use only signed certificates. 
...
the earliest supported TLS version; do not select TLSv1.0 . 
