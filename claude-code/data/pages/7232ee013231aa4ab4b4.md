---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-3-addressed-issues.html
fetched_at: 2026-09-16T10:42:19Z
source: palo-alto-main
---

# PAN-OS 8.1.3 Addressed Issues Clear
...
Updated on 
 ...
 Fixed an issue where RAID rebuilding after
disk replacement either failed or took longer than expected. 

 PAN-101101 

 Fixed an issue with inconsistencies in the
...
even though the password has not, yet, expired. 
 ...
 Fixed an intermittent issue where the dataplane
...
dropped GTPv2-C Modify Bearer Response packets due to a sequence-number mismatch. 
 ...
 Fixed an issue where the route ( routed )
...
firewall. 
 ...
 Fixed an issue where emails were not sent
using the configured email service route as expected. 

 PAN-99585 

 Fixed an issue where a PA-3200 Series firewall
processed traffic that was in suspended mode 

 PAN-99584 

 Fixed an issue where a PA-5200 Series firewall
processed traffic that was in suspended mode. 
...
packets. 
 ...
 Fixed an issue on a VM-Series firewall on
Azure where a process ( logrcvr ) stopped responding. 

 PAN-99316 

 Fixed an issue where the SAP Success Factor
...
to misinterpret the cookies. 
 ...
 Fixed an issue where NetFlow caused an invalid
...
dropped ARP packets and increased the flow_arp_throttle counter. 
 ...
 Fixed an issue where a firewall frequently
...
was incorrectly identified as an SFP (1Gbps) transceiver. 
 ...
 Fixed an issue where user-account group
members in subgroups ( n +1) were unnecessarily
queried when nested level was set to n . 

 PAN-98116 

 Fixed an issue where PA-3000 Series firewalls
...
applications incorrectly. 
 ...
 Fixed an issue on PA-3000 Series, PA-3200
...
behind a proxy server. 
 ...
 Fixed an issue where an error ( mailsend: failed to get stat of file )
...
though there were no issues with the firewall sending PDF reports. 
 ...
 Fixed an issue where device-group operations
...
as expected even though User-ID mappings were present on the dataplane. 
 ...
 Fixed an issue with the Japanese language
...
from the CLI and, though available from the web interface ( Device Setup Management Banners and Messages Banners ),
...
running PAN-OS 8.1.2 was unable to connect to the Logging Service. 
 ...
 Fixed an issue where the default for newly
...
unsupported versions was disabled ( Objects Decryption Decryption Profile ).
...
did not display as expected on Panorama ( Template Device Dynamic Updates ). 

 PAN-96918 

 Fixed an issue where an unreachable DNS
...
that packet to be an inter-vsys inbound packet. 
 ...
 Fixed an issue where endpoints could not
...
Online Certificate Status Protocol (OCSP) to validate certificates ( Network GlobalProtect Portals <portal> Authentication ). 
 ...
 Fixed an issue where PA-220 firewalls that
...
in a dataplane restart. 
 ...
 Fixed a memory corruption error that caused
the dataplane to restart when content decode length was zero. 

 PAN-96113 

 Fixed an issue where the show routing protocol bgp rib-out CLI
...
subnetwork as the BGP peer interface. 
 ...
 Fixed an issue where the GTP Protection
...
was attached. 
 ...
 Fixed an issue where Panorama virtual appliances
...
Policy was configured with Security Options (Tunnel Inspection zones). 
 ...
 Fixed an issue where browsers failed to
...
decryption of custom response pages up to 17,999 bytes. 
 ...
 Fixed an issue where the parent device group
...
which impacts TACACS+ servers and causes a failed authorization. 
 ...
 Fixed an where issue where non-local administrators
using TACACS were unable to log in to the CLI. 

 PAN-95090 

 Fixed an issue where imported custom applications
did not display in Security Policies that were created through the
web interface. 

 PAN-95061 

 Fixed an issue on PA-220 firewalls where
...
to passive state). 
 ...
 Fixed an issue where firewalls receiving
IP addresses via DHCP failed to resolve FQDN objects to an IP address. 

 PAN-94777 

 Fixed an issue where a 500Internal Server error
occurred for traffic that matched a Security policy rule with a
URL Filtering profile that specified a continue action ( Objects Security Profiles URL Filtering ) because the
firewall did not treat the API keys as binary strings. 

 PAN-94698 

 Fixed an issue on PA-5000 Series firewalls
...
Setting ( Policies <rule> Actions ). 
 ...
 Fixed an issue with firewalls in a high
...
request. 
 ...
 Fixed an issue where an XML API call to
...
in a parse error. 
 ...
 Fixed an issue on PA-800 Series, PA-3200
...
as expected. 
 ...
 Fixed an issue where the default static
...
per the schedule. 
 ...
 Fixed an issue where FTP traffic failed
and hit an incorrect security policy due to missing predict sessions. 

 PAN-94291 

 Fixed an issue where a firewall failed to
...
firewall rebooted. 
 ...
 Fixed an issue where, when an administrator
...
when interfaces received IP address through DHCP. 
 ...
 Fixed an issue on an M-100 appliance where
logging stopped when a process ( vldmgr ) stopped responding. 

 PAN-93864 

 Fixed an issue where the password field
...
attached the certificate profile to the portal configuration. 
 ...
 Fixed an issue where the Panorama task manager
...
and CVE-2018-0489). Refer to www.kb.cert.org/vuls/id/475445 for details. 
 ...
 Fixed an issue on PA-200 firewalls where
...
packets. 
 ...
 Fixed an issue where the non-session-owner
...
failed. 
 ...
 Fixed an intermittent Panorama issue where,
...
that do not affect traffic. 
 ...
 Fixed an issue where the Vulnerability Protection
...
package are displayed. 
 ...
 Fixed an issue in an NSX environment where
...
interface. 
 ...
 Fixed an intermittent issue where a race
...
server. 
 ...
 Fixed an issue where the firewall was intermittently
sending incorrect bytes-per-packet values for some flows to the
NetFlow collector. 

 PAN-92105 

 Fixed an issue where the Panorama Log Collectors
...
an issue with premature deletion of proxy sessions. 
 ...
 Fixed an issue where a certificate was loaded
without a digital signature, which caused the configuration ( configd ) daemon
to stop responding. 

 PAN-91316 

 Fixed an issue where you couldn't unlock
...
column ( Device Administrators ). 

 PAN-91259 

 Fixed an issue where the predict session
...
these sessions. 
 ...
 Fixed an issue where, in a multiple virtual
...
sessions from the hardware session offload processor. 
 ...
 Fixed an issue on Panorama where the Last
Commit State column ( Panorama Managed Devices ) did not get
updated after a Template-Only configuration push to firewalls. 

 PAN-90535 

 Fixed an issue where the firewall unnecessarily
...
attribute from authentication server ( Network GlobalProtect Gateways <gateway> Agent Client Settings <clients_configuration> IP Pools ) in the GlobalProtect
...
returned an error. 
 ...
 Fixed an issue where the firewall was sending
...
unrecoverable until the NPC was restarted. 
 ...
 Fixed an issue where a firewall did not
...
identified as an SFP (1Gbps) transceiver. 
 ...
 Fixed an issue where a Panorama virtual
...
default tunnel interface ( Network GlobalProtect Gateways <gateway> General ) even after you enabled
...
format. 
 ...
 PAN-79291 
 Fixed an intermittent issue with ZIP hardware
offloading where firewalls identified ZIP files as threats when
they were sent over Simple Mail Transfer Protocol (SMTP). 

 PAN-42036 

 Fixed a rare intermittent issue on PA-800
