---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/network/network-interfaces-vlan
fetched_at: 2026-08-13T16:52:04Z
source: palo-alto-main
---

# Network > Interfaces > VLAN Clear
...
remove the current NetFlow server assignment from the interface. 
 ...
 Select a VLAN or click VLAN to
...
route that points to the default gateway that the DHCP server provides. 
 ...
 Select to configure the firewall (as a DHCP
...
node. 
 ...
 Select to enable the router advertisement
...
only if you enable Send RA . 
 ...
 Valid
Lifetime — Length of time, in seconds, that the firewall considers
the address as valid. The valid lifetime must equal or exceed the Preferred
...
can use it to send and receive traffic. After the preferred lifetime expires,
the firewall cannot use the address to establish new connections,
but any existing connections are valid until they exceed the Valid Lifetime .
Default is 604,800. 

 On-link —Select if systems with IP addresses
within the advertised prefix are reachable without a router. 
...
the address has a longer lifespan). 
...
hyphens, periods, and underscores. 
 ...
 GUA from
Pool —Global Unicast Address (GUA) that comes from the
Prefix Pool chosen. Getting this GUA is the goal of using prefix
delegation. 

 ULA —Unique Local Address is a private
address in the address range fc00::/7 for connectivity within a
private network. Select ULA if there is no DHCP server. 
...
the GUA. 
 ...
 Dynamic —The
DHCPv6 Client is responsible for choosing an identifier to configure
...
or Type = Inherited 
 ...
 VLAN Interface IPv6 Router Advertisement, Type
...
(Type = Static) 
 ...
 DNS Support is available if you Enable
Router Advertisement on the Router Advertisement tab. 
...
(Type = DHCPv6 Client or Type = Inherited) 
 ...
 VLAN Interface IPv6 DNS Support, Type = DHCPv6 Client
or Type = Inherited 
...
on the list. 
 ...
 Configure the firewall with the suffixes that
...
and is basically saying, “send me the packets meant for these addresses.” 

 ( Recommended )
Enable NDP Proxy if you are using Network Prefix Translation IPv6
(NPTv6). 

 If you Enable NDP Proxy , you
...
interface from the DHCP server. 
 ...
 Select a Certificate Profile that
...
a domain name. 
 ...
 Select the DDNS vendor (and version number)
