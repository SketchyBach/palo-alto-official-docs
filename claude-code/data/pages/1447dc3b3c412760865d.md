---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-0/pan-os-release-notes/pan-os-9-0-addressed-issues/pan-os-9-0-9-addressed-issues.html
fetched_at: 2026-09-16T13:55:56Z
source: palo-alto-main
---

# PAN-OS 9.0.9 Addressed Issues Clear
...
Updated on 
 ...
 Fixed an issue where the WF-500 cluster
...
required Superuser privileges to perform a full commit. 
 ...
 ( VM-Series firewalls in Azure environment
...
during a commit when Netflow was enabled. 
 ...
 Added a mechanism to detect corrupted or
...
using API to configure larger sets of objects (more than 25 objects). 
 ...
 Fixed an issue where, in a high availability
...
Services plugin to 1.6.0-h1. 
 ...
 Fixed a configuration lock issue where Panorama
timed out due to a process ( configd ) being unable to
read another process ( mongod ). 
...
categorization. 
 ...
 Fixed an issue where a process ( configd ) consistently
...
was not set. 
 ...
 Fixed an issue in Panorama where the Security
...
machine. 
 ...
 Fixed an issue in Panorama where a memory
leak occurred during an HA sync commit. 

 PAN-134309 

 Fixed an issue where a process ( devsrvr ) restarted
...
the maximum transmission unit (MTU) size limit was lower than expected. 
 ...
 Fixed an issue in Panorama where custom
...
upgrade to 8.1.10 or 9.0.4 and later versions. 
 ...
 Made improvements to the log storage for
VM-Series for NSX Panorama. 

 PAN-131945 

 Fixed an issue where Device > VM-Series on
the firewall web interface showed a blank screen. 

 PAN-131792 

 Fixed an issue where the Name log
...
caused a memory leak on a process ( sslvpn ). 
 ...
 Fixed an issue where reports from Panorama
...
logs. 
 ...
 Fixed an issue on Panorama where Applications
and Threats content update deployment failed due to the content
version date check. 

 PAN-130558 

 Fixed an issue on the firewalls where SNMP
...
were two zones with same names under different virtual systems. 
 ...
 Fixed an issue in Amazon Web Services (AWS)
where Ethernet1/1 failed DHCP renewal after the hour. 

 PAN-129281 

 Fixed an issue where a process ( useridd ) restarted
...
import of software, content, and plugins. 
 ...
 Fixed an issue where User-ID running on
...
scanners. 
 ...
 Fixed an issue where system log entries
...
multiple times, which caused the firewall to reboot. 
 ...
 Fixed an issue where the /opt/pancfg partition
...
with significant delay (15-60 seconds). 
 ...
 Added two ciphers for GlobalProtect Portal
TLS connections. 
...
using different domain override settings. 
 ...
 Fixed an issue where a process ( configd ) crashed
...
and affected client performance while accessing HTTP video applications. 
 ...
 Fixed an issue with log reading performance
when using WMI for server monitoring with PAN-OS integrated User-ID
agent. 

 PAN-116720 

 A fix was made to address a reflected cross-site
...
perform administrative actions ( CVE-2020-2036 ). 
 ...
 Fixed an issue where Static, Connected,
...
stopped responding. 
 ...
 Fixed an issue where Panorama ran out of
memory due to high memory usage on a process ( configd ). 

 PAN-106763 

 Fixed an issue where the dataplane crashed
...
advertised by the Area Border Router (ABR). 
 ...
 Fixed an issue on an M-Series appliances
