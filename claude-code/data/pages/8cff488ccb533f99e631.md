---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-palo-alto-panorama
fetched_at: 2026-09-16T07:25:20Z
source: palo-alto-main
---

# Provision to Palo Alto Networks Panorama Clear
...
Updated on 
 ...
 On the right, click the More Options menu button for the machine that you want to provision a certificate to, and select Provision . 

 From the Search a certificate by name, expiration, or fingerprint field, begin typing the certificate name you want to provision. Click the certificate when it appears in the list. 

 Verify that you've selected the correct certificate by reviewing the Subject DN, Validity, and Fingerprint. 

 In the Certificate Name field, enter the name for this certificate as you want it to appear on your Palo Alto Networks Panorama. 

 Note: When provisioning a certificate to the Palo Alto Networks Panorama, Next-Gen Trust Security checks whether the name you enter in this field is already in use. If the name isn't in use, Next-Gen Trust Security will use it. 

 (Optional) In the Template field, select the template to use when creating the certificate. The certificate will be pushed to devices that are using this template. 

 (Optional) In the Installation Endpoint field, enter a TLS Service Profile to associate with the certificate. The certificate will be pushed to all devices using this profile, if it is shared. 

 If you don't want the certificate to be pushed when you save, toggle the Push certificate on save slider to Off . 

 Click Save . 

 Note: You can schedule your provisions daily, weekly, or monthly. Learn more 

 After saving, the certificate is pushed to the Palo Alto Networks Panorama tenant that you specified. The tenant's virtual service is updated to use that certificate, and an installation is created on the Installations tab. 
