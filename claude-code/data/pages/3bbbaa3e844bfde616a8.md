---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-cisco-aci/cisco-aci-plugin-201/known-issues-in-panorama-plugin-for-cisco-aci-201.html
fetched_at: 2026-09-16T13:00:24Z
source: palo-alto-main
---

# Known Issues in Panorama Plugin for Cisco ACI 2.0.1 Clear
...
in the Panorama plugin for Cisco ACI 2.0.1. 

 PLUG-4467 

 When adding match criteria to a dynamic
address group, if an extraneous space or character is added at the
end of the tag, the dynamic address group count on the Cisco ACI dashboard
does not match the dynamic address count displayed after clicking
the tile to view additional information. 

 Workaround: Remove any extraneous spaces or characters
from your dynamic address group configuration to resolve the count
mismatch. 

 PLUG-1550 

 The Panorama plugin for Cisco ACI stops monitoring if
it requests more memory than Panorama allocates to plugins. If you
do not allocate enough memory to your Panorama, you might encounter
this issue. 

 Workaround: Log in to the Panorama CLI and execute the
command request plugins reset-plugin only plugin plugin-name cisco .
This command restarts the plugin and releases the memory used by
the plugin. To avoid this issue in the future, increase the memory
of your Panorama. Allocate additional memory to
your Panorama virtual appliance to avoid this issue in the future. 

 PLUG-1387 

 The plugin does not immediately process new monitoring
definitions after you configure and commit them under Panorama Cisco ACI Monitoring Definitions . Instead,
the monitoring definition must complete the configured monitoring interval
before it begins processing the new monitoring definition. For example,
if the monitoring interval is set to 10 minutes, the monitoring
definition status will update after 10 minutes. 

 Workaround: Select Panorama Cisco ACI Monitoring Definition and
click Synchronize Dynamic Objects to force
Panorama to sync with the APIC before the monitoring interval expires. 
