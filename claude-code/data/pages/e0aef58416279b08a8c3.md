---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-kemp-loadmaster
fetched_at: 2026-09-16T07:25:19Z
source: palo-alto-main
---

# Provision to a Kemp Virtual LoadMaster Clear
...
Updated on 
 ...
 Use this procedure to provision a certificate from Next-Gen Trust Security to a Kemp Virtual LoadMaster machine. 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Installations > Machines . 

 Select the Kemp Virtual LoadMaster machine you want to provision a certificate to. 
 ...
 In Certificate Name , enter the name you want the certificate to use on the Kemp LoadMaster. 

 What if the certificate name is already in use? 

 When provisioning a certificate, Next-Gen Trust Security checks whether the name already exists on the Kemp LoadMaster: 
 ...
 In Virtual Service Name , enter the name of the virtual service to associate with the certificate. 

 In Virtual Service IP Address , enter the IP address for the virtual service. 

 In Service Port , enter the port number used by the virtual service. 

 From Protocol , select the network protocol used by the virtual service: 

 TCP 

 UDP 

 (Optional) To require a Server Name Indication (SNI) hostname, enable Require SNI hostname . 

 (Optional) To prevent the certificate from being pushed immediately, set Push upon saving to No . 

 Click Save . 

 After saving, Next-Gen Trust Security provisions the certificate to the specified Kemp LoadMaster virtual service, updates the service configuration, and creates an installation record on the Installations tab. 
