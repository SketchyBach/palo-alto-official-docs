---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/c-about-activity-logging
fetched_at: 2026-09-16T07:24:51Z
source: palo-alto-main
---

# Overview: Event Logging Clear
...
Updated on 
 ...
 To identify users who are experiencing login issues, you can create a filter that shows failed Next-Gen Trust Security login attempts.
The filtered results show which users experienced login failures and when those failures occurred. 

 Workspaces and the Event Log 

 Events belong to the workspace they happened in. Which events you see depends on the workspace you have selected: 

 Under Tenant , the log covers every workspace, and a Workspace column names the one each event came from. You can filter on that column to narrow the log to one workspace or to several at once. 

 Inside a workspace, the log covers that workspace alone. The Workspace column is not shown, because every row would name the workspace you are already in, and there is nothing to filter on. 

 There is no way to see events from two workspaces side by side without selecting Tenant , which requires tenant-level access. 

 What’s Next 

 Create and apply event log filters 
