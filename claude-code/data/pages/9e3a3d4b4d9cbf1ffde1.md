---
url: https://docs.paloaltonetworks.com/ngfw/help/11-2/network/network-routing-routing-profiles/network-routing-routing-profiles-filters
fetched_at: 2026-08-13T16:48:10Z
source: palo-alto-main
---

# Network > Routing > Routing Profiles > Filters Clear
...
route aggregation, and route redistribution. 
 ...
 Enter a name for the access list (maximum
...
access list; range is 1 to 65,535. 
...
the entry. The access list ends with an implicit Deny Any . 
 ...
 Address —In the subsequent Address field,
enter an IPv4 address and enter a Wildcard mask
to indicate a range of addresses. A zero (0) in the mask indicates
that bit must match the corresponding bit in the address; a one
(1) in the mask indicates a “don’t care” bit. 
...
enter an IPv6 address. 
 ...
 ( IPv6 only ) Select to match only
...
Address is Any or None . 
 ...
 Enter a name for the prefix list (maximum
of 63 characters). The name must start with an alphanumeric character,
underscore (_), or hyphen (-), and contain zero or more alphanumeric
characters, underscore (_) or hyphen(-). No dot (.) or space is
allowed. 
 ...
 Add an entry (rule)
and enter the sequence number of the rule in the list of rules for this
prefix list; range is 1 to 65,535. 
...
slash and prefix length. Optionally enter the prefix length that
the prefix must be Greater Than or Equal to
(range is 0 to 32 for IPv4; 0 to 128 for IPv6). Optionally enter
the prefix length that the prefix must be Less Than Or
Equal to (range is 0 to 32 for IPv4; 0 to 128 for IPv6).
For example, enter a Network of 192.168.3.0/24 with a prefix length Greater
Than Or Equal to 25 and a prefix length Less Than Or Equal to 26. 
 ...
 Enter a name for the AS_Path access list
...
the entry. 
 ...
 AS Path access lists end with an implicit Permit
Any rule. Use an AS Path access list to deny autonomous systems. 
 ...
 Enter a name for the community list (maximum
of 63 characters). The name must start with an alphanumeric character,
underscore (_), or hyphen (-), and contain zero or more alphanumeric
characters, underscore (_) or hyphen(-). No dot (.) or space is
allowed. 
...
The list ends with an implicit Deny Any rule. 
 ...
 Select one of the well-known communities
from the list or enter a community. 
 ...
 Enter a name for the BGP route map (maximum
of 63 characters). The name must start with an alphanumeric character,
underscore (_), or hyphen (-), and contain zero or more alphanumeric
characters, underscore (_) or hyphen(-). No dot (.) or space is
allowed. 
 ...
 Add an entry (rule)
and enter the sequence number of the rule in the list of rules for this
route map; range is 1 to 65,535. 
...
or none . 
 ...
 Select IPv4 or IPv6 as
the address family on which to match. 

 Address—Access List 

 Select an access list you created that specifies
the addresses to match. Default is None . 

 Address—Prefix List 

 Select a prefix list you created that specifies
...
you crated that specifies the route source to match. Default is None . 
 ...
 Mark the route as a less specific route
...
AS_PATH information from the aggregated routers was preserved. 
 ...
 Enter the Aggregator AS. The Aggregator
...
4,294,967,295. 
 ...
 Enter the aggregator’s Router ID (usually
a loopback address). 
 ...
 ( IPv6 only ) IPv6 has four address
...
the firewall to prefer global unicast addresses. 
 ...
 Select the source address with /prefix length
to set. 

 IPv4 Next-Hop 

 ( IPv4 only ) Select none , peer-address (Use
Peer Address) , or unchanged . 

 IPv6 Next-Hop 

 ( IPv6 only ) Select none or peer-address
(Use Peer Address) . 
...
or none . 
 ...
 Select to overwrite the Regular Community
with what is added in Regular Community field. 
 ...
 Select to overwrite the Large Community
...
or space is allowed. 
 ...
 Select the protocol to which routes are
being redistributed. 
 ...
 Leave unused numbers between sequence numbers
...
or none . 
 ...
 Enter the value to set the metric
