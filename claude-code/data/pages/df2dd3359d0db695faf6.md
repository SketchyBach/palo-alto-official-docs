---
url: https://cortex-docs.paloaltonetworks.com/data-security-documentation/onboard-and-configure/deployment-checklist/set-up-users-and-roles/user-group-management
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Create and manage user groups, roles, and scopes. 

 Manage user groups to assign roles and control user access. 
 ...
 This default method allows you to associate users with groups created and managed within Cortex Data Security. 
 ...
 This approach establishes your corporate Identity Provider (IdP) as the absolute source of truth, allowing group assignments defined in your enterprise directory to be seamlessly reused inside Cortex Data Security. 

 Methodology : Administrators create user group shells inside Cortex Data Security and associate them with the user groups defined in the IdP. This allows you to reuse your existing organizational hierarchy, access permissions, and team structures directly into the security operations console without introducing operational fragmentation or duplicative group-association overhead. 
 ...
 Critical capitalization requirement : String evaluation across authentication mappings, attribute configurations, and group designations enforces absolute case mapping rules. Strict attention to exact character capitalization must be maintained across all configurations. If the group name string in the IdP does not match the string in Cortex Data Security with identical uppercase and lowercase letters, the mapping will fail completely, and users will not inherit their permissions. 

 Active session mechanics : This flow operates dynamically during user login and does not alter or update the permanent group mappings listed within the Cortex Data Security console. The session flow works as follows: 
 ...
 Consequently, these groups do not appear in the persistent list of Cortex groups associated with this user inside the Cortex Data Security console. 
 ...
 Methodology : The Cloud Identity Engine (CIE) uses the System for Cross-domain Identity Management (SCIM) protocol to automatically synchronize groups from your Identity Provider (IdP) directly into CIE. Then, Cortex Data Security synchronizes the CIE groups that were selected using Import AD Group into its local list of groups. 
 ...
 The Cloud Identity Engine (CIE) is used exclusively for directory group management; it is not utilized for individual user account management, provisioning, or authentication workflows. Consequently, disabling, suspending, or removing user objects directly within CIE does not automatically disable, restrict, or delete those corresponding users inside Cortex Data Security. If you use Single Sign-On (SSO) for Cortex Data Security authentication, see the User De-provisioning and Restrictions section in Authenticate users using SSO for complete instructions on handling directory lifecycle cleanups and managing stale accounts. 
 ...
 Cortex creates a new User Group of type AD Group and immediately fetches the current members in the background. An update appears in Notifications when the import is complete. Following the import, Cortex Data Security automatically runs periodic background syncs with the CIE to ensure the group's membership stays up to date. 

 Note 

 If an imported group is later deleted from your Active Directory, Cortex Data Security automatically deletes the corresponding user group at the next sync cycle. 
