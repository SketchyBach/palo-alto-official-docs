---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-vmware-nsx/vmware-nsx-plugin-201/known-issues-in-panorama-plugin-for-vmware-nsx-201.html
fetched_at: 2026-09-16T13:00:36Z
source: palo-alto-main
---

# Known Issues in Panorama Plugin for VMware NSX 2.0.1 Clear
...
in the Panorama plugin for VMware NSX 2.0.1. 

 PAN-91111 

 The template status under Panorama Managed Devices Summary Template is
blank for newly deployed firewalls in NSX after upgrading to Panorama 8.1.0.
Although the template status is blank, the template configuration
is successfully pushed to the firewall. 

 Workaround: Adding new configuration to the template followed
by a commit-and-push updates the template stats to In Sync. 

 PAN-88091 

 When a zone is configured in a template
stack for use with VMware NSX, that zone is not pushed. 

 Workaround: Create zones as a part of template instead
of a template stack. 

 PLUG-361 

 You cannot downgrade directly from 8.1.x
to 8.0.x when VMware NSX plugin 2.0.x is installed. 

 Workaround: Downgrade from 8.1.0 to 8.0.5, install a plugin
version 1.0.x, and then downgrade to 8.0.0. 
