---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/c-Cloud-discovery-service-sectionOverview/machine-discovery
fetched_at: 2026-09-16T07:24:27Z
source: palo-alto-main
---

# Discover Certificates On Machines Clear
...
Updated on 
 ...
 Note: Password-protected PFX and PKCS#12 keystores require keystore credentials to be discovered. Citrix ADC machines support providing these credentials during discovery. For details, see Citrix ADC below. For all other machine types, password-protected PFX and PKCS#12 files are skipped automatically, and the discovery job continues without failure. 
 ...
 Verify that required connection fields (such as address and credentials) are populated. 

 Note: For Cloudflare machines, discovery does not include certificates associated with wildcard hostnames. 
 ...
 Resource Group – Limits discovery to a single resource group. Leave blank to search all resource groups in the subscription. 

 Exclude expired certificates 

 Discovery schedule 

 Note: Certificates that the Application Gateway loads from Microsoft Azure Key Vault are not discovered by this machine type. Discover those certificates with a Microsoft Azure Key Vault machine instead. 
 ...
 Resource types to discover – Virtual servers, services, service groups, or any combination. 

 Partitions – Comma-separated list. Leave blank to include all partitions. 

 Exclude expired certificates 

 Keystore and private key credentials – (Optional) Enter a newline-separated list of passwords to decrypt password-protected PFX certificates on the Citrix ADC machine. When discovery encounters an encrypted PFX file, it attempts to decrypt it using the passwords you provide. 

 Note: Citrix ADC supports discovery of password-protected PFX files. For all other machine types, PFX files are skipped during discovery. 
 ...
 Tag Key – The tag key a secret must carry to be discovered. The default is venafi:managed , the tag Next-Gen Trust Security applies to every secret it creates. 

 Tag Value – The value the tag key must have. The default is true . 

 Exclude Expired Certificates 

 Include Secrets Scheduled for Deletion – Also examine secrets that are scheduled for deletion in Amazon Secrets Manager. 

 Discovery schedule 

 Note: If the machine defines a Secret Name Prefix , discovery returns only secrets whose names begin with that prefix. Discovery skips, and does not fail on, secrets it cannot read or interpret, such as secrets the IAM user cannot decrypt, secrets whose value is not a certificate, and password-protected PKCS#12 secrets. These secrets are recorded in the discovery log. 

 For details on the rest of the machine's settings, see Create an Amazon Secrets Manager machine . 

 Click Discover Now . 

 A message below the machine name shows when discovery starts. Refresh the page to see completion status. 

 Note: Discovery continues even if individual certificates encounter errors. If a discovery run encounters a large number of errors, it may stop early. 
 ...
 Select the Installations tab to view discovered certificate installations. 

 Important: Installations that remain missing for multiple discovery runs are automatically removed. 

 Important: For Cloudflare machines, discovery includes only custom certificates . Universal and backup certificates are not discovered because they are managed directly by Cloudflare. 
