---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-cisco-trustsec/panorama-plugin-for-cisco-trustsec-102/known-issues-in-the-panorama-plugin-for-cisco-trustsec-102.html
fetched_at: 2026-09-16T13:00:42Z
source: palo-alto-main
---

# Known Issues in the Panorama Plugin for Cisco TrustSec 1.0.2 Clear
...
in the Panorama plugin for Cisco TrustSec version 1.0.2. 

 PLUG-5911 

 When the Panorama Plugin for Cisco TrustSec
version 1.0.2 is running on a Panorama version earlier than 10.0.0,
if you are pushing identical IP tags to multiple device groups,
the Panorama Plugin for Cisco Trust Sec creates a message for each device
group and sends it to configd. Because there are many mappings,
the message is large, and creating and sending it takes time. 

 Workaround : Put all device groups under
a parent device group, and put the parent device group in the plugin
notify group at Panorama Cisco
TrustSec Setup Notify
Groups . 
 ...
 When the Panorama Plugin for Cisco TrustSec
version 1.0.2 is running on a Panorama version earlier than 10.0.0,
if you have multiple device groups assigned to a single notify group
and the monitoring interval is set to 10 seconds, the plugin can
take longer than 10 seconds to process the IP-to-tag mapping information retrieved
from the pxGrid server. 
