---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-21/vm-series-plugin-2117/addressed-issues-in-vm-series-plugin-2117.html
fetched_at: 2026-09-16T12:58:58Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 2.1.17 Clear
...
Updated on 
 ...
 Learn which issues were fixed in VM-Series plugin 2.1.17 

 The following list describes the issue fixed in the VM-Series plugin 2.1.17. 

 PLUG-16107 

 Fixed an issue that caused VM-Series firewalls on Azure to experience high root partition
 utilization due to third-party extensions creating many files on the firewall. 

 PLUG-15682 

 Fixed an issue that caused resource group authentication to fail when the VM-Series
 firewall on Azure after upgrading to PAN-OS 10.1.12. 

 PLUG-15510 

 Fixed an issue that prevented the command request plugins vm_series
 dpdk-mbuf-cfg from successfully applying configuration settings to the
 VM-Series firewall. 

 PLUG-15451 

 Fixed an issue that prevented the interface from moving from the active firewall to the
 passive firewall upon an HA failover event on the VM-Series firewall on AWS. 
