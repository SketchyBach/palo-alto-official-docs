---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/c-Cloud-discovery-service-sectionOverview/c-custom-reports
fetched_at: 2026-09-16T07:24:25Z
source: palo-alto-main
---

# Track Inventory with Custom Reports Clear
...
Updated on 
 ...
 A report belongs to the workspace that was selected when it was created, or to the tenant if it was created under Tenant . 

 Under Tenant , the inventory lists every report in the tenant and a Workspace column names the owner of each one. Reports that belong to the tenant rather than to a workspace show the tenant's own name. You can sort on the column, and you can filter it with the in operator to list the reports from one workspace or several. Selecting the tenant's name in that filter returns the reports the tenant owns. 

 Inside a workspace, the inventory lists that workspace's reports and the Workspace column is not shown, because every row would name the workspace you are already in. 

 Note: The Workspace column filters the list of reports, not the data inside them. To limit what a report covers to particular workspaces, add a workspace filter when you build it. See Create a custom report . 
 ...
 Only users with the Superuser role can access and use Custom Reports. What a report covers follows the workspace you had selected when you created it: a report created under Tenant reports on the whole tenant, and a report created inside a workspace reports on that workspace. 
