---
url: https://docs.koi.ai/guides/protect-ai-tools-with-koi/agent-activity
fetched_at: 2026-09-06T10:19:32.492Z
...
Koi uses the hooks system built into Cursor and Claude Code. Hooks are intervention points in the agent execution loop that fire before or after specific actions.
...
Hooks capture events locally as structured JSON

Koi collects and normalizes events centrally

Events appear in the Agent Activity inventory
...
Check Koi dashboard → Agent Activity for the event

Claude Code:

Open managed settings file (see paths above per OS in 'Supported Paths')

Confirm Koi hooks appear under "hooks"
