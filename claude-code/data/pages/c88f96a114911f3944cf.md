---
url: https://docs.paloaltonetworks.com/saas-security/sspm/onboard-saas-apps-supported-by-sspm/onboard-a-claude-app-to-sspm
fetched_at: 2026-08-13T17:34:05Z
source: palo-alto-main
---

# Onboard a Claude App to SSPM Clear
...
Onboard a Claude App to SSPM 
 ...
 Learn how you can manually provide SSPM with the configuration values for a Claude
 instance. 
 ...
 The onboarding process for Claude is different from most applications,
 because SSPM does not support scans for Claude. During the onboarding process for
 ...
 Although SSPM does not support scans for Claude, you can still onboard a Claude
 application to SSPM. However, when you onboard a Claude application to SSPM, you
 don’t supply SSPM with any authentication credentials. SSPM does not establish a
 connection to your Claude instance and does not run scans to determine the Claude
 settings. Instead, you will manually enter your Claude instance's settings, which
 ...
 Claude instance, but this tile is isolated to SSPM. SSPM will show rule violations
 for the Claude instance, but SSPM determines the violations based entirely on the
 ...
 To onboard a Claude instance, you complete the following steps: 

 Log in to the Claude as an administrator. 
 ...
 Select Configuration SaaS Security Posture Security Applications Add Application and click the Claude tile. 

 On the Posture Security tab, Add New instance. 

 SSPM displays the onboarding page, which lists the Claude configuration
 settings and our recommended configuration values. 

 From your Claude instance, identify the configuration values, and provide these
 configuration values to SSPM. 

 To avoid triggering rule violations in SSPM, you can update the values in
 Claude to match our recommended values. 
 ...
 SSPM adds a tile on the Applications page to represent your Claude instance.
 Based on the values that you entered, SSPM will determine if your Claude
 instance has any rule violations. 

 Because SSPM is not scanning your Claude instance, you must ensure that the
 information that SSPM has about your Claude instance remains accurate. If
 you change a setting value in Claude, manually update that value in SSPM. To
 do this, complete the following steps: 

 Navigate to the Applications page ( SSPM Applications ). 

 Locate the Claude instance's tile and View
 Details . 
