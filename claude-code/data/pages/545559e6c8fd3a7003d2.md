---
url: https://cortex-docs.paloaltonetworks.com/cortex-agentix/configure-cortex-agentix/users-and-roles-management/set-up-authentication/authenticate-users-using-sso
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Configure SAML 2.0 SSO for Cortex AgentiX with identity provider mapping, user groups, roles, and tenant access. 
 ...
 Unlike users who authenticate through the Customer Support Portal (CSP), users who log in via SSO do not require the Cortex User role to be assigned in the CSP. Their access and permissions are governed by the SAML Group Mapping configured in Cortex AgentiX. 
 ...
 In Cortex AgentiX, navigate to Settings → Configurations → Access Management → Authentication Settings . 
 ...
 There is no public API endpoint available to provision or de-provision users programmatically within Cortex AgentiX. All target accounts must be initialized or explicitly managed using the native interactive Single Sign-On (SSO) or Customer Support Portal (CSP) interface workflows defined in this guide. To review the list of supported programmatic actions and ingestion endpoints, see the Cortex AgentiX API Reference guide. 
 ...
 (Optional) Select the default role that you want any user to automatically receive when they are granted access to Cortex AgentiX through SSO. This is an inherited role and is not the same as a direct role assigned to the user. 
 ...
 (Optional) Specify the URL for a specific page that you want users to be directed to after they’ve been authenticated by your organization’s IdP and log in to Cortex AgentiX. 

 IdP Single logout URL 

 (Optional) Specify your IdP single logout URL provided by your organization’s IdP to ensure that when a user initiates a logout from Cortex AgentiX, the identity provider logs the user out of all applications in the current identity provider login session. 
 ...
 (Optional) Requires users to log in to Cortex AgentiX using additional authentication methods, such as biometric authentication. 
 ...
 (Optional) Requires users to reauthenticate to access the Cortex AgentiX tenant if requested by the idP, even if they already authenticated to access other applications. 
