---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-software-firewall-license-plugin/panorama-software-firewall-license-plugin-123/addressed-issues-in-the-panorama-software-firewall-license-plugin-123.html
fetched_at: 2026-09-16T13:01:29Z
source: palo-alto-main
---

# Addressed Issues in the Panorama Software Firewall License Plugin 1.2.3 Clear
...
Updated on 
 ...
 The following list describes addressed issues in the Panorama Software Firewall License
 plugin 1.2.3. 

 PLUG-18180 

 Resolved an issue that displayed the panorama-server incorrectly in the bootstrap
 parameters on cloud platforms in an HA environment. 

 PLUG-20322 

 Fixed an issue where the software firewall license plugin incorrectly displayed the
 panorama-server IP as NO_MATCHES in Panorama License Manager Show Bootstrap
 Parameters . 

 PLUG-21323 

 Resolved an issue where the edit-license-profile and
 set-license-profile CLI operations incorrectly required a
 description. These commands now function correctly without the additional parameter. 

 PLUG-21325 

 Fixed an issue where bootstrap definitions appeared incorrectly in the offline license
 manager's grid view. 

 PLUG-22299 

 Resolved an issue that prevented enabling offline licensing through the CLI. 

 PLUG-22386 

 Resolved an issue where an incorrect error message was displayed when an invalid license
 manager name was provided in the request plugins sw_fw_license
 bootstrap-parameters license-manager <license-mgr-name> CLI
 command. 

 PLUG-22821 

 Fixed an issue where firewalls were not deleted from managed devices following an
 autoscaling scale-in event. New CLI commands are available to set and view the stale
 entry grace period. 
