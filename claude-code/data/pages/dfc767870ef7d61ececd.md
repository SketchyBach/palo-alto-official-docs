---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-ips-signature-converter/panorama-ips-signature-converter-plugin-203/known-issues-in-the-ips-signature-converter-plugin-203.html
fetched_at: 2026-09-16T13:01:35Z
source: palo-alto-main
---

# Known Issues in the IPS Signature Converter Plugin 2.0.3 Clear
...
Updated on 
 ...
 Known issues in the Panorama intrusion prevention system (IPS) Signature Converter
 plugin 2.0.3. 

 The following list describes known issues in the Panorama intrusion
 prevention system (IPS) Signature Converter plugin 2.0.3. 
...
with the following warning: [FP risk] The order of the conditions are swapped since the last condition is negate 
...
together using semicolons ( ; ) will
not convert. 
 ...
 If a rule uses any of these content modifiers— depth , within , offset ,
or distance —they must use integer values
or the rule will not convert. 
...
to Commit and Push these patterns to your
firewalls. 
...
construct, will convert but will cause a commit failure: 
 ...
 Rules with the threshold keyword set to type threshold or type both will convert
as brute force signatures . However,
if you convert a rule with the threshold keyword
set to type limit , the rule will convert
...
the Suricata documentation . 
 ...
 A brute force rule using the threshold keyword
with seconds greater than 3600 will
not convert. 
...
written for the file-data context: 
 *\/2\.0\/method
*\/2\.0\/method\/(checkConnection|config|delay|error|get|info|setOnline|update) 

 CON-48472 

 There is a maximum of 63 characters allowed for a URL
in the reference option of a rule.
The converter ignores URLs that exceed the 63-character limit. 
 ...
 A rule to prevent brute force attacks by using the threshold element
must have a count between 1 and 255
to convert successfully. 

 PLUG-5153 

 The depth and offset rule
modifiers are ignored. 

 PLUG-5405 

 A rule fails conversion when it includes a pcre pattern
that would convert to use the tcp-context-free , udp-context-free ,
or file-data custom signature context . 

 PLUG-5343 

 A rule with multiple content patterns that use the
 distance or
 within modifiers
 ...
 <context>tcp-context-free</context>
 <negate>no</negate>
 </pattern-match>
 </operator>
 </entry>
 ...
 <reference>
 <member>attack.mitre.org/techniques/T1205</member>
 <member>blogs.cisco.com/security/synful-knock</member>
 <member>tools.cisco.com/security/center/viewAlert.x?alertId=40411</member>
 </reference>
