---
url: https://docs.prismacloud.io/content-collections/runtime-security/vulnerability-management/vulnerability-explorer
fetched_at: 2026-09-16T13:35:20Z
source: prisma-cloud
---

# Vulnerability Explorer | Prisma Cloud arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 The CVE statistics listed in the Vulnerability Explorer are per CVE-ID and not a total count of all CVEs under all resources. 
 ...
 Running containers can introduce additional environmental factors that increase the calculated score for a vulnerability. For example, when the container runs as root, it could exacerbate the problem. A list of container traits that heighten the risk is listed in the detailed information dialog when you click on a row in the top ten tables. 
 ...
 In Prisma Cloud Runtime Security > Manage > System > Scan > Scan settings > Running images , when the option Only scan images with running containers is turned off, the image details show the Start time when the Defender first reads the image. This is applicable for all images (deployed and not deployed). 
 ...
 DoS {High/Low}  — Component is vulnerable to denial of service attacks, such as buffer overflow attacks, and ICMP floods. The risk is categorized as high or low based on impact. 
 ...
 Exploit in the wild  — Exploit attempts of this vulnerability that have been seen in the wild. All vulnerabilities are from the CISA KEV Catalog . 
 ...
 Sensitive information  — Vulnerability exists in a container or a serverless function that stores private keys or has environment variables that provide sensitive information. 

 Root Mount  — Vulnerability exists in a container with access to the host filesystem. 

 Runtime socket  — Vulnerability exists in a container with access to the host container runtime socket. 

 Host Access  — Vulnerability exists in a container with access to the host namespace, network, or devices. 
 ...
 X: Supported V: Supported with specific considerations or variations 
 ...
 Enhanced RHEL Vulnerability Reporting in Prisma Cloud 

 To improve accuracy in vulnerability detection, Prisma Cloud now includes RPM module and stream information for Red Hat Enterprise Linux (RHEL) images in the vulnerability reports. This enhancement ensures that all RPM packages installed by a module are properly examined during scans, reducing false positives and improving detection reliability. 

 Understanding RPM Modules and Streams 

 In Red Hat Enterprise Linux (RHEL), an RPM module is a collection of related RPM packages that represent a software component, including an application, its dependencies, and supporting utilities. 

 Module Structure 

 Module Streams: Virtual repositories within the AppStream repository, each representing a specific version of the module with independent updates. 

 Stream Activation: Only one stream of a module can be active at a time, meaning only one version of a component can be installed on a system. 
 ...
 Source Package: python39 

 Benefits of Module-Aware Vulnerability Reporting 

 Improved Accuracy: CVE fixes are matched to the correct module stream, reducing incorrect vulnerability detection. 

 Reduced False Positives: Vulnerabilities fixed in older streams are no longer misreported. 

 Comprehensive Coverage: All RPM packages installed by a module are linked to their respective vulnerabilities. 

 Previous Exclude Base Image Vulnerabilities 

 Next Exporting Software Bill of Materials on CycloneDX 
