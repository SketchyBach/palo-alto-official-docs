---
url: https://docs.paloaltonetworks.com/ngfw/help/11-1/network/network-network-profiles/network-network-profiles-sd-wan-interface-profile
...
internet connections or internet breakout capability, such as cable
modem, ADSL, and other internet connections. 

 You can disable VPN Data Tunnel Support 
 for private link types such as MPLS, satellite, or microwave
 that do not have internet breakout capability except Private
 Link1 , Private Link2 ,
 Private Link3 , and
 Private Link4 link types .
 However, you must first ensure the traffic cannot be intercepted
 because it will be sent outside of the VPN tunnel. 
...
from the hub. The VPN Data Tunnel Support setting determines
...
the private data flow doesn’t use). The firewall uses zones to segment DIA
failover traffic from private MPLS traffic. 
