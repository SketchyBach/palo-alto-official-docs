---
url: https://cortex-docs.paloaltonetworks.com/appsec-rules/secrets-security/appsec-secret-144
fetched_at: 2026-09-06T11:15:36Z
source: cortex-platform
---

# MariaDB Credentials detected in code | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center

For the complete documentation index, see llms.txt . This page is also available as Markdown . 

 Ask 
 On this page 

 Reference 

 Alerts & Rules 

 AppSec Rules 

 Secrets Security 

 MariaDB Credentials detected in code 

 Rule Details 

 Cortex AppSec Rule ID 

 APPSEC_SECRET_144 

 Category 

 Database Credentials 

 Severity 

 HIGH 

 Framework 

 Git 

 Impact 

 MariaDB Credentials (username/password) grant an attacker unauthorized access to the database. 

 The impact is severe, compromising the confidentiality, integrity, and availability of all data and system functions. 

 How to Fix 

 To remediate exposed Database credentials: 

 Log in to your database administration tool or console. 

 Identify the database user associated with the exposed connection string. 

 Rotate the password for that specific database user immediately. 

 Update the connection strings in your application configuration or secrets manager. 

 Restart your application instances if necessary to apply the new connection string. 

 Next, remove the secret from your codebase: 

 Locate the exposed connection string in your codebase. 

 Replace the hardcoded value with a secure variable or reference. 

 Finally, clean your version control history: 

 Permanently remove the sensitive data from your version control history to ensure it cannot be retrieved. 

 Previous Contentful Content Delivery API Key detected in code 

 Next AWS SES client secret detected in code 

 Last updated 1 month ago 

 Was this helpful?
