---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/t-cloud-predict-dashboard/c-validating-certificates/t-validating-certs-manually-using-ValidateNow
fetched_at: 2026-09-16T07:25:29Z
source: palo-alto-main
---

# Running a Validation Manually Clear
...
Updated on 
 ...
 You can manually validate installed certificates using the Validate Now feature. 

 Tip: In addition to standard validation checks, Validate Now can also discover additional TLS server endpoints associated with certificates already in your inventory. 

 To Run a Validation Manually 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Inventory > TLS Server Endpoints . 

 On the TLS Server Endpoints page, do one of the following: 

 To search for a specific certificate, enter certificate details in the search bar. 

 To use a saved filter, open the Filters list and select the filter. 

 To manually filter the list, click Filter next to the search bar, specify the attributes to include, and click Apply . 

 In the Certificate Name column, click the certificate you want to validate. 

 In the certificate details view, click the TLS SERVER ENDPOINTS tab. 

 Click Validate Now . 

 A Validation in progress message appears at the top of the page. When validation completes: 

 TLS Validation and Chain Validation display Success , or 

 A Validation Failed message appears. 

 Tip: You can confirm when validation last ran by checking the Last Seen column. After running Validate Now , this value updates to a minute ago . If you do not see a status change, refresh the page. 
