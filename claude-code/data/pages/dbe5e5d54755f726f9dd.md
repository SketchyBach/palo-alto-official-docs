...
Updated on 
 ...
 Palo Alto Networks’ Enterprise Data Loss Prevention (E-DLP) ) is a cloud-native security service
 that safeguards sensitive data from loss, theft, and misuse. Traditional on-premise
 solutions cannot effectively protect data distributed across cloud apps, remote
 workforces, and diverse devices. Enterprise DLP addresses these challenges with a
 unified, scalable, and intelligent approach. 

 This article explains Enterprise DLP ’s core concepts, key features, integration with
 the Palo Alto Networks security ecosystem, and configuration in the Prisma Browser . 

 Core Concepts 

 These are the core concepts behind Enterprise DLP : 
 Unified, Cloud-delivered Approach - Enterprise DLP is a single,
 centralized service that integrates natively with Palo Alto Networks
 products such as NGFWs, Prisma Access, Prisma Browser). 

 Data Classification as the Foundation - Enterprise DLP 
 identifies sensitive data using advanced classification methods that extend
 beyond simple keyword matching. 

 Multi-channel Protection - Enterprise DLP protects data in
 motion (network traffic, email, file uploads), at rest (SaaS
 and cloud storage), and in use (endpoints and browsers). 

 Centralized Policy Engine - Enterprise DLP provides one console
 for managing data control sets, classifiers, and incidents, ensuring
 consistent policy enforcement across network, cloud, and endpoint
 controls. 
 ...
 Prisma Access & Prisma Browser Integration — Enterprise DLP 
 applies the same classifiers to remote workers and browser-based activity,
 enforcing last-mile controls. 
 ...
 Configure Enterprise DLP in Prisma Browser 

 This section outlines how to configure Enterprise DLP in Prisma Browser by
 defining data profiles, applying them in rules, and monitoring incidents. 

 Before You Begin 

 Before you begin - make sure you understand the following concepts: 
 Enterprise DLP integrates with Access & Data Control rules 
 in Prisma Browser . 

 You can create a new rule or modify an existing one. 

 Data Profiles must be defined in the central Enterprise DLP 
 console before they can be applied in Prisma Browser . 
 ...
 Enterprise DLP can operate using OCR
 information, however it is only supported for Cloud-assisted profiles with File
 Download and File Upload controls. 

 Step 1: Define Data 

 Check for existing profiles 

 Review the predefined profiles in the Enterprise DLP console before
 creating a new one. 
 ...
 the Enterprise DLP Profile Creation Help . 
 ...
 Processing : Content is sent to Enterprise DLP cloud
 service. 
 ...
 events in the central Enterprise DLP console. This gives you a
 single dashboard to view all incidents across your network, cloud, and
 browser. 
 ...
 When either the Prisma Browser local scanning or the Enterprise DLP
 detects sensitive data that violates a policy, two critical types of forensic data
 are generated: 
 ...
 Snippets are globally enabled in the Enterprise DLP settings.
 ...
 In the Tracking tab, select Content scanning ( Enterprise DLP ) from either On or Enhanced. 
 ...
 Evidence storage is globally enabled in the Enterprise DLP 
 settings. 
 ...
 ( Enterprise DLP ) from either On or
 Enhanced . 
 ...
 The integration between Enterprise DLP and the Prisma Browser streamlines investigation for security teams: 

 Prisma Browser events ( or local administrators)
 include a direct link to the corresponding Enterprise DLP 
 incident . 
 ...
 can view the data snippet, examine the surrounding Prisma Browser 
 event details, and access the full evidence file. 

 Cloud Scanning Thresholds 

 When using Enterprise DLP with Prisma Browser , some data profiles
 ...
 machine-learning–based analysis—that require files to be uploaded to the Enterprise DLP cloud service for inspection and analysis. 
 ...
 times out). 
 Fail Unmatch: If the Enterprise DLP settings action
 is set to Allow , a failed scan (due to threshold) results
 in a file action policy in Prisma Browser to be skipped
 (i.e, treated as "no match"). 

 Fail Block: If the Enterprise DLP action is set to
 Block , any scan that fails to complete within the
 threshold is treated as a "fail block," and the file is
 prevented from being uploaded/downloaded/printed, even if the
 Prisma Browser policy is set to allow the file action
 upon a match. 
