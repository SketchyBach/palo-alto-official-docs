...
Updated on 
 ...
 ( VM-Series firewalls on ESXi with Intel E810/X710 NICs using
 PCI passthrough ) 
 To accelerate failover recovery, the
 physical layer (PHY) on the passive firewall instance remains
 powered on. Intel verified that re-negotiation latency on
 passthrough NICs cannot be bypassed; consequently, maintaining an
 active link state is required to achieve sub-second port
 initialization. Although the CLI displays a powered-down status when
 the firewall halts packet forwarding across these interfaces, the
 NIC retains an active physical link. As a result, connected upstream
 switches detect the interface link state as
 UP . 
