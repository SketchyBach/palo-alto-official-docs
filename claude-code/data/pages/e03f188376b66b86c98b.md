---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-aws/aws-plugin-512/addressed-issues-in-panorama-plugin-for-aws-512.html
fetched_at: 2026-09-16T12:59:46Z
source: palo-alto-main
---

# Addressed issues in Panorama Plugin for AWS 5.1.2 Clear
...
Updated on 
 ...
 The following are the fixed issues in the Panorama plugin for AWS 5.1.2: 

 PLUG-17007 

 Fixed an issue where shared DAGs fail to show associated IP addresses when the match
 criteria contains the characters like newline, space, or brackets. 

 PLUG-17012 

 Fixed an issue where shared DAGs fail to show associated IP addresses when the match
 criteria contains the characters like carriage return. 

 PLUG-17228 

 In the Panorama console, the AWS Plugin dashboard becomes unresponsive or takes a long
 time to display the IP tags when you check for the IP tags that have a long IP tags
 list. The issue is now fixed. 

 PLUG-17255 

 Fixed an issue that caused failure to display IP tags even though those IP tags are
 already referenced in the security policy and AWS plugin dashboard showed these tags as
 not used in the security policy. 

 PLUG-17304 

 Fixed an issue that showed a malformed scripts message for AWS plugin during an HA
 synchronization in Panorama. 
