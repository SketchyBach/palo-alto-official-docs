---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-kubernetes/panorama-plugin-for-kubernetes-302.html
fetched_at: 2026-09-16T13:01:10Z
source: palo-alto-main
---

# Panorama Plugin for Kubernetes 3.0.2 Clear
...
Updated on 
 ...
 The Panorama plugin for Kubernetes 3.0.2 does not introduce any new feature. 

 You can disable the tag pruning running the following command: 

 request plugins kubernetes set-tag-pruning-flag value False 

 On a High Availability Panorama setup, the tag pruning CLI commands must be
 configured on both the HA peers. 

 For more information, see Tag Pruning . 

 In the Panorama plugin for Kubernetes 3.0.2, the Kubernetes watcher API shuts down connections if
 there has not been any update to the cluster. Thus, the watcher in Monitoring
 Definitions will remain in a Disconnected state with the following error
 message: 

 *Stopped watching Pods, unable to get resourceVersion.* Waiting for successful polling before watching again. 

 After you deploy a new Pod, the Monitoring Definition watcher changes to
 the Connected state. 
