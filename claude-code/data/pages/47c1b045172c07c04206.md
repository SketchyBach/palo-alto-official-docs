...
# Logs Collected by Prisma Agent Clear
...
Updated on 
 ...
 Learn about the log filenames and locations that Prisma Agent 
 collects. 
 ...
 The debug logging mechanism for the Prisma Agent collects debug logs from
 critical Prisma Agent processes and maintains them on disk. You or someone
 ...
 Prisma Agent logs are automatically generated and provide an audit
 ...
 the endpoint will gather the Prisma Agent logs into a log bundle
 ( .zip file). 

 On macOS, Windows, and Linux endpoints, Prisma Agent are located the logs in the following locations: 
 On macOS: 
 /Library/Application
 ...
 In addition, Prisma Agent log entries are available in other
 locations: 

 On macOS, you can look in the system logs for Prisma Agent 
 ...
 page ( Configuration Endpoint Management ), you can conveniently generate the Prisma Agent logs and
 ...
 The logs that Prisma Agent collects reside in the several folders in the
 log bundle. Depending on the operating system on the endpoint, the logs for Prisma Agents are structured slightly differently. 
 ...
 The following are examples of contents in the Prisma Agent log bundle on
 desktop devices. 

 DLP —Folder that contains Endpoint DLP logs (if enabled on
 the endpoint) (macOS and Windows agents only) 

 Logs —Folder that contains the Prisma Agent 
 system logs and user
 logs. 
 <user> —Folder that contains user-related Prisma Agent logs: 
 ...
 the activity for the Prisma Agent app 

 DEM —Autonomous DEM logs (if installed on the endpoint)
 (macOS and Windows agents only) 

 System —Folder that contains system-related Prisma Agent logs: 
 ...
 Service (PASrv), which is the backend service for the Prisma Agent . When the log exceeds the maximum
 ...
 When critical executables crash on your operating system, Prisma Agent 
 ...
 The Prisma Agent logs are available in six levels of verbosity.
...
pacli loglevel set <trace | debug | info | warn | error | critical | off> 
 ...
 debug Debug logs (default verbosity level after installation) For Prisma Agent Development 
 ...
 trace Trace logs For Prisma Agent Development 
