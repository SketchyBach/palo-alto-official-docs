---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-aws/panorma-plugin-for-aws-301/whats-new-in-panorma-plugin-for-aws-301.html
fetched_at: 2026-09-16T12:59:54Z
source: palo-alto-main
---

# What’s new in Panorama Plugin for AWS 3.0.1 Clear
...
Updated on 
 ...
 The Panorama plugin for AWS 3.0.1 introduces three CLI
commands to set, show, and clear AWS regions during IAM role validation. 

 From the CLI on the active and passive Panorama, enter the following
commands: 

 request plugins aws set-aws-region —Use
this command to set AWS region for the plugin to validate the IAM
role. 

 request plugins aws show-aws-region —Use
this command to show the AWS region used for IAM role validation. 

 request plugins aws clear-aws-region —Use
this command to clear the AWS region used for IAM role validation.
When you use this command, the plugin clears the existing AWS region and
sets it to the default us-west-2 region. 

 On a High Availability Panorama setup, the set region CLI commands must
 be configured on both the HA peers. 
