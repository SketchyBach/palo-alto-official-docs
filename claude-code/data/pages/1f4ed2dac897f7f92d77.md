---
url: https://docs.paloaltonetworks.com/pan-os/10-1/pan-os-release-notes/pan-os-10-1-5-known-and-addressed-issues/pan-os-10-1-5-addressed-issues
fetched_at: 2026-08-13T17:03:41Z
source: palo-alto-main
---

# PAN-OS 10.1.5 Addressed Issues Clear
...
on the queue number. 
 ...
 Fixed an issue where, when pre-generated
...
Fixed an issue where HSCI interfaces didn’t come up when using BiDi
transceivers. 

 PAN-186785 

 Fixed an issue where, after logging in,
...
115 managed devices or 120 dynamic groups. 
 ...
 Fixed an issue where index creation failed
...
broke on tunnel rekey with multiple data processing cards (DPC). 
 ...
 Fixed an issue where log queries that included
...
issues that caused the dataplane to restart unexpectedly 
 ...
 Fixed an issue where the dataplane exited
...
Router > BGP > Peer Group > Peer Address ). 
 ...
 Fixed an issue where processing corrupted
IoT messages caused the wificlient process
to restart. 

 PAN-185163 

 Fixed an issue where the distributord process
hit the FD limit, which caused User-ID redistribution to not function properly. 

 PAN-184761 

 Fixed an issue where Security policies were
...
where logs were delayed when querying for logs. 
 ...
 Fixed an issue where Terminal Service agent
...
or intermediate certificate. 
 ...
 Fixed an memory leak issue in the mgmtsrvr process,
which resulted in an out-of-memory (OOM) condition and high availability
(HA) failover. 

 PAN-183428 

 Fixed an issue where, when exporting or
...
error occurred with GlobalProtect gateway inactivity logout time. 
 ...
 Fixed an issue where the firewall randomly
disconnected from the WildFire URL cloud. 

 PAN-183112 

 Fixed an issue where the threat log type ml-virus wasn't
forwarded to Panorama or to external servers. 

 PAN-182954 

 ( PA-7000 Series firewalls with Log Processing
Cards (LPC) only ) Fixed an issue where excessive threat ID
lookups caused logs to be lost. 

 PAN-182903 

 Fixed an issue where SD-WAN failover on
...
device. This issue occurred when redundant PSUs were connected. 
 ...
 Fixed an issue where Panorama Global Search
...
lifetime expiration. 
 ...
 Fixed an issue where the stats dump file
was not generated properly. 

 PAN-181360 

 Fixed an issue where staggering scheduled
...
type. 
 ...
 Fixed memory corruption issues in PAN-OS
...
that caused continuous DNS retries. 
 ...
 Fixed an issue where DNS security caused
...
when the peer firewall IP address was in a different subnet. 
 ...
 Fixed an issue where admins and other Superusers
...
issue occurred to IP address users being added. 
 ...
 Fixed an issue where quarantined devices
...
responding due to buffer overflow. 
 ...
 Fixed an issue where restarting the management
...
enabled when attempting to view local logs. 
 ...
 Fixed an issue where the CLI output of show location ip <ip address> returned
unknown. 
...
XML API section was unable to set the License Deactivation API key. 
 ...
 Fixed an issue where a process ( devsrvr ) stopped
...
the configuration was committed ( CVE-2022-0024 ). 
 ...
 Fixed an issue where, when system logs and
...
did not show new logs from firewalls. 
 ...
 Fixed an issue where, when the address object
...
the object in the Security policy was renamed as well. 
 ...
 Fixed an issue where, when you disabled
...
in blue and was still able to be modified to a different value. 
 ...
 Fixed an issue where log collectors generated Failed to
check IoT content upgrade system logs even when no IoT
license was installed. 

 PAN-176889 

 Fixed an issue where the log collector continuously
...
was lost when performing a failover in an HA active/passive setup. 
 ...
 Fixed an issue where importing a firewall
...
will be created if unique) was unchecked. 
 ...
 Fixed an issue where scheduled email alerts
...
(CNAME) records when caching was enabled. 
 ...
 Fixed an issue where the header did not
...
process stopped responding, which caused authentication to fail. 
 ...
 Fixed an issue where alerts related to syslog
connections were not generated in the system logs. 
 ...
 Panorama was running PAN-OS 10.2 and managed firewalls were
 downgraded from PAN-OS 10.2 to PAN-OS 10.1.4 or earlier
 PAN-OS release 

 Panorama was upgraded from PAN-OS 10.1 to PAN-OS 10.2 and
 managed firewalls were running PAN-OS 10.1.4 or earlier
 PAN-OS 10.1 release. 
...
AUX2 interfaces through SNMP. 
 ...
 Fixed an issue where log forwarding profiles
...
system logs. 
 ...
 Fixed an issue where enabling Use proxy to fetch logs from Strata Logging Service caused Panorama to not show logs when
 queried. 
...
responding due to a memory leak. 
 ...
 Fixed an issue where a Security policy configured
...
to stop responding. 
 ...
 Fixed an intermittent issue where IP address-to-username mappings
...
limit on the configd process to 32GB. 
 ...
 Fixed an issue where, when the TTL value
...
process due to a connection timeout. 
 ...
 Fixed an OOM condition that occurred due
...
across TLS sessions decrypted by the firewall were malformed. 
 ...
 Fixed an issue where the email subject of
scheduled reports was enclosed in single quotation marks. 

 PAN-174564 

 ( VM-Series firewalls on a Kernel-based
...
an issue where a process ( all_task ) stopped responding. 
 ...
 Fixed an issue where the semi-colon (;)
...
logging rate limit was improved to prevent log loss. 
 ...
 Fixed an issue where HIP profile objects
...
HIP objects. 
 ...
 Fixed an issue in active/active HA configurations
...
with WildFire. 
 ...
 Fixed an issue where the IPSec tunnel configuration
...
to stop responding and the dataplane to restart. 
 ...
 Fixed an issue where ACC > Threat activity
did not include the threat name after upgrading to a PAN-OS 10.0
release. 

 PAN-169917 

 Fixed an issue on Panorama where AUX interface
IP addresses did not populate when configuring service routes. 

 PAN-169796 

 Fixed an issue where the high availability
...
applied to the email server service route. 
 ...
 Fixed an issue on Panorama where multiple
...
license token file was incorrect. 
 ...
 Fixed an issue where the stats
dump report was empty. 

 PAN-163831 

 Fixed an issue where IPv6 addresses were
...
to the firewall from Panorama failed with the following error message: client routed requesting last config in the middle of a commit/validate. Aborting current commit/validate . 
 ...
 ( Firewalls in HA active/passive configurations
...
vendors when IKEv2 used SHA2-based certificate authentication. 
 ...
 Fixed an issue where TLS 1.3 Forward Proxy
...
the following error message was displayed: Not enough space to load content to SHM . 
 ...
 Fixed an issue on Panorama where logs that
...
on FIPS-mode firewall decryption that used DHE ciphers. 
 ...
 Fixed an issue where applications did not
work via the Clientless VPN when they were configured on a vlan
interface 

 PAN-156289 

 Fixed an issue where the default severities
for Content Update errors were inaccurate. 

 PAN-151692 

 Fixed a permission issue where a Panorama
