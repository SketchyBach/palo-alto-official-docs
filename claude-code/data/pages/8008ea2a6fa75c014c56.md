---
url: https://cortex-docs.paloaltonetworks.com/cortex-agentix/configure-cortex-agentix/automations/playbooks/build-your-playbook/add-objects-from-the-task-library/add-manual-tasks-and-blank-tasks/create-a-communication-task
fetched_at: 2026-09-16T08:51:05Z
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Create communication tasks in Cortex AgentiX playbooks to gather data and send surveys. 

 Create Cortex AgentiX playbook communication tasks to send surveys to internal and external users. Use collected issue data for issue analysis and subsequent playbook tasks. Send scheduled surveys for issue updates. Send a standalone question survey to determine how an issue was handled. 

 Communication tasks support two survey types: 
 ...
 Stand-alone questions. These are presented to users directly in the message, and users answer directly in the message (not an external survey). 
 ...
 Web Survey Title sets the title shown on the survey. 

 Short Description appears above the survey questions. Select Preview to review it. 

 Question sets the question for recipients. 

 Answer Type supports: 
 ...
 Mandatory requires recipients to answer before submitting. 

 Help Message appears when recipients hover over the help icon. 

 Placeholder displays an example value until users enter a value. 

 Drag questions to change their display order. 
...
NOTE: Only enter an Effective time if you want the same exact time recorded every time the playbook task executes. Using selects an integration instance. Leave it empty to use all instances. 

 Extend context appends action results to context data. 

 Ignore outputs prevents output storage, except extended outputs. 

 Execution timeout (seconds) sets the command timeout. 

 Indicator Extraction mode controls extraction: 

 None disables indicator extraction. 

 Inline extracts indicators before other playbook tasks. 

 Out of band extracts indicators while other tasks run. 

 Mark results as note adds results as a note. 

 Mark results as evidence adds results as evidence. 

 Run without a worker runs the task without a worker. 

 Skip this branch if this script/playbook is unavailable skips unavailable work. 

 Quiet Mode hides inputs, outputs, and extracted indicators. Errors and warnings remain visible. 

 Optional: Select Preview to customize the email's colors, text, and submit button. 
 ...
 Select Save .
The task is added to the playbook editor. A user icon indicates that it requires manual input. 
 ...
 In this example, we create a stand-alone question with a single-select answer. This question is not mandatory. If we select the First option as the default checkbox, the reply option "0" is the default value in the answer field. 

 data-collection-eg.png 

 In this example, we create a question based on a custom issue field that is marked as mandatory. You can add a question based on a field. To add a field, click Add Question based on field. 

 data-collection-eg2.png 
 ...
 The authorized users are usually external, not in Cortex AgentiX, and they will not be able to access anything else in Cortex AgentiX. 
 ...
 If you are using NGINX as a reverse proxy with SSL termination, configure the NGINX configuration file to enable access to data collection links in emails. 
