---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/10-1/pan-os-upgrade/upgrade-the-vm-series-firewall/upgrade-the-vm-series-model-in-an-ha-pair.html
fetched_at: 2026-09-16T10:52:29Z
...
When you apply a new capacity license on the VM-Series firewall, the
model number and the associated capacities are implemented on the
firewall. 

 Verify the VM-Series System Requirements for
your firewall model before you upgrade. If your firewall has less
than 5.5GB memory, the capacity (number of sessions, rules, security
...
when a capacity mismatch is detected and is then re-enabled when
both HA peers have matching capacity licenses. 

 If the firewalls
in the HA pair have different major software versions (such as 9.1 and
9.0) and different capacities, both devices will enter the Suspended
...
the show high-availability interface ha2 command
