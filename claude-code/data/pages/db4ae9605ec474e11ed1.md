---
url: https://cortex-docs.paloaltonetworks.com/demisto-sdk-development-guide/demisto-sdk-guide/demisto-sdk-commands/generate-yml-from-python
fetched_at: 2026-09-06T10:39:12Z
source: cortex-platform
---

# generate-yml-from-python | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Generate integration YAML metadata from Python code using the Demisto SDK metadata collector. 

 Generates a YAML file from Python code that includes its special syntax. 

 The output file name is the same as the Python file name but with the .yml extension instead of .py . 

 The generation currently supports integrations only. The feature is supported from content Base Pack version 1.20.0 and later. You must have the following line in your Python code:
 ...
 The path to the source Python file used for generation. (Required) 

 -f, --force 

 Overwrite the existing YAML file. If this is not used and the YAML file already exists, the script does not generate a new YAML file. 

 Initialization and Integration Configuration 

 The initialization begins with the YMLMetadataCollector object: 

 metadata_collector = YMLMetadataCollector(integration_name="sample_name") 

 The generation only works if the name of the initialized object is metadata_collector . 

 The only mandatory argument for the YMLMetadataCollector is the integration_name , which is used as your integration ID. 

 All other arguments for the integration are passed in this initialization. See the examples below. Other arguments include: 
 ...
 To specify integration configuration keys, use the keyword conf=[ConfKey(), ConfKey()] . For example: 
 ...
 This results in the following metadata for the configuration key: 
 ...
 To add command metadata, wrap the command function using @metadata_collector.command() . For example: 
 ...
 To reduce code duplication, reuse the following arguments: 
 ...
 When running funky_command(client=SomeClient) the following is printed: 
 ...
 Specify restore=True in the metadata_collector.command arguments. 

 The restored arguments do not show up in the command arguments in the YAML. 

 Other arguments, including client , args , can be added to the function declaration without issues. 
 ...
 This results in the following metadata for the command outputs: 
 ...
 To add command description, you can specify it in @metadata_collector.command under the description key. 
 ...
 To explicitly add input arguments use the InputArgument object. For example: 
 ...
 Where options and input_type behave similarly to those in the ConfKey: options to specify predefined arguments with list of options, and input_type to specify an enum class as the predefined options. 
 ...
 In order to specify the input argument as a default argument of the command (to be used without specifying argument name), specify default_arg=True . For example !ip 1.1.1.1 will be equivalent to !ip ip=1.1.1.1 if ip is set as a default argument. 
 ...
 Where output_type is converted from a Python type to available context outputs types. 

 The output prefix in the context data is the one specified in the @metadata_collector.command , for our example it will be funk . 

 Multiple prefixes 

 Tto have multiple prefixes, specify the multiple_output_prefixes=True argument in @metadata_collector.command and add a prefix argument to every OutputArgument . 
 ...
 If prefix was not specified in the OutputArgument , the general outputs_prefix provided in @metadata_collector.command is used. If outputs_prefix is not defined, the integration_name provided in YMLMetadataCollector initialization is used. 

 Metadata from docstring and declaration 

 As an alternative to specifying the command metadata in the @metadata_collector.command usage, you can specify the description, the output arguments, and the input arguments in the docstring of the function. 

 Description 

 To specify a description of the command in the docstring, use the first line (or lines). For example: 
 ...
 The description is Very funky command . Use an empty line to end the description. For example: 
 ...
 The description is The command is used to give up the funk.\n It is a very funky command. . 

 Args 

 To specify input arguments, use the Args section. For example: 
 ...
 This results in the following input argument metadata: 
 ...
 To specify whether the argument is an array, you can either add an argument type in the docstring, such as: 
 ...
 or specify it as a type in the function declaration, such as: 
 ...
 If your argument has predefined options, you can either add it as an enum class as the argument type, such as: 
 ...
 Or specify options=[<option1<,<option2>, ...] . such as: 
 ...
 To specify a default, use default=<The default>. For example: 
 ...
 To specify the argument as a default argument, add default argument. to the argument description. This will result in being able to use the argument without specifing argument name. For example !ip 1.1.1.1 will be equivalent to !ip ip=1.1.1.1 if ip is set as a default argument. 
 ...
 The docstring in the example above results in the following input argument metadata: 
 ...
 To specify command outputs, use the Context Outputs section. For example: 
 ...
 To specify multiple output prefixes in the docstring you can specify the multiple_output_prefixes=True in @metadata_collector.command and add the prefixes to the command names. For example: 
 ...
 Metadata from explicit @metadata_collector.command arguments can be used along with metadata provided in docstring , as long as it is not the same field. For example, you can provide inputs_list via @metadata_collector.command and Context Outputs in the docstring , specifying both inputs and outputs. 

 If a field is specified explicitly in @metadata_collector.command and in the docstring , the explicit one is used. 
