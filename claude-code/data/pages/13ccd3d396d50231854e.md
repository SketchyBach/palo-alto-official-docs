---
url: https://cortex-docs.paloaltonetworks.com/cortex-agentix/configure-cortex-agentix/cortex-agentix-data-sources-and-connectors/vendor-specific-data-sources-and-connectors/salesforce/ingest-and-run-salesforce-automation-and-remediation
fetched_at: 2026-09-06T10:16:17Z
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Use Salesforce connector data for Cortex AgentiX. 

 Cortex AgentiX provides different methods for connecting to your Salesforce instance. Your choice depends on whether you need to ingest security event logs for monitoring, use the guided wizard setup for integrated services, or deploy specific legacy content packs for niche workflows. 
...
Requires the following OAuth scopes: 
 ...
 Perform requests on your behalf at any time (refresh_token) Essential for background automation. Allows Cortex AgentiX to rotate expired tokens and stay connected 24/7 without manual login. 

 Cortex AgentiX can ingest identity metadata, login history, audit trails, and security monitoring events from Salesforce via capabilities to help you secure user identities, monitor for real-time threats, and automate issue response. To simplify setup, a wizard enables you to select specific capabilities based on your operational needs. The wizard then automatically identifies and provisions the underlying integrations required to support these capabilities. 
 ...
 Find your application (the one that you defined for Cortex AgentiX), click the arrow button in the last column, and select Edit Settings. 
 ...
 The Consumer Key and Consumer Secret will be sent to the email address that you configured earlier for the Cortex AgentiX External Client App. 
 ...
 Find your application (the one that you defined for Cortex AgentiX), click the arrow button in the last column, and select Edit Policies. 
...
Do not include https:// or trailing slashes. 
 ...
 Click Add to create the path. Once the status of the path shows as Approved in the Egress Configuration table, Cortex AgentiX can successfully authenticate and execute commands against your Salesforce environment. 

 Task 6. Configure Cortex AgentiX 

 In Cortex AgentiX, navigate to Settings → Data Sources & Integrations . 
...
The new Salesforce connector has the description: Salesforce CRM services for identity management, automation, remediation and SaaS Posture Security. 
 ...
 You can edit and test an existing instance after a successful initial connection between Salesforce and Cortex AgentiX. Do this by clicking Test . 

 Important 

 If a “connected application” for Cortex AgentiX data collection already exists, there is no obligation to migrate to an “External Client App” - but as Connected Applications are deprecated by Salesforce, it is recommended to migrate. 
