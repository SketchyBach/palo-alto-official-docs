---
url: https://docs.paloaltonetworks.com/prisma-browser/integrations/third-party-integrations/enforcing-prisma-browser-mobile-access-on-managed-devices-usiing-mdm-app-configuration
fetched_at: 2026-09-16T08:22:05Z
source: palo-alto-main
---

# Enforce Prisma Browser for Mobile Access on Managed Devices Using MDM App
        Configuration Clear
...
Updated on 
 ...
 Enforcing Mobile Access on Managed Devices Using MDM App Configuration 
 ...
 Prisma Browser for Mobile. By integrating with your existing MDM (Mobile
 Device Management) solution, you can enforce Prisma Browser sign-in policies
 based on device management status and reuse existing MDM compliance rules. 

 Key Benefits 

 Secure Access : Enforce that only MDM-enrolled, compliant devices can
 access Prisma Browser . 

 Reuse Compliance Rules : No need to redefine device posture separately in
 Prisma Browser —leverage what’s already set in your MDM. 
 ...
 necessary compliance checks and that Prisma Browser for Mobile is deployed with
 ...
 Prisma Browser for Mobile access on iOS-devices. Configure Microsoft Intune
 as follows: 
 ...
 You can configure Microsoft Intune as the MDM that will be used to enforce Prisma Browser for Mobile access on Android devices. The method is very similar
 to the method for iOS, with a few changes. This section will cover the differences.
 ...
 In the Prisma Browser , navigate to Directories → Devices . 
 ...
 Prisma Browser for Mobile application via the MDM AppConfig. 

 2. Open the MDM Console 

 Use the following process to push the management key to the Prisma Browser for
 Mobile via AppConfig. 
 ...
 Value : Enter the Configuration value from the Prisma Browser console. 

 To
 avoid accidentally mistyping this value, we recommend that you
 copy the value and paste. 
 ...
 process is similar—create an AppConfig with the Prisma Browser _management_key and assign it to relevant devices/groups. 
 ...
 On start-up, the Prisma Browser app reads the configuration value using
 platform-native AppConfig APIs. 
 ...
 The Prisma Browser app reads the configuration from the
 ...
 Keys can be rotated anytime in the MDM and console. 

 Security is at least equivalent to Desktop/Mac MDM detection. 
