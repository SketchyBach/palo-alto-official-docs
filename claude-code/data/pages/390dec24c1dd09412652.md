---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-cloudflare
fetched_at: 2026-09-16T07:25:19Z
source: palo-alto-main
---

# Provision to Cloudflare Clear
...
Updated on 
 ...
 Use this procedure to provision a certificate from Next-Gen Trust Security to a Cloudflare machine. 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Installations > Machines . 

 Select the Cloudflare machine where you want to install the certificate. 
 ...
 Note: The selected certificate must match the Cloudflare zone name . A zone typically corresponds to a DNS zone for a specific domain. 

 In Zone , enter the Cloudflare zone name as it appears in your Cloudflare account. 

 (Optional) In Description , enter a description for the installation. 

 In Bundle Method , select one of the following options: 

 User Defined – Paste a custom certificate chain. Specify intermediates followed by the leaf certificate. Use this option for self-signed certificates. 

 Modern – Builds an efficient chain using newer or fewer intermediate certificates. 

 Compatible – Uses widely supported intermediate certificates to maximize client compatibility. 

 Note: If your Cloudflare plan supports dual certificate packs (both modern ECC and legacy RSA certificates), you can provision both certificate types to the same zone. Next-Gen Trust Security installs both certificates successfully, and the machine identity status displays INSTALLED for each certificate. Cloudflare automatically serves the appropriate certificate type (ECC to modern browsers, RSA to legacy or mobile clients) based on the client's capabilities. 

 (Optional) To prevent the certificate from being pushed immediately, set Push upon saving to No . 

 Note: Cloudflare assigns a certificate ID only after the certificate is pushed. Until the certificate is pushed, you cannot create another installation for the same zone. 

 Click Save . 

 After saving, Next-Gen Trust Security provisions the certificate to the specified Cloudflare zone and creates an installation record on the Installations tab. 
