---
url: https://docs.paloaltonetworks.com/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-aws/panorama-plugin-for-aws-410/what-s-new-in-panorama-plugin-for-aws-410
fetched_at: 2026-09-16T07:42:43Z
source: palo-alto-main
---

# What's New in Panorama Plugin for AWS 4.1.0 Clear
...
Updated on 
 ...
 Tag Pruning: The tag pruning enables you to send IPs for Dynamic Address
 Groups used in security Policies. You can now monitor and learn more tags. It also
 overcomes the 10MB payload size limitation. 
 The plugin implements tag pruning to
 match tags in the payload with those configured in Dynamic Address Groups used
 in Security Policies. This process retains IPs for tags used in security
 policies at the Device Group level while pruning unused tags. The plugin sends
 IPs only for tags utilised in security policies within the configured Device
 Group. Security policies are parsed after each Panorama commit to update
 IP-Tags. Empty tags remain available for creating Dynamic Address groups but are
 not propagated to the firewall unless used in a Security Policy. This ensures
 efficient tag management and policy enforcement across the network
 infrastructure. 
 ...
 Dynamic Address Group and use it in a security policy. 
 ...
 You
 cannot club the above mentioned use cases together to nest Dynamic
 Address Groups. 
 ...
 SDB, running the following command: 
 request aws plugin
 sync 

 On a High Availability Panorama setup with plugin 5.3.0 and
 below - the tag pruning CLI commands must be configured on both the
 HA peers. 

 Ensure that your tag matching criteria has a definitive format under
 the Objects section of Panorama when creating Dynamic Address
 groups. 

 For example: 

 If there is a space between ' and at the start of
 the IP tag or at the end of the IP tag, then it will be an invalid
 tag. 

 ' aws.ec2.tag.Name.Client-1' - This is an example of
 invalid tag. The tag contains space between start of the IP tag and
 the single quote. 

 'aws.ec2.tag.Name.Client-1 ' - This is an example of
 invalid tag. The tag contains space between end of the tag and the
 single quote. 
