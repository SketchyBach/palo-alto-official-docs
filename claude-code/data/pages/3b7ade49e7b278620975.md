---
url: https://docs.paloaltonetworks.com/sd-wan/help/3-4/panorama-sd-wan-plugin/sd-wan-devices
...
SD-WAN Devices 
 ...
 Onboard SD-WAN branch office and hub site firewalls to Panorama for centralized policy
...
appear consecutively on the list of devices for ease of use. 
 ...
 You can select BGP Policy to have Panorama create and push to firewall a
 Security policy rule that enables BGP to run between branches and hubs. In SD-WAN plugin
 ...
 resources at the hub location; the hub processes traffic,
 ...
 ( Mandatory for HA peers ) ( PAN-OS 12.1.2 and later
 releases, SD-WAN 3.4.0 and later releases ) Enable
 Add HA Peer to configure both SD-WAN
 enabled HA peers (active and passive devices) simultaneously from a
 single window. 

 Enable Multi-VR Support 

 ( Optional ) ( PAN-OS 11.2.0 and later releases,
 and SD-WAN plugin 3.3.0 and later releases )
 Enable Multi-VR Support to
 configure multiple virtual routers on the SD-WAN hub. 

 With multiple virtual routers on the SD-WAN hub, you can have
 overlapping IP subnet addresses on branch devices connecting
 to the same SD-WAN hub. When you select the
 Type of SD-WAN device as
 Hub , you will be able to
 configure multiple virtual routers on the hub by selecting
 the Enable Multi-VR Support 
 option. 

 When you enable this feature, ensure
 that the virtual router names on branches match with the
 virtual router name on at least one hub part of the VPN
 cluster. 

 ( Optional ) ( PAN-OS 11.2.3 and later releases,
 and SD-WAN plugin 3.3.1 and later releases )
 Enable Multi-VR Support to
 configure multiple virtual routers on the SD-WAN branch. 

 With multiple virtual routers on the SD-WAN branch, you can
 have overlapping IP subnet addresses on both hub and branch
 devices. When you select the Type of
 SD-WAN device as Branch , you will be
 able to configure multiple virtual routers on the branch by
 selecting the Enable Multi-VR Support 
 option. 
 ...
 Link Tag (when hub firewall is enabled) 

 Select the Link Tag you created for the hub virtual interface
 (or branch virtual interface), which Auto VPN will assign to the
 virtual interface. You’ll use this Link Tag in a Traffic
 Distribution profile to enable the hub (or branch) to participate in
 DIA AnyPath. 

 ( Mandatory for HA peers ) ( PAN-OS 12.1.2 and later
 releases, SD-WAN 3.4.0 and later releases ) HA
 Peer 

 HA Peer Name 

 SD-WAN plugin auto fills the secondary HA peer for the selected
 primary HA peer. 

 HA Peer Site 

 Enter a user-friendly site name that identifies the HA peer. For
 example, enter the city name where the HA peer is deployed. 

 ( PAN-OS 12.1.5 and later releases, SD-WAN 3.4.1 and later
 releases ) Capacity Setting 

 Enable the Capacity-Setting to optimize memory
 usage and enter SD-WAN rules, virtual interfaces (VIFs), and VIF
 members to adjust the capacity values to meet your deployment
 requirements. 

 The Capacity-Setting configuration allows you
 to define the maximum operational limits for SD-WAN. This helps you
 manage hardware resources by aligning the device capacity with your
 specific network scale requirements. 

 When you enable the capacity settings, the
 device automatically populates the fields with default values. You
 can then modify these values based on your preferences. 
 ...
 Choose or enter a value in the Max Tunnels Per
 VIF . 

 Error Correction 

 To enable Forward Error Correction , click the
 Error Correction . 

 Validate Capacity 

 To ensure the total memory consumption remains within platform
 limits, click Validate Capacity . 

 Once the configuration passes validation, push the settings to the
 firewall. You must manually reboot the firewall for the new capacity
 settings to take effect before making any configuration change. 
 ...
 routes with a hub or branch firewall in a SD-WAN BGP topology and
 ...
 commit to all SD-WAN cluster nodes, and subsequently downgrade to a
 SD-WAN plugin version earlier than 2.0.2, and then, you must
 perform all configuration related to Remove Private
 AS outside of the SD-WAN plugin or directly on the
 firewalls. 
 ...
 and push to firewall a Security policy rule that enables BGP to run
 ...
 ( Optional ) ( Multiple virtual routers support on
 SD-WAN hub — Supported in PAN-OS 11.2.0 and later
 releases, and SD-WAN plugin 3.3.0 and later
 releases ) 

 ( Optional )
 ( Multiple virtual routers support on SD-WAN branch —
 Supported in PAN-OS 11.2.3 and later releases, and
 SD-WAN plugin 3.3.1 and later releases ) 

 Virtual Router 

 Enter a virtual router name to define a new virtual router. 

 Zone 

 Select an already created Zone in
 the hub template ( Network Zones ) that’s appropriate for the virtual router that you're
 configuring. 
 ...
 Select this tab if you're adding a SD-WAN hub or branch device that
 ...
 inner IPv4 header to the VPN header of the encapsulated packets to
 preserve the original ToS information. This also copies the Explicit
 ...
 Local certificate type is
 selected by default. SCEP is not supported. 

 Certificate Name —Enter a name for the
 certificate you're importing. 

 File Format —Select one of the
 following: 

 Encrypted Private Key and Certificate
 (PKCS12) —Contains both the certificate
 and the key. 

 Multiple Certificates
 (.tar) —Contains multiple certificates
 archived in a tar format. 

 CSV File Name —Enter the
 CSV filename that you want to import. 

 Download Sample CSV —To
 import multiple certificates, click and download
 the Certificates.csv file and enter the required
 details. 

 Archive all the certificates along with the
 .csv file into a .tar file. Certificate format
 .pkcs12 is supported. 

 Certificate File —Click
 Browse to navigate to the
 location where the certificate file is located. Click on the
 file and select Open . 

 Block Private Key Export —Click to
 prevent any administrators, including superusers, from
 exporting the private key for the specified certificate. 

 Passphrase and Confirm
 Passphrase —Enter to access the key. 
 ...
 Certificate Name —Enter a name for the
 certificate you're creating. 

 Common Name —Enter the Common Name,
 which is the IP address or FQDN to appear on the
 certificate. 

 Shared —Click if this certificate is to
 be shared among multiple virtual systems. 

 Signed By —Select External Authority
 (CSR) or enter the firewall IP address. This entry must be a
 CA. 

 Certificate Authority —Click if the
 firewall is the root CA. 

 Block Private Key Export —Prevents any
 administrators, including superusers, from exporting the
 private key. 

 Algorithm —Select RSA or Elliptic Curve
 DSA to generate the key for the certificate. 

 Number of Bits —Select 512, 1024, 2048,
 or 3072 as the number of bits in the key. 

 Digest —Select md5, sha1, sha256,
 sha384, or sha512 as the method to revert the string from
 the hash. 

 Expiration (days) —Enter the number of
 days that the certificate is valid. 

 Certificate Attributes :
 Type —Select additional attribute
 types (DN, FQDN, IPADDR, UFQDN) from the drop-down to be in
 the certificate. 
 ...
 Click the check box at the bottom of the screen to cause HA peers to
 appear consecutively on the list of devices for ease of use. 
 ...
 Select the physical, subinterface, or Aggregate Ethernet interface
 ...
 the branch firewall to Prisma Access and retrieve one or
 more service IP addresses to the Prisma Access compute
 nodes. 
