---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-azure/panorama-plugin-for-azure-321/addressed-issues-in-panorama-plugin-for-azure-321.html
fetched_at: 2026-09-16T12:59:31Z
source: palo-alto-main
---

# Addressed Issues in Panorama Plugin for Azure 3.2.1 Clear
...
in the Panorama plugin for Azure 3.2.1. 
 ...
 Fixed an issue where the Azure plugin
failed to upgrade the PAN-OS version of the deployed firewalls through
redeploy. 

 PLUG-10775 

 Fixed an issue where the Azure plugin
would not process a subscription successfully, if the tags contained
special characters. 

 PLUG-10785 

 Fixed an issue where an undeploy that
was triggered following a failed deployment, was not clearing the template-stacks
and device-groups that were created during the deployment. 

 PLUG-10840 

 Fixed an issue where the InvalidResourceReference error
was observed while adding an inbound stack to an existing deployment
and redeploying it. 

 PLUG-10898 

 Fixed an issue where the vNet Resource
Groups were not refreshed when the regions were changed in the UI. 
