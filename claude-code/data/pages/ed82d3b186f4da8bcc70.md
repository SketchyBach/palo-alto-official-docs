---
url: https://cortex-docs.paloaltonetworks.com/cortex-cloud-runtime-security/endpoint-security/install-and-manage-endpoints/set-up-endpoint-protection/set-up-endpoint-profiles-and-exception-rules/set-up-agent-settings-profiles
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Configure the User Interface options for Cortex Cloud. 
 ...
 When enabled, allows access to Cortex Cloud. 
 ...
 Enable XDR Pro Endpoint Capabilities , and then configure the capabilities required by your organization. The Cortex Cloud Pro features are hidden until you enable this option. 

 Notice 

 Requires a Cortex Cloud Pro per Endpoint license. When you enable this feature, a Cortex Cloud Pro per Endpoint license is consumed. 
 ...
 With this option you can also select the File Search and Destroy Monitored File Types where Cortex Cloud monitors all the files on the endpoint, or only common file types. If you choose Common file types, Cortex Cloud monitors the following file types: 
 ...
 Depending on the type of scan you defined, the agent Ping scan takes 30 minutes, and Nmap takes 60 minutes. Following each scan, Cortex Cloud aggregates the IP addresses that were collected, and displays the results in the Asset Management table. 
 ...
 If you disable content updates, the agent stops retrieving them from the Cortex Cloud tenant, and keeps working with the current content on the endpoint. 
 ...
 For One release before the latest one , Cortex Cloud upgrades the agent to the previous release before the latest, including maintenance releases. Major releases are numbered X.X, such as release 8.0, or 8.2. Maintenance releases are numbered X.X.X, such as release 8.2.2. 
 ...
 Cortex Cloud deploys serverless peer-to-peer distribution to Cortex XDR agents in your LAN network by default. Within the six hour randomization window during which the Cortex XDR agent attempts to retrieve the new version, it will broadcast its peer agents on the same subnet twice: once within the first hour, and once again during the following five hours. If the agent did not retrieve the files from other agents in both queries, it will proceed to the next download source defined in your profile. 

 To enable P2P, you must enable UDP and TCP over the port specified for P2P Port . By default, Cortex Cloud uses port 33221. You can change the port number, if required by your organization. 
 ...
 If the files are not available on the Broker VM at the time of the request, the agent proceeds to download the files directly from the Cortex Cloud server. 
 ...
 When the option Upload Using Cellular Data is enabled, the Cortex XDR agent uses cellular data to send unknown apps to the Cortex Cloud for inspection. Standard data charges may apply. When this option is disabled, the Cortex XDR agent queues any unknown files and sends them when the endpoint connects to a Wi-Fi network. If configured, the data usage setting on the Android endpoint takes precedence over this configuration. 
