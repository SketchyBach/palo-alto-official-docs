---
url: https://cortex-docs.paloaltonetworks.com/cortex-xdr-5.x/protect-your-endpoints/install-and-manage-endpoints/set-up-endpoint-protection/set-up-endpoint-profiles-and-exception-rules/set-up-malware-prevention-profiles
fetched_at: 2026-09-06T09:42:22Z
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Configure malware prevention profiles for supported endpoint platforms. 
 ...
 In this profile, the Report options configure the endpoints to report the corresponding suspicious files, actions, processes, or behaviors to Cortex XDR, without blocking them. The Disabled options configure the endpoints to neither analyze nor report the corresponding malware or behavior. 

 Risky Profile Status: When setting an action to anything other than the recommended security posture, the platform may flag the profile as Risky because potentially malicious code can run or persist on your endpoints. 
 ...
 When enabled, the Cortex XDR agent sends unknown files to Cortex XDR, and Cortex XDR sends the files to WildFire for analysis. 
 ...
 When enabled, Cortex XDR treats all grayware with the same Action Mode as configured for malware. 
 ...
 When enabled, the Cortex XDR agent sends unknown files to Cortex XDR, and Cortex XDR sends the files to WildFire for analysis. For macro analysis, the Cortex XDR agent sends the Microsoft Office file containing the macro. 
 ...
 For On-Write File Examination settings, configure the actions that Cortex XDR should take during the on-write process for various file types.
When a file type is enabled, the Cortex XDR agent monitors for malicious files during the on-write process, and if it finds any, it generates issues and quarantines the files. 
 ...
 If on-write actions were configured in earlier versions of Cortex XDR, the same configuration has been preserved and applied globally for all file types. 
 ...
 Configure Java Files Examination to analyze and prevent malicious Java files from running. File types supported: jar, class, java, jsp, jspx, war. 
 ...
 When the Cortex XDR agent detects attempts to run malicious Java files, it performs the configured action.
Risky Profile Status: Setting the action to Block is the recommended security posture. If you set this to Report or Disabled, the platform may flag the profile as Risky because potentially malicious Java code can run or persist on your endpoints. 
 ...
 Upload files to WildFire 

 Enabled 

 Disabled 

 When enabled, the Cortex XDR agent sends unknown files to Cortex XDR WildFire for analysis. 
 ...
 When the Cortex XDR agent identifies a remote network connection that attempts to perform malicious activity—such as encrypting endpoint files—the agent can automatically block the IP address to close all existing communication, and to block new connections from this IP address to the endpoint. When Cortex XDR blocks an IP address per endpoint, that address remains blocked throughout all agent profiles and policies, including any host-firewall policy rules. You can view the list of all blocked IP addresses per endpoint from the Action Center, as well as unblock them to re-enable communication as appropriate. 
 ...
 The Report option configures XDR agents to analyze connections, to allow the transmission of packets in your network, but to report them to Cortex XDR. 
 ...
 Configure ELF Loading Examination to detect and prevent ELF files from being loaded. 
 ...
 By default, Cortex XDR agent blocks malware from running, but does not quarantine the file. You can enable one of the options to quarantine files, depending on the verdict issuer. 
 ...
 Treat grayware as malware : When enabled, a grayware verdict is considered malware.
When disabled, grayware is considered benign. 
