---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-21/vm-series-plugin-217/addressed-issues-in-vm-series-plugin-217.html
fetched_at: 2026-09-16T12:59:05Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 2.1.7 Clear
...
fixed in the VM-Series plugin 2.1.7. 

 PLUG-10236 

 Fixed an issue in AWS environments, where
the bootstrapped PA-VMs in HA mode were observed to change to interface
mode after the first reboot. 

 PLUG-10237 

 Fixed an issue in AWS environments, where
the command show plugins vm_series aws ha state was
not displaying valid outputs on the passive firewall. 

 PLUG-10393 

 Fixed and issue where the plugin reports AWS Cloudwatch metric
 SSLProxyUtilization as number of concurrent SSLProxy sessions
 instead of a percent value between 0 to 100. 

 PLUG-10443 

 Fixed an issue that caused autoscaled
VM-Series firewalls on Azure to be named PA-VM instead of the hostname
provided during configuration. 

 PLUG-10498 

 Fixed an issue where the interfaces incorrectly show Driver name
 mismatch. Setting to pktmmap after a PAN-OS upgrade to 10.1. 

 PLUG-10639 

 Fixed an issue where users were unable
to login to PAN-OS 10.1.X PA-VM launched in Azure from custom image. 
