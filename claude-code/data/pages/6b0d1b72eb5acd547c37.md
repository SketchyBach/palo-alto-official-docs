---
url: https://docs.prismacloud.io/content-collections/runtime-security/vulnerability-management/scan-serverless-functions
fetched_at: 2026-09-16T13:35:22Z
source: prisma-cloud
---

# Configure Serverless Function Scanning | Prisma Cloud arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 In the console, go to Defend > Vulnerabilities > Functions > Functions . . Click on Add Rule . . Enter a rule name and configure the rule. . In the rule configuration, add a Collection that includes serverless functions deployed in the specific cloud region. 

 + For more information on creating collections, see Collections topic . Click Save . . View the scan report by navigating to Monitor > Vulnerabilities > Functions > Scanned functions . 
 ...
 Scanning Google Cloud Functions (Gen 1) 
 ...
 Scanning Google Cloud Functions (Gen 2) 

 Prisma Cloud provides comprehensive security for Google Cloud Functions, second generation (Gen 2), by monitoring both the function management layer and the underlying infrastructure. 

 Gen 2 functions are essentially an abstraction layer built on top of Google Cloud Run. To ensure no security blind spots, Prisma Cloud interacts with both the user-facing Google Cloud Functions API and the underlying Google Cloud Run API. 

 Deployment Methods and Scanning Coverage 

 Gen 2 functions utilize Cloud Run infrastructure, which adds another method for resource deployment. Prisma Cloud secures functions deployed with both methods: 

 Gen 2 Functions created using the Google Cloud Functions API. Prisma Cloud scans the function metadata and source code. 

 Cloud Run Service created using the Google Cloud Run API. Prisma Cloud scans these resources and their deployment artifacts as part of its Cloud Run coverage. 

 IAM Permissions Required 

 To secure Gen 2 functions effectively, Prisma Cloud requires read access to function metadata, the underlying Cloud Run service, and the deployment artifact (ZIP or container). 

 Standard Permissions 

 The following permissions are required to map the function to its storage artifact. 

 Note: These are included in the onboarding Terraform. 
...
Required. Complete URI for Console, including the protocol and port. Only the HTTPS protocol is supported. 
 ...
 Access Key ID to access Prisma Cloud. If not provided, the TWISTLOCK_USER environment variable is used, if defined. Otherwise, "admin" is used as the default. 
