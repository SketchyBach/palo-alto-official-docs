---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-vmware-nsx/vmware-nsx-plugin-201/addressed-issues-in-panorama-plugin-for-vmware-nsx-201.html
fetched_at: 2026-09-16T13:00:19Z
source: palo-alto-main
---

# Addressed Issues in Panorama Plugin for VMware NSX 2.0.1 Clear
...
Updated on 
 ...
 The following list describes issues address
in the Panorama plugin for VMware NSX 2.0.1. 

 PLUG-358 

 Fixed an issue that prevented services from being configured
in steering rules on Panorama. 

 PLUG-351 

 Fixed an issue that caused commits to
fail when upgrading Panorama to 8.1 because template references
were not migrated from the template to the template stack. 

 PLUG-344 

 Fixed an issue that prevented the generation
of a steering rule when a valid intrazone/interzone security is
configured. 

 PLUG-339 

 Fixed an issue where, after importing
a Panorama configuration to a new Panorama, the new Panorama reconfigures
the Service Definition OVF locations in NSX Manager when it is not
required, which causes a service deployment failure. 
