---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/c-about-activity-logging/t-create-apply-activityLogging-filter
fetched_at: 2026-09-16T07:24:49Z
source: palo-alto-main
---

# Finding Logged Data Through Filtering Clear
...
Updated on 
 ...
 Click System Settings > Certificate Settings > Event Logs . 

 Click + , and then select one or more filters. 

 For each filter, select one or more values from the dropdown. 
 ...
 Event log filter category Criteria Example use 

 Criticality 0, 1 Review important events such as login failures or role changes. 

 Description Free-form input Find events related to a specific user name or IP address. 

 Event Login Failed View recent failed login attempts. 

 Event User Role Changed Review changes to user roles. 

 Event Certificate Issued See when certificates were issued and to whom. 

 Event Certificate Retired Review certificate retirement activity. 

 Event Certificate Validation Finished Check certificate validation results and status. 

 Event Type Authentication, Certificates, Certificate Authority, Users Filter events by high-level functional area. 

 Timestamp 7, 14, 30 days View events that occurred within a recent time window. 

 Workspace One or more workspaces Narrow the log to the events that happened in specific workspaces. 

 Filter the Event Log by Workspace 

 The Workspace filter appears only when Tenant is selected, because inside a workspace the log already covers that workspace alone. It uses a single operator, in , and accepts more than one workspace, so you can compare activity across a few workspaces without giving up the rest of your filters. 

 Click + , and then select Workspace . 

 Select one or more workspaces from the dropdown. 

 Click Apply Filter . 

 Note: The Workspace column cannot be sorted. Filter on it instead, or sort by Timestamp within the workspaces you have filtered to. 
