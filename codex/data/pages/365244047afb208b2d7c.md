---
url: https://docs.paloaltonetworks.com/ngfw/help/10-1/device/device-certificate-management-certificate-profile
...
You can configure a separate certificate profile for each of these services. 
 ...
 ( Required ) Enter a name to identify
...
After you save the profile, you can’t change its Location . 
 ...
 If GlobalProtect only uses certificates
...
Name. 
 ...
 None —Typically for GlobalProtect device
or pre-login authentication. 

 Domain 

 Enter the NetBIOS domain so the PAN-OS software
...
OCSP URL (starting with http:// or https:// ). 

 By default, the firewall uses the certificate selected in
...
the template that was used to sign the certificate. 
 ...
 Select this option to use a certificate
...
back to the CRL method if the OCSP responder is unavailable. 
 ...
 Specify the interval (1 to 60 seconds) after
...
status cannot be retrieved within timeout 
...
issued to the authenticating device 

 ( GlobalProtect only ) Select this
