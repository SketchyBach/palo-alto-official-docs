---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/10-0/pan-os-panorama-api/pan-os-xml-api-use-cases/upgrade-a-firewall-to-the-latest-pan-os-version-api.html
...
content download completes successfully: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<show><jobs><id>2</id></jobs></show>&key=<apikey>'
...
content: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd<request><content><upgrade><install> <version>latest</version></install></upgrade></content></request>key=<apikey>'
...
the content installation completes successfully: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<show><jobs><id>3</id></jobs></show>&key=<apikey>'
...
for the latest available PAN-OS software updates: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<request><system><software><check></check>
...
so download that version: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<request><system><software><download><version>7.1.0-c65</version></download></software></system></request>&key=<apikey>' 

 Use the jobid in the response to ensure that the system
update download completes successfully: 

 curl -X GET 'https://<firewall>/api/?type=op&cmd=<show><jobs><id>318</id></jobs></show>&key=<apikey>'
...
to ensure that the system update installs successfully: 
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<show><jobs><id>320</id></jobs></show>&key=<apikey>'
 ...
 curl -X GET 'https://<firewall>/api/?type=op&cmd=<request><restart><system></system></restart></request>&key=<apikey>'
