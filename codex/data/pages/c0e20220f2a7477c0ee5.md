---
url: https://docs.paloaltonetworks.com/pan-os/10-2/pan-os-release-notes/pan-os-10-2-10-known-and-addressed-issues/pan-os-10-2-10-known-issues
fetched_at: 2026-09-16T07:37:47Z
source: palo-alto-main
---

# PAN-OS 10.2.10 Known Issues Clear
...
Updated on 
 ...
 10.2.10. This list includes issues specific to Panorama™, GlobalProtect™, VM-Series
 ...
 The WildFire analysis report on the firewall log
 viewer ( Monitoring WildFire Submissions ) does not display the following data fields: File
 ...
 The sample analysis statistics that are returned when
 ...
 The following WildFire appliance CLI command does not
 ...
 The WildFire appliance might erroneously generate and
 ...
 Workaround: Ensure all WildFire
 appliance interfaces that are enabled use matching protocols (all
 IPv4 or all IPv6). 

 WF500-5632 

 The number of registered WildFire appliances reported
 in Panorama ( Panorama Managed WildFire Appliances Firewalls Connected View ) does not accurately reflect the current status of
 ...
 counter is missing from debug log-receiver statistics .
 This causes the firewall to match a HIP object but not on the HIP
 ...
 PAN-270549 ( PAN-OS 10.2.10-h2 through PAN-OS
 10.2.10-h12 ) 
 ...
 ( PA-5250 firewall only ) Applying filters under Monitor Log Display either causes the logs to load slowly or not load at
 all. 
 ...
 Exporting reports in PDF or CSV format and processing hourly
 ...
 PAN-263226 ( PAN-OS 10.2.10-h2 and 10.2.10-h3
 only ) 
 ...
 If you use PAN-OS 10.2.10-h3, 10.2.11, or an earlier version,
 configure the RADIUS server so that it does not send the message
 authenticator back to client. 

 Use other protocols, such as LDAP, Kerberos, TACACS+, SAML,
 RADIUS EAP, instead of RADIUS PAP or CHAP. 
 ...
 an SSH Service Profile ( Device Certificate Management SSH Service Profile ) Hostkey configured in a Template from the Template
 Stack. 
 ...
 On the Panorama management server, different threat
 names are used when querying the same threat in the Threat Monitor ( Monitor App Scope Threat Monitor ) and ACC . This results in the
 ...
 Type for an existing administrator ( Device Administrators or Panorama Administrators ) from Superuser to a
 ...
 A file upload to Box.com exceeding 6 files gets stuck
 and fails to upload if you specify an Enterprise DLP data filtering
 profile ( Objects DLP Data Filtering Profiles with the Action set to
 Block to a Security policy rule ( Policies Security ). 

 PAN-204689 

 Upon upgrade to PAN-OS 10.2.4, the following
 ...
 On the Panorama management server, pushing a
 ...
 Preview Changes ( Commit Push to Devices Edit Selections or Commit Commit and Push Edit Selections ). 
 ...
 When using a 10.2.2 Panorama to manage a Panorama
 ...
 custom payload format under Device Server Profiles HTTP yields a Javascript error. 
 ...
 Workaround: Configure the log interface
 ...
 Workaround: Restart the log receiver
 ...
 displays no synchronization status ( Panorama Managed Devices Summary ) after a bootstrapped firewall is successfully added
 to Panorama. 

 Workaround: After the bootstrapped
 firewall is successfully added to Panorama, log in to the Panorama web
 interface and select Commit Push to Devices . 

 PAN-187643 

 If you enable SCTP security using a Panorama template
 ...
 The configured Advanced Threat Prevention inline cloud
 ...
 On a firewall with Advanced Routing enabled, if there
 ...
 Workaround : Do not use a
 logical router instance with no interfaces bound to it. 

 PAN-186283 

 Templates appear out-of-sync on Panorama after
 ...
 On HA deployments on AWS and Azure, Panorama fails to
 populate match criteria automatically when adding dynamic address
 groups. 

 Workaround: Reboot the Panorama HA
 pair. 

 PAN-184406 

 Using the CLI to add a RAID disk pair to an M-700
 appliance causes the dmdb process to crash. 
 ...
 Static IP addresses are not recognized when "and"
 operators are used with IP CIDR range. 

 PAN-181933 

 If you use multiple log forwarding cards (LFCs) on the
 ...
 On a PA-5400 Series firewall (minus the PA-5450),
 ...
 On the Panorama management server, pushing an
 unsupported Minimum Password Complexity ( Device Setup Management ) to a managed firewall erroneously displays
 commit time out as the reason the
 commit failed. 

 PAN-180104 

 When upgrading a CN-Series as a DaemonSet
 ...
 A user interface issue in PAN-OS renders the contents
 ...
 PAN-OS 10.2.0 is not supported on PA-7000 Series
 ...
 When the firewall is deployed on N3 and N11 interfaces
