---
url: https://docs.paloaltonetworks.com/pan-os/10-1/pan-os-release-notes/pan-os-10-1-14-known-and-addressed-issues/pan-os-10-1-14-known-issues
fetched_at: 2026-08-13T17:03:38Z
source: palo-alto-main
---

# PAN-OS 10.1.14 Known Issues Clear
...
specific to PAN-OS ® 10.1.14. This list includes issues
...
eval or trial license expires and then install the production license. 
 ...
 If you are using the Panorama plugin for Azure, do not upgrade to
 PAN-OS 10.1.12. When installed on 10.1.12, the Panorama plugin for
 Azure fails to connect to Azure. 
...
connecting after you restart the web server with the following command: debug software restart process web-server . 
 ...
 ( PAN-OS 10.1.14-h9 only ) 

 This issue is now resolved. See PAN-OS 10.1.14-h10 Addressed Issues . 
 ...
 DNS resolution may fail if DNS server IP is obtained through
 DHCP. 

 Workaround: Configure the DNS server with a static IP or renew
 ...
 FIPS-CC VM-Series only. Upgrading to PAN-OS 10.1.10-h2 or PAN-OS
 10.1.11 changes all locally created Security policy actions to
 Deny. 

 Workaround: Before upgrading, save a backup of the current
 configuration. After upgrading, load the backup configuration to
 restore the security policy action settings. 
 ...
 DNS resolution fails for firewall and Panorama plugins if the DNS
 ...
 DNS resolution fails when interfaces are configured as DHCP and a DNS
 ...
 The firewall is unable to retrieve the most current external dynamic
...
10.1.9. 
 ...
 When FIPS mode is enabled on VM-Series firewall in Microsoft Azure
