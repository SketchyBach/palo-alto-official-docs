---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/co-overview-issuing-certs/adding-a-certificate-authority/adding-digicert-ca/editing-digicert-ca
fetched_at: 2026-09-16T07:24:54Z
source: palo-alto-main
---

# Manually Refresh DigiCert Clear
...
Updated on 
 ...
 Changes made to certificate authority settings on the CA side can impact Next-Gen Trust Security, for example, if validated organizations are added or removed. 

 If you need to refresh the configuration for a DigiCert CA connection in Next-Gen Trust Security you can use the following procedure. 
 ...
 In the list of CA's, click the DigiCert entry in the inventory. 

 Click Account Refresh. 

 Next-Gen Trust Security will authenticate with DigiCert and use its API to retrieve the current configuration and update the options stored in Next-Gen Trust Security. 

 If the updated configuration makes a request policy invalid, the affected request policy is marked invalid, and you'll see a warning. If that happens, go to the affected request policy(s) and update as needed. 

 What's Next 

 Edit a request policy 
