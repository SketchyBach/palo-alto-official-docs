---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-5-addressed-issues.html
fetched_at: 2026-09-16T10:42:19Z
source: palo-alto-main
---

# PAN-OS 8.1.5 Addressed Issues Clear
...
the Push Scope Selection list ( Commit Push to Devices Edit Selections Device Groups ). 
...
being appended by an external device. 
 ...
 Fixed an issue where PA-800 Series firewalls
...
where the dataplane failed due to an internal path monitoring failure. 
 ...
 Fixed an issue where hardware packet buffers
...
a down interface within an Aggregate Ethernet (AE) interface. 
 ...
 Fixed an intermittent issue where newly
created IPSec Tunnels ( Network IPSec Tunnels Add )
did not activate. 
...
error which caused commits to fail. 
 ...
 Fixed an issue with Panorama where administrators
...
restarted while processing SMTP traffic. 
 ...
 Fixed an issue on Panorama managed devices
where the green Template Values Exist indicator incorrectly displayed
after you closed any interface settings ( Device Setup Interfaces )
even when you did not make any changes. 

 PAN-104771 

 Fixed an issue where the HTTP header insertion
entries caused the dataplane to restart. 

 PAN-104764 

 Fixed an issue on Panorama management server
...
is disabled. 
 ...
 Fixed an issue where a GTP PDP update did
not update the GTP-U session which caused subsequent GTP traffic
to drop. 

 PAN-104524 

 Fixed an issue where the firewall logged
...
feature. 
 ...
 Fixed an intermittent issue where the replace device CLI
command caused the configuration lock to stop responding. 

 PAN-104163 

 Fixed an issue where the show config audit base-version command
continuously increased the number of file descriptors and caused
the management server ( mgmtsrvr ) to exit and restart. 

 PAN-104073 

 Fixed an issue where the replace device old <serial number> new <serial number> command
caused the configuration process ( configd ) to stop
responding. 

 PAN-103820 

 Fixed and issue where the template stack
retains the dynamic update schedule information after you remove
it. 

 PAN-103383 

 Fixed an issue where a firewall blocked
SMTP traffic when processing ZIP files due to too many packet-process
loops. 

 PAN-103346 

 Fixed an issue where the LDAP Service Route
Configuration ( Device Setup Services Service Route Configuration )
did not respond when Customize was selected
and non-management interfaces were enabled. 

 PAN-103248 

 Fixed an issue where the process ( routed ) infinitely
...
the FragmentSmack vulnerability (CVE-2018-5391 / PAN-SA-2018-0012). 
 ...
 Fixed an issue where SSL enabled applications
accessed through a GlobalProtect™ Clientless VPN caused buffer leaks. 

 PAN-102743 

 ( PA-5250, PA-5260, “PA-5280-8.1-only”,
...
enabled log redundancy. 
 ...
 Fixed an issue where installing path monitoring
for static route on a satellite in a Large Scale VPN (LSVPN) infrastructure
failed. 

 PAN-101819 

 Fixed an issue where the Panorama Controller
did not display all commit-all jobs
for Panorama Nodes ( Panorama Interconnect Tasks )
...
that contained incorrect source MAC and source IP addresses. 
 ...
 ( The following PA-7000 Series NPCs only:
...
of heavy traffic. 
 ...
 Fixed an issue where the EDL and FQDN address
...
fields ( Panorama Authentication
Profile Add Authentication )
for the GlobalProtect gateway template stack, the saved configuration
did not get applied. 

 PAN-101425 

 Fixed an issue where after a redistribution
...
configured as the destination interface in a QoS policy rule. 
 ...
 Fixed an issue where Dynamic Updates pushed
...
state ( Device Setup Operations ) from Panorama failed
to import to the firewall. 

 PAN-100448 

 Fixed an issue where a scheduled external
dynamic list refresh displayed incorrect update values. 

 PAN-100447 

 ( VM-Series firewalls in a high availability
...
where Dead Peer Detection (DPD) ( Network IKE Gateways Add )
was enabled and failed during IKE negotiations. 

 PAN-100256 

 Fixed and issue on a firewall where a Device
Group was selected, the App Scope Network Monitor report ( Monitor App Scope > Network Monitor ) failed to
display data. 

 PAN-100244 

 Fixed an issue where a failed commit or
...
eventually, caused the Network Processing Cards (NPCs) to restart. 
 ...
 Fixed an issue on Panorama M-Series and
virtual appliances where Push Scope Selection ( Commit Push to Devices ) selected firewalls
not in the hierarchy of the firewall you selected. 

 PAN-99966 

 Fixed an issue where Commit and Push ( Commit Commit and Push )
...
after a manual failover. 
 ...
 Fixed an issue where the destination NAT
...
address-and-port pair for only one connection. 

 See Limitations for PA-7000 Series firewalls 
...
signature displayed the following error message: Unauthorized request . 
 ...
 Fixed an issue where the Captive Portal
configured with RADIUS authentication failed when a username contained
the "at" ( @ ) character. 

 PAN-99085 

 Fixed an issue where firewalls did not purge
...
fail. 
 ...
 Fixed an intermittent issue on Panorama
M-Series and virtual appliances where GlobalProtect Gateway Configuration ( Network GlobalProtect Gateways Authentication )
...
entered credentials through the web interface. 
 ...
 Fixed an issue when you configured service
...
Ethernet (AE) subinterfaces returned incorrect values. 
 ...
 Fixed an issue where General Information ( Dashboard )
...
after you imported a configuration. 
 ...
 Fixed an issue where you could not set the
Captive Portal session timeout ( Device Setup Session )
...
to Panorama ) caused the firewalls to restart. 
 ...
 Fixed an issue Panorama M-Series and virtual
appliances with the characteristic Data Breaches ( Objects Application Filters )
enabled caused all Device Groups entries not to display. 

 PAN-97698 

 Fixed an issue where the firewall took longer
...
modification of attributes in a SAML Response packet. 
 ...
 Fixed an issue where the command delete report custom scope shared report-name <report name> file-name did
...
did not rotate error logs correctly, which caused disk space issues. 
 ...
 Fixed an intermittent issue where a null
pointer exception caused the configuration ( configd )
process to stop responding. 

 PAN-96440 

 Fixed an issue where the static route was
...
which caused GCP to time out before media availability was provided. 
 ...
 Fixed an issue where administrators with
...
custom role-based administrators made changes to Managed Collectors ( Panorama ). 
 ...
 Fixed an intermittent issue on a VM-Series
