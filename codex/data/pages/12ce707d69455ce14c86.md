---
url: https://docs.paloaltonetworks.com/ngfw/help/10-2/device/device-setup-session/tcp-settings
fetched_at: 2026-08-13T16:43:22Z
...
TCP Settings 
 ...
 Select this option if you want the firewall to forward segments that exceed the TCP out-of-order
 queue limit of 64 per session. If you disable this option, the
 firewall drops segments that exceed the out-of-order queue limit. To
 see a count of the number of segments that the firewall dropped as a
 result of disabling this option, run the following CLI command: 
...
in response to SYN 
 ...
 Enable this option to allow a response to
...
or out of sequence. 
 ...
 The TCP timestamp records when the segment
...
in connectivity issues. 
 ...
 Set globally whether to drop or bypass packets
that contain out-of-sync ACKs or out-of-window sequence numbers. 

 Drop —Drop packets that contain an asymmetric
path. 

 Bypass —Bypass scanning on packets that
contain an asymmetric path. 
...
process is called out-of-band processing. 
...
out-of-band processing ( not recommended ). 
 ...
 Illegal TCP segments without any flags set
...
no TCP flags, enabling this option may result in connectivity issues. 
 ...
 Enabled globally by default to convert (Multipath
TCP) MPTCP connections to standard TCP connections. 
...
in TCP Drop . 
 ...
 Select one of the following options to set
the cleartext proxy behavior for SIP TCP sessions when a segmented
SIP header is detected. 

 Always Off —Disables
the cleartext proxy. Disable the proxy when the SIP message size
is generally smaller than the MSS and when the SIP messages fit
within a single segment, or if you need to ensure TCP proxy resources
are reserved for SSL forward proxy or HTTP/2. 

 Always enabled —Default. Uses TCP proxy
for all SIP over TCP sessions to help with the correct reassembly and
ordering of TCP segments for proper ALG operation. 

 Automatically enable proxy when needed —When
selected, the cleartext proxy is automatically enabled for sessions
where the ALG detects SIP message fragmentation. Helps optimize
the proxy when it is also used for SSL forward proxy or HTTP/2. 
...
or later ) 
 ...
 If enabled, the checksum for the original
