---
url: https://docs.paloaltonetworks.com/prisma-browser/deployment/prisma-mobile-browser
...
Updated on 
 ...
 Superuser or Prisma Browser Roles 
 ...
 mTLS (Client Certificate Authentication) for Prisma Browser for Mobile 

 Mutual TLS (mTLS) authentication , also known as client certificate
 authentication, is supported on Prisma Browser for Mobile. This feature enables
 device-based authentication to enterprise web applications, web application
 firewalls (WAFs), next-generation firewalls (NGFWs), and internal services that
 require client certificate verification. 

 Prerequisites 

 The device must be enrolled in an MDM solution. 

 The MDM App Configuration must include the
 DEVICE_COMPLIANCE_LOGIN key for Prisma
 Access Browser for Mobile. 

 An administrator must deploy the client certificate to the device prior
 to use. 

 The client certificate must be stored in the device system keychain
 (installed via MDM). 

 For desktop and browser
 extension deployments, use the Automatic Client Certificate
 Selection setting to configure URL-based certificate
 matching. This setting does not apply to mobile deployments, where
 certificates are managed at the device level. 

 Android 

 Prisma Browser for Mobile on Android supports mTLS for all browser web
 traffic. Administrators must pre-install client certificates on devices using a
 mobile device management (MDM) solution, requiring a managed device
 deployment. 

 iOS 

 Prisma Browser for Mobile on iOS supports mTLS exclusively for the browser
 login authentication flow. Client certificates must be pre-installed on the
 device. mTLS authentication for other enterprise applications accessed within
 the browser is currently unsupported on iOS. 
 ...
 For more information on the available Mobile Browser attributes, refer to Configure Prisma Browser Mobile Device
 Posture Attributes . 
 ...
 articles: 
 File Download 
 The following File Download controls operate
 differently in the Prisma Browser for Mobile: 
 ...
 File Upload 
 The following File Upload controls operate differently in
 ...
 The Print control can also be used to
 manage File Downloads by printing to a PDF. 

 Screenshot 
 The following screenshot control operates differently in
 the Prisma Browser : 
 ...
 enforcement point (EP) with SSL decryption enabled can cause HSTS errors
 on iOS. To resolve this, choose one of the following configurations: 
 Route traffic without decryption 

 Enable decryption without routing all traffic (by excluding
 mobile devices). 
 SSL decryption is generally
 unnecessary, as PAB Mobile provides native network control
 and CDSS. 

 Routing all traffic
 through Prisma Access while SSL decryption is enabled is
 not supported. 
