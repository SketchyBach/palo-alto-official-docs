---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-vmware-nsx-avi
fetched_at: 2026-09-16T07:25:21Z
source: palo-alto-main
---

# Provision to VMware NSX Advanced Load Balancer (AVI) Clear
...
Updated on 
 ...
 Use this procedure to provision a certificate from Next-Gen Trust Security to a VMware NSX Advanced Load Balancer (AVI) machine. 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Installations > Machines . 

 Select the VMware NSX ALB (AVI) machine you want to provision a certificate to. 
 ...
 In Certificate Name , enter the name you want the certificate to use on the VMware NSX ALB (AVI). 

 What if the certificate name is already in use? 

 When provisioning a certificate, Next-Gen Trust Security checks whether the name already exists on the VMware NSX ALB (AVI): 
 ...
 (Optional) In Tenant , enter the name of the tenant that owns the virtual service. 

 Note: If the virtual service belongs to the default admin tenant, this field can be left empty. 

 In Virtual Service , enter the name of the virtual service to associate with the certificate. 

 (Optional) To prevent the certificate from being pushed immediately, set Push upon saving to No . 

 Click Save . 

 After saving, Next-Gen Trust Security provisions the certificate to the specified VMware NSX ALB (AVI) tenant and updates the virtual service. An installation record is created on the Installations tab. 
