---
url: https://cortex-docs.paloaltonetworks.com/cortex-cloud-runtime-security/endpoint-security/install-and-manage-endpoints/set-up-endpoint-protection/configure-global-agent-settings
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 From Cortex Cloud, select Settings → Configurations → General → Agent Configurations. 
 ...
 Note: Disabling this capability takes immediate effect on new hashes, fresh agent installations, and existing security policies. It could take up to a week to take effect on existing agents in your environment pending agent caching. 
 ...
 Note: If you have any Cortex Cloud filters, starring policies, exclusion policies, scoring rules, log forwarding queries, or automation rules configured for XSOAR/3rd party SIEM, we advise you to update those to support the changes before activating the feature. For example, change the query to include the previous description that is still available in the new description, instead of searching for an exact match. 
