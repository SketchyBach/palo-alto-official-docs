---
url: https://docs.prismacloud.io/admin-guide/33/runtime-defense/runtime-defense-containers
fetched_at: 2026-09-16T13:37:03Z
source: prisma-cloud
---

# Runtime defense for containers | 33 | Prisma Cloud arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Processes started from modified binaries  — Detect when a modified process was spawned. A modified process indicates that binaries from a container image were modified and executed after the container was started. You can Enable and Disable this detection using the Processes started from modified binaries option under the Runtime rule > Processes tab. Once enabled, Defender takes action on this type of incident according to the configured effect . 

 Crypto miners  — Detect when a crypto miner process was spawned. If detected, a crypto miner incident type is created in Incident Explorer. You can Enable and Disable this detection using the Crypto miners option under the Runtime rule > Processes tab. Once enabled, Defender takes action on this type of incident according to the configured effect . 

 Reverse shell attacks  — Detect that a process was identified as running a reverse shell, which is a method used by attackers for gaining access to a victim’s system. If detected, a Reverse Shell Incident type is created in Incident Explorer. You can Enable and Disable this detection using the Reverse shell attacks option under the Runtime rule > Processes tab. Once enabled, Defender takes action on this type of incident according to the configured effect . 

 Processes used for lateral movement  — Detect when a process, such as netcat, known to facilitate lateral movement between resources on a network was spawned. If detected, a lateral movement incident type is created in Incident Explorer. You can Enable and Disable this detection using the Processes used for lateral movement option under the Runtime rule > Processes tab. Once enabled, Defender takes action on this type of incident according to the configured effect . 

 Processes started with SUID  — Detect suspicious privilege escalation by watching for binaries with the setuid bit that are executed. You can Enable and Disable this detection using the Processes started with SUID option under the Runtime rule > Processes tab. Once enabled, Defender takes action on this type of incident according to the configured effect . 
 ...
 Previous Runtime defense 

 Next Runtime defense for hosts 
