---
url: https://cortex-docs.paloaltonetworks.com/data-security-documentation/data-management/broker-vm/set-up-and-configure-broker-vm
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 When the Broker VM is collecting data, the optimal outgoing bandwidth into the Cortex Data Security server should be about 25% of the incoming data traffic into the Broker VM applets. 

 Important 

 There can be instances in which the Broker VM requires up to 50% of the incoming bandwidth as outgoing. Such instances can be, network instability between the Broker VM and Cortex Data Security, or data that is being collected, but not well compressed. 
 ...
 br- <XDR tenant> .xdr.<region>.paloaltonetworks.com HTTPS over TCP port 443 
 ...
 Information needed to communicate with your Cortex Data Security tenant. Used by tenants deployed in all regions. 

 br- <xdr-tenant> .xdr.federal.paloaltonetworks.com HTTPS over TCP port 443 
 ...
 Enable access to Cortex Data Security 

 Enable access to Cortex Data Security from the Broker VM to allow communication between agents and collectors and Cortex Data Security. The Broker VM communicates with the Cortex Data Security tenant with TLS 1.2 (or higher, if that applies). 

 For more information on enabling access to Cortex Data Security, see Enable access to required PANW resources . 
 ...
 For Broker VM version 9.0 and earlier, Cortex Data Security will only accept 172.17.0.0/16 . 
...
For example: ssh -i [/path/to/private.key] admin@[broker_vm_address] 
 ...
 (Optional) Update the SSL Server certificates for the Broker VM. Upload your signed server certificate and key to establish a validated secure SSL connection between your endpoints and the Broker VM. Ensure the Private Key is uploaded in an unencrypted format. When you configure the server certificate and the key files in the Broker VM, Cortex Data Security automatically updates them in the tenant UI. Cortex Data Security validates that the certificate and key match, but does not validate the Certificate Authority (CA). 
 ...
 If adding a CA certificate to the Broker VM is not possible, ensure that you’ve added the Broker Service FQDNs to the SSL Decryption Exclusion list on your firewalls. See Enable Access to Cortex Data Security . 
 ...
 (Optional) Collect and Generate New Logs (Requires Broker VM 8.0 and later). Your Cortex Data Security logs will download automatically after approximately 30 seconds. 
 ...
 After a successful registration, Cortex Data Security displays a notification. 
 ...
 Previous What is the Broker VM? 

 Next Broker VM image installations 
