...
# Known Issues Related to PAN-OS 8.1 Releases Clear
...
PAN-OS ® 8.1 releases, which includes known issues specific
to Panorama™ and GlobalProtect™, as well as known issues that apply
more generally or that are not identified by an issue ID. See also the Known Issues Specific to the WF-500 Appliance . 
...
informational system log. 
 ...
 To check for a software integrity
check failure, select Monitor > Logs and
enter the filter: (severity eq critical) and (eventid eq fips-selftest-integ) . 
...
check. 
 ...
 If you configure GlobalProtect portals and
...
Admin console and select Device management Chrome management User settings . 
...
firewalls. 
 ...
 Upon upgrade to PAN-OS 8.1.25, the following GlobalProtect settings
 do not work: 
 ...
 After successful upgrade to PAN-OS 8.1.23, ACC 
 widgets display as In Progress and no
 data is loaded. 

 PAN-177363 

 Dedicated Log Collector system and config
...
on Panorama in Management Only mode. 
 ...
 On the Panorama management server, local
or Dedicated Log Collector mode cannot successfully join an ElasticSearch
cluster when added to a Collector Group ( Panorama Collector Groups ) if the SSH
key length for a Log Collector in the cluster is greater than 2048 characters. 
...
unable to configure a master key ( Device Master Key and Diagnostics )
for a managed firewall if an interface ( Network Interfaces Ethernet ) references
a zone pushed from Panorama. 
...
every 5 minutes. 
 ...
 On the Panorama management server in a high
availability (HA) configuration, content updates ( Panorama Dynamic Updates ) manually
uploaded to the active HA peer are not synchronized to the passive
HA peer when you Install a content update
and enable Sync to HA Peer . 

 PAN-161955 

 Firewalls erroneously generate a high severity
system log ( Monitor Logs System ) when the firewall connects
to a syslog server. 

 PAN-160410 

 In the ACC , data
cannot be imported or exported when a User filter ( ACC Network Activity Set Tab Filters User )
that contains characters not supported by URL format, such as DOMAIN/USER ,
is applies to the Network Activity widget. 

 PAN-157240 

 When a firewall has hardware offloading
...
get stuck in Exchange Start state. 
...
the next commit. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.17 Addressed Issues . 

 On the Panorama management server, you cannot
context switch to the web interface of a managed firewall running
PAN-OS 8.1.16. 

 Workaround: Downgrade the managed firewall
to PAN-OS 8.1.15 or earlier release. 

 PAN-151909 

 On the Panorama management server, Preview
Changes ( Commit Commit to Panorama ) incorrectly
displays an existing route as Added and the new route as an existing
route in the Candidate Configuration when you configure a new virtual
router route ( Network Virtual
Router ) 

 PAN-150172 

 This issue is
now resolved. See PAN-OS 8.1.15-h3 Addressed Issues . 
...
with forward proxy enabled. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.15 Addressed Issues 

 SNMP objects from the HOST-RESOURCES-MIB
...
a delay in a log query and ingestion. 
 ...
 There is an issue with Online Certificate
...
( reportd ) to Cortex Data Lake due to G1 issuer certification. 
 ...
 Pushing a configuration from a Panorama
...
8.1 produces a HTTP/2 warning. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.13 Addressed Issues 

 ( PA-7000 Series firewalls running PAN-OS
...
check for duplicate addresses in address groups ( Objects Address Groups )
and duplicate services in service groups ( Objects Service Groups ) when created
from the CLI. 

 PAN-131915 

 There is an issue when you implement a new
firewall bootstrap with a USB drive where the bootstrap fails and
displays the following error message: no USB device found . 
...
now resolved. See PAN-OS 8.1.15 Addressed Issues 
 ...
 The Name log filter ( Monitor Logs Traffic )
is not maintained when viewing the Log Viewer for a Security policy
rule ( Policies Security )
from the drop-down menu. 

 PAN-130630 

 ( VM-Series firewalls in Azure Load Balancer
...
now resolved. See PAN-OS 8.1.13 Addressed Issues 
 ...
 There is an issue where the firewall incorrectly
...
due to an incompatible driver. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.12 Addressed Issues 
...
firewall processes corrupt packets. 
 ...
 There is an issue where VM-Series firewalls
...
now resolved. See PAN-OS 8.1.12 Addressed Issues 
 ...
 There is an issue on Panorama M-Series and
...
1.4. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.11 Addressed Issues 

 ( PA-7000 Series firewalls using PA-7000-20G-NPC
...
to stop responding. 
 ...
 There is an issue on M-500 Panorama management
...
in to the firewall CLI. 
 ...
 Restart the devsrvr process: debug software restartprocess device-server 
...
Panorama disables the 1/1 Ethernet interface in the Panorama configuration
as expected but the interface still displays as Up after
...
change. 
 ...
 This issue
is now resolved. See PAN-OS 8.1.10 Addressed Issues . 
...
inspection issues related to the impacted decoder(s). 
 ...
 This issue is
now resolved. See PAN-OS 8.1.9 Addressed Issues 

 There is an issue where the firewall incorrectly
displays application dependency warnings ( Policies Security ) after you initiate
a commit. 

 PAN-116436 

 This issue is
now resolved. See PAN-OS 8.1.8 Addressed Issues ; see
PAN-94475. 
...
and causes a process ( CDB ) to stop responding. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.8 Addressed Issues . 

 ( PAN-OS 8.1.7 only ) A VM-Series
...
now resolved. See PAN-OS 8.1.9 Addressed Issues . 
 ...
 ( PA-200 firewalls only ) There is
a rare out-of-memory (OOM) condition. 

 PAN-111456 

 The SCTP service object does not function
...
now resolved. See PAN-OS 8.1.9 Addressed Issues . 
 ...
 There is an issue with a memory leak associated
...
restart of the configuration ( configd ) process. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.10 Addressed Issues . 

 The Panorama management server returns a
Secure Copy (SCP) server connection error after you create an SCP
Scheduled Config Export profile ( Panorama Scheduled Config Export ). This
issue occurs when the SCP server password exceeds 15 characters
in length. 
...
drop H.245 traffic. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.7 Addressed Issues . 

 If you use Panorama running PAN-OS 8.1.6
...
and Panorama will stop displaying device groups. 
...
followed by a Push to Devices operation for
the modified device group and template configurations. 
...
now resolved. See PAN-OS 8.1.9 Addressed Issues . 
 ...
 On the Panorama management server, the Include
Device and Network Templates setting is disabled by
default when you attempt to push changes to managed devices, which
causes your push to fail. 

 Workaround: Before you commit
and push the configuration changes from Panorama to your managed
devices, edit the push scope ( Commit Push to Devices Edit Selections or Commit Commit and Push Edit Selections ) to Include
...
now resolved. See PAN-OS 8.1.6 Addressed Issues . 
 ...
 ( HA configurations only ) The dataplane
...
PAN-OS 8.0.14 or PAN-OS 8.1.5. 

 Workaround: Temporarily
modify the IKE phase 2 lifetime for both peers ( Network Network Profiles IPsec Crypto ) to increase the
interval between rekey events (default is one hour) to avoid a rekey
event before you complete the upgrade on the second peer. Alternatively,
remove the HA configuration, upgrade both firewalls, and then restore
the HA configuration. 

 PAN-109526 

 The system log does not display the URL
for CRL files correctly, the URLs are displayed with encoded characters. 

 PAN-108805 

 This issue is
now resolved. See PAN-OS 8.1.5 Addressed Issues . 
...
(default). 
 ...
 Workaround: In PAN-OS 8.1.5, you can use
the following CLI commands to switch to software-based DFA scanning: 

 set system setting dfa-mode [hw-dfa|sw-dfa] —Switch
between DFA scanning options (persistent across restarts and does
not require a reboot). 

 set system setting dfa-mode-default —Restore
the default DFA setting. 

 show system setting dfa-mode —Show
the current DFA scanning configuration. 
...
now resolved. See PAN-OS 8.1.6 Addressed Issues . 
 ...
 Memory issues on Palo Alto Networks hardware
and virtual appliances cause intermittent management plane instability. 

 PAN-107636 

 This
issue is now resolved. See PAN-OS 8.1.7 Addressed Issues . 
...
now resolved. See PAN-OS 8.1.5 Addressed Issues . 
 ...
 ( PAN-OS 8.1.4 only ) Firewalls fail
...
a DH group other than group1 . 
 ...
 This issue is
now resolved. See PAN-OS 8.1.4-h2 Addressed Issues . 

 ( PA-3200 Series firewalls running PAN-OS
...
8.1.4. 
 ...
 There is a display-only issue on Panorama
that results in a commit failed status
for Template Last Commit State ( Panorama Managed Devices Summary ). 
...
on PA-5200 Series firewalls in an HA configuration. 
...
interfaces. 
 ...
 ( Panorama in FIPS mode only when managing
...
now resolved. See PAN-OS 8.1.10 Addressed Issues . 
 ...
 There is an issue where scheduled SaaS reports
generate and email empty PDF reports. 
...
after a few hours of uptime. 
 ...
 Adding a disk to a Panorama 8.1 virtual
...
group as expected. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.4 Addressed Issues . 

 Extended Authentication (X-Auth) clients
...
now resolved. See PAN-OS 8.1.5 Addressed Issues . 
 ...
 The Panorama Controller does not display
all commit-all jobs for Panorama Nodes
( Panorama Interconnect Tasks ) and the Panorama Controller
does not push those missing jobs when you Push to Devices if the
associated Panorama Node is running a PAN-OS 8.1 release. 
...
a Device Group. 
 ...
 Workaround: Log in to the CLI on the
...
mappings: debug object registered-ip clear all . 
 ...
 An invalid public key is intermittently
applied to the administrator account when deploying a VM-Series
firewall in Google Cloud using the Google web interface. 
...
now resolved. See PAN-OS 8.1.5 Addressed Issues . 
 ...
 There is a rare issue where a failed commit
...
causes the firewall to drop traffic. 
...
and manually refresh the FQDN list. 
 ...
 ( PAN-OS 8.1.3 and later PAN-OS 8.1 releases
...
a partial configuration load ( Panorama Setup Operations ). 
 ...
 This
issue is now resolved. See PAN-OS 8.1.5 Addressed Issues . 
...
(the one that owns the predict session of GRE). 
 ...
 ( HA configurations running PAN-OS 8.0.9
...
expected between HA peers. 

 Workaround: Re- Enable
Config Sync ( Device High Availability General Setup settings ). 

 PAN-98735 

 This issue is
now resolved. See PAN-OS 8.1.3 Addressed Issues . 

 Upgrading a Panorama management server on
Microsoft Azure from PAN-OS 8.1.0 to PAN-OS 8.1.1 or PAN-OS 8.1.2
results in an autocommit failure. 
...
and, when finished, import your exported configuration. 
...
then import and reload the exported configuration. 
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 Panorama appliances running PAN-OS 8.1.2
cannot connect to the Logging Service: 

 When you deploy
a Panorama 8.1.2 virtual appliance, Panorama is unable to connect
to the Logging Service and firewalls are unable to forward logs
to the Logging Service. 

 If you upgrade a Panorama virtual appliance with Logging
Service enabled to PAN-OS 8.1.2, both Panorama and the firewalls
will continue to connect to the Logging Service but will not display
information about Logging Services instances when you run the request logging-service-forwarding customer info fetch CLI command. 
...
on KVM from the Virtual-manager console or virsch CLI. 
 ...
 The GlobalProtect gateway ignores the Enable
X-Auth Support setting when you enable or disable it
through the firewall web interface ( Network GlobalProtect Gateways <gateway> Agent Tunnel Settings ). 

 Workaround: Enable
or disable X-Auth support by running the set network tunnel global-protect-gateway <gateway> ipsec third-party-client rekey-noauth {yes| no} configuration
mode CLI command. 

 PAN-96734 

 This issue is
now resolved. See PAN-OS 8.1.2 Addressed Issues . 

 The configuration daemon ( configd )
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 PA-7000 Series and PA-5200 Series firewalls
...
receivers. 
 ...
 Workaround: On the firewall, commit a configuration
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 After end users successfully authenticate
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 In a deployment where the firewall connects
...
to the BGP peer. 
 ...
 Workaround: Move the next hop to the
...
now resolved. See PAN-OS 8.1.4 Addressed Issues . 
 ...
 Firewalls in an HA active/active configuration
...
See PAN-OS 8.1.2 Addressed Issues . 

 Firewalls that collect port-to-username
...
even after you clear the User-ID cache. 
 ...
 On VM-Series firewalls that have Data Plane
...
times out and fails. 
 ...
 In a deployment where a Log Collector connects
...
requires the VMware NSX 2.0.4 or later plugin. 

 On the Panorama management server, selecting
...
requires the VMware NSX 2.0.4 or later plugin. 
 ...
 VM-Series firewalls for NSX and firewalls
in an NSX notify group ( Panorama VMware NSX Notify Group )
briefly drop traffic while receiving dynamic address updates after
the primary Panorama in an HA configuration fails over. 

 PAN-95443 
 This issue is now resolved.
See PAN-OS 8.1.2 Addressed Issues . 

 A VM-Series firewall on KVM in DPDK mode
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 Mobile endpoints that use GPRS Tunneling
...
change the passwords to apply password profile settings.) 
 ...
 After you delete disconnected and connected
...
command) for the disconnected TS agents you deleted ( Device User Identification Terminal Services Agents ). 
...
operation. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 On Panorama Log Collectors, the show system masterkey-properties CLI
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 A firewall receiving IP addresses via DHCP
...
Filtering profile that specifies a Continue action ( Objects Security Profiles URL Filtering ) because the
firewall does not treat the API keys as binary strings. 
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 The firewall records GPRS Tunneling Protocol
(GTP) packets multiple times in firewall-stage packet captures (PCAPs). 

 PAN-94402 

 Upgrading firewalls from PAN-OS 8.0 to 8.1
causes the loss of user mapping information and therefore disrupts
user-based policies in the following HA configurations: 

 Active/active (in
this example, the primary/secondary peers are firewall1/firewall2)—During
the period after you upgrade firewall1 to PAN-OS 8.1 but before
you upgrade firewall2, firewall1 loses user mapping information.
When you finish upgrading both firewalls to PAN-OS 8.1, HA synchronization
restores the lost mapping information on firewall1. 

 Active/passive (in this example, the active/passive
peers are firewall1/firewall2)—After you upgrade firewall2 to PAN-OS
8.1 but before you upgrade firewall1, firewall2 loses user mapping
information but does not enforce policies because it is still in
a passive state. However, after you trigger failover by suspending
firewall1 (in anticipation of upgrading it), firewall2 becomes the
active peer and fails to enforce user-based policies because its
mapping information is still missing. After you then upgrade firewall1
and trigger failback, firewall1 resumes enforcing policy and HA
synchronization ensures the mapping information is complete on both
firewalls. 
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 On the Panorama management server, the Task
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 A Panorama Collector Group forwards Threat
...
now resolved. See PAN-OS 8.1.4 Addressed Issues . 
 ...
 When the file-forwarding queue limit is
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 The firewall does not apply tag-based matching
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 Firewalls randomly retain IP address-to-username
...
that the mapping was deleted or expired. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.1 Addressed Issues . 

 Device monitoring does not work on the Panorama
management server. 

 Workaround: To enable Panorama to
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 The request system external-list show type ip name <EDL_name> CLI
command does not display external dynamic list entries after you
restart the management server ( mgmtsrvr ) process. 
...
minimum PAN-OS release version for a threat signature. 
 ...
 This issue is
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 The management server process ( mgmtsrvr )
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 When you enable SSL decryption on a firewall,
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 The Panorama management server generates
...
forwarding to a syslog server ( Panorama Server Profiles Syslog )
and commit configuration changes ( Commit Commit to Panorama ). 
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 The GlobalProtect agent can't split tunnel
...
connection ( Network GlobalProtect Gateways <gateway-config> Agent Client Settings <client-setting-config> Split Tunnel Domain and Application ). 
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 The password field does not display in the
GlobalProtect portal login dialog if you attach the certificate
profile to the portal configuration. 
...
username field to None in the certificate profile. 
 ...
 The logging status of a Panorama
...
log collection ( Panorama Managed Collectors Interfaces ). This results in
...
the management (MGT) interface for log collection. 
 ...
 This
issue is now resolved. See PAN-OS 8.1.2 Addressed Issues . 

 SSL decrypted traffic fails
...
Based Forwarding (PBF) policy rules ( Policies Policy Based Forwarding ). 
 ...
 This
issue is now resolved. See PAN-OS 8.1.3 Addressed Issues . 

 High log rates cause disk space
...
This will cause the commit to fail because a local Log Collector
is not supported on a Panorama management sever in Management Only
mode. 

 PAN-93640 

 This
issue is now resolved. See PAN-OS 8.1.1 Addressed Issues . 

 On firewalls, the Log Collector
...
Group. 
 ...
 Workaround: Configure the management (MGT) interface
for log collection. 
...
( Objects Security
Profile Groups ), the Security Profile
Group doesn’t list the SCTP Protection profile in its drop-down
...
issue is now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 On firewalls in an HA configuration,
...
issue is now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 PA-5200 Series firewalls send
logs to the passive or suspended Panorama virtual appliance in Legacy
mode in an HA configuration. 

 Workaround: On the active
Panorama, run the request log-fwd-ctrl device <firewall_serial_number> action start CLI
command, where <firewall_serial_number> is
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 Firewall CPU usage reaches 100 per cent
due to SNMP polling for logical interfaces based on updates to the
Link Layer Discovery Protocol (LLDP) MIB (LLDP-V2-MIB.my). 
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 PA-7000 Series firewalls cause slow traffic
...
TCP segments during IPSec encryption. 
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 The firewall reports the incorrect hostname
...
on the firewall. 
 ...
 This
issue is now resolved. See PAN-OS 8.1.16 Addressed Issues . 
...
issue is now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 When configuring a Google Cloud
...
request. This delay causes DHCP IP assignments to fail. 
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 The firewall generates System logs with
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 ( VM-50 Lite firewalls only ) There
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 On Panorama management servers in an HA
...
and firewalls. 
 ...
 Workaround: Manually deploy the dynamic
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 A Panorama Collector Group does not forward
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 A small percentage of writable third-party
...
download and install a maintenance release with this fix. 
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 Enabling jumbo frames ( Device Setup Session )
reduces throughput because: 

 The firewalls hardcode
the maximum segment size (TCP MSS) within TCP SYN packets and in
server-to-client traffic at 1,460 bytes when packets exceed that
size. 

 PA-7000 Series and PA-5200 Series firewalls hardcode the
maximum transmission unit (MTU) at 1,500 bytes for the encapsulation
stage when tunneled clear-text traffic and the originating tunnel
session reside on different dataplanes. 
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 PA-5200 Series firewalls in an active/passive
...
reboot it. 
 ...
 Workaround: On the passive firewall, set
...
configure a log forwarding profile for correlated events. 
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 Firewalls in an active/passive HA configuration
...
and Border Gateway Protocol (BGP). 
 ...
 This
issue is now resolved. See PAN-OS 8.1.8 Addressed Issues . 

 You cannot configure an IP
address using templates for HA2 ( Device High Availability Data Link (HA2) )
when set to IP or Ethernet for Panorama management servers in an
HA configuration. 

 Workaround: Configure HA2 in the
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 The firewall web interface displays a blank Device Licenses page when
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 On PA-3250 and PA-3260 firewalls, the hardware
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 Panorama Log Collectors do not receive some
firewall logs and take longer than expected to receive all logs
when the Collector Group has spaces in its name. 
...
issue is now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 Log Collectors that belong
to a collector group with a space in its name fail to fully connect
to one another, which affects log visibility and logging performance. 
...
firewalls ( Panorama Managed Devices Health ). This results in some
session statistics being displayed as 0. 
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 End users cannot authenticate to GlobalProtect
...
issue is now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 The firewall dataplane restarts
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 The firewall drops IPv6 traffic while enforcing
...
directly attached network. 
 ...
 Workaround: Above the bidirectional
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 An Aggregate Ethernet (AE) interface with
...
switch LACP peer reboots and comes back up. 
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 On PA-7000 Series firewalls in an HA configuration,
the HA3 link does not come up after you upgrade to PAN-OS 8.0.6
or a later release. 

 Workaround: Unplug and replug the
HSCI modules. 
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 On a PA-5000 Series firewall configured
to use an IPSec tunnel containing multiple proxy IDs ( Network IPSec Tunnels <tunnel> Proxy
IDs ), the firewall drops tunneled traffic
after clear text sessions are established on a dataplane other than
the first dataplane (DP0). 

 Workaround: Use Palo Alto
...
command). 
 ...
 The firewall generates false positives during
...
now resolved. See PAN-OS 8.1.6 Addressed Issues . 
 ...
 Threat logs record incorrect IMSI values
...
now resolved. See PAN-OS 8.1.2 Addressed Issues . 
 ...
 ( PA-3050, PA-3060, PA-5000 Series, PA-5200
...
HA failover on firewalls with hardware offloading enabled (default). 
...
now resolved. See PAN-OS 8.1.4 Addressed Issues . 
 ...
 On PA-3200 Series firewalls, Ethernet ports
...
ports 1, 5, 9, 11, and 12 function at 10Mbps, 100Mbps, or 1,000Mbps.) 
 ...
 This
issue is now resolved. See PAN-OS 8.1.9 Addressed Issues . 

 When you configure a PA-5220
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 VM-Series firewalls stop displaying URL
...
Profiles URL Filtering ). 
 ...
 This issue is
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 After receiving machine account names in UPN
...
now resolved. See PAN-OS 8.1.9 Addressed Issues . 
 ...
 The firewall stops enforcing policy after
you manually refresh an External Dynamic List (EDL) that has an
invalid IP address or that resides on an unreachable web server. 
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 A VM-Series firewall on KVM in MMAP mode
doesn't receive traffic after you configure it to use the i40e single-root
input/output virtualization (SR-IOV) virtual function (VF). 

 PAN-87990 

 The WF-500 appliance becomes inaccessible
...
now resolved. See PAN-OS 8.1.1 Addressed Issues . 
 ...
 When you configure a GlobalProtect gateway
...
firewall Security policy. 
 ...
 Logs are temporarily unavailable on Panorama
Log Collectors because the vldmgr process restarts. 

 PAN-86903 

 In rare cases, PA-800 Series firewalls shut
...
7 processing is completed if asymmetric routing is involved. 
 ...
 Authentication policy rules based on multi-factor
...
now resolved. See PAN-OS 8.1.7 Addressed Issues . 
 ...
 When you disable decryption for HTTPS traffic,
end users who don't have valid authentication timestamps can access
HTTPS services and applications regardless of Authentication policy. 
...
now resolved. See PAN-OS 8.1.4 Addressed Issues . 
 ...
 After you disable the Skip Auth
...
command. 
 ...
 VM-Series firewalls cannot monitor more
...
( Network GlobalProtect Gateways <gateway> General ) even after you enable
IPv6: Warning: tunnel tunnel ipv6 is not enabled. IPv6 address will be ignored! 

 PAN-82987 

 This
issue is now resolved. See PAN-OS 8.1.4 Addressed Issues . 

 The web interface intermittently
becomes unresponsive during ACC queries. 

 PAN-82278 

 Filtering does not work for Threat logs
...
the FQDN with the IP address in the Kerberos server profile. 
 ...
 Panorama cannot push address group objects
from device groups to managed firewalls when zones specify the objects
in the User Identification ACL include or exclude lists ( Network Zones )
...
now resolved. See PAN-OS 8.1.3 Addressed Issues . 
 ...
 An intermittent issue occurs with ZIP hardware
...
Protocol (SMTP). 
 ...
 There is an issue where HIP-related
...
or earlier release. 
 ...
 Workaround: Ensure all firewalls
...
sessions remain open until they time out. 
...
cluster. (In a two-node cluster, both nodes are controller nodes
...
workarounds to sync the controller nodes: 
 ...
 After you
import the two-node cluster into Panorama, push the configuration
from Panorama to the cluster. After the push succeeds, Panorama
reports that the controller nodes are in sync. 
...
workaround. 
 ...
 Configure service advertisement on the local CLI of the cluster controller
and then import the configuration into Panorama. The service advertisement
can advertise that DNS is or is not enabled. 
...
dns-service enabled no 
 ...
 Both commands result
in Panorama reporting that the controller nodes are in sync. 

 PAN-72861 
 When you configure a PA-7000 Series or PA-5200
Series firewall to perform tunnel-in-tunnel inspection, which includes
GRE keep-alive packets ( Policies Tunnel Inspection <tunnel_inspection_rule> Inspection Inspect Options ), and
you run the clear session all CLI command
while traffic is traversing a tunnel, the firewall temporarily drops
tunneled packets. 

 PAN-71765 

 Deactivating a VM-Series firewall from Panorama
completes successfully but the web interface does not update to
indicate that deactivation finished. 
...
( Network GlobalProtect Portals <portal> Clientless VPN Applications ). 
...
logged out from the PAN-OS web interface. 
...
access the GlobalProtect portal. 
 ...
 Authentication using auto-filled credentials
...
dynamic list server and returns a URL access error ( Objects External Dynamic Lists ). 
 ...
 Entering vSphere maintenance mode on a VM-Series
...
of the VM-Series firewall. 
 ...
 The automatic license deactivation workflow
for firewalls with direct internet access does not work. 

 Workaround: Use
the request license deactivate key features <name> modemanual CLI
command to Deactivate a Feature License_or_Subscription_Using_the_CLI .
To Deactivate a VM-Series firewall , choose Complete
Manually (instead of Continue )
and follow the steps to manually deactivate the VM. 

 PAN-55825 
 Performing an AutoFocus remote search that
...
quotation mark. 
 ...
 HA for VM-Series firewalls does not work
...
and Korea (Seoul). 
 ...
 When you change the reporting period for
a scheduled report, such as the SaaS Application Usage PDF report,
the report can have incomplete or no data for the reporting period. 
...
the time period on an existing report. 
 ...
 In Traffic logs, the following session end
...
or decrypt-error. 
 ...
 On the VM-Series for NSX firewall, when
...
and Push ). To successfully redirect traffic
to the VM-Series for NSX firewall, you must push both device group
and template configurations when you modify the zone configuration
to ensure that the zones are available on the firewall. 

 PAN-53663 

 When you open the SaaS Application Usage
report ( Monitor PDF Reports SaaS Application Usage ) on
multiple tabs in a browser, each for a different virtual system
(vsys), and you then attempt to export PDFs from each tab, only
the first request is accurate; all successive attempts result in
...
before initiating the next export request. 
 ...
 On the NSX Manager, when you unbind an NSX
...
server but are not sent to the VM-Series firewalls. 
...
Service Manager and select NSX
Config-Sync ). 

 PAN-51952 

 If a security group overlap occurs in an
...
does not always take precedence over a policy with a lower weight. 
...
the wrong service profile (VM-Series firewall). 
 ...
 When using the CLI to configure the management
...
and send-hostname . 
 ...
 PAN-51869 
 Canceling pending commits does not immediately
remove them from the commit queue. The commits remain in the queue
until PAN-OS dequeues them. 

 PAN-51673 

 BFD sessions are not established between
two RIP peers when there are no RIP advertisements. 
...
peer. 
 ...
 The NSX Manager fails to redirect traffic
...
and results in the following error: 
 ...
 PAN-51122 
 For the VM-Series firewall, after you manually
...
is not supported when configured on a shared gateway. 
 ...
 When you use a Mac OS Safari browser, client
certificates will not work for Captive Portal authentication. 
...
in Shared and the sequence with the same name is in a virtual system. 
...
gateway) to ensure uniqueness. 
 ...
 Vulnerability detection of SSLv3 fails when
...
ingress interface for third-party IPSec traffic. 
 ...
 PAN-40130 
 In the WildFire Submissions logs, the email
...
for PCI pass-through functionality. 
 ...
 PAN-40075 
 The VM-Series firewall on KVM running on Ubuntu
12.04 LTS does not support PCI pass-through functionality. 

 PAN-39728 
 The URL logging rate is reduced after you enable
...
you configure the custom report. 
 ...
 The firewall does not clear unused NAT IP
...
command. 
 ...
 PAN-37511 
 Due to a limitation related to the Ethernet
...
to Panorama ) to ensure that Panorama recognizes
...
not display in Panorama Managed
Devices . 

 Furthermore, when Panorama
...
configuration between the Panorama HA peers. 
 ...
 When deleting the VM-Series deployment,
all VMs are deleted successfully; however, sometimes a few instances
still remain in the datastore. 
...
the Security Policy on the NSX Manager. 
 ...
 The VM-Series firewall fails to deploy and
displays the following error message: Invalid OVF Format in Agent Configuration. 
...
the vCenter Server: /etc/init.d/vmware-vpxd tomcat-restart . 
 ...
 When HA failover occurs on Panorama at the
time that the NSX Manager is deploying the VM-Series NSX edition
firewall, the licensing process fails with the following error: vm-cfg:failed to process registration from svm device. vm-state: active . 
...
sessions are secured properly. 
 ...
 PAN-36393 
 When deploying the VM-Series firewall, the
...
message if the VM-Series firewall is successfully deployed. 
 ...
 PAN-36088 
 When an ESXi host is rebooted or shut down,
...
the functional state of the guests that are unavailable. 
 ...
 PAN-36049 
 The VMware vCenter Server/vmtools displays
...
as Ubuntu. 
 ...
 When you edit a traffic introspection rule
...
when you remove the destination (TCP or UDP) port. 
...
the rule and add a new one. 
 ...
 When defining traffic introspection rules
(to steer traffic to the VM-Series firewall) on the NSX Manager,
...
traffic. 
 ...
 Duplicate packets are steered to the VM-Series
...
mode. 
 ...
 On a VM-Series NSX edition firewall, when
...
dynamic update of the added or removed Security Group. 

 Workaround: Select Panorama VMware Service Manager ,
and Synchronize Dynamic Objects to initiate
a manual synchronization to get the latest update. 

 PAN-34855 
 On a VM-Series NSX edition firewall, Dynamic
...
are blocked for 20 seconds. 
 ...
 Firewalls store SSH host keys used for SCP
...
When a failover occurs, the SCP log export fails. 

 Workaround: Log
in to each peer in HA, select Device Scheduled Log Export <log_export_configuration> ,
and Test SCP server connection to confirm
the host key so that SCP log forwarding continues to work after
a failover. 

 PAN-23732 
 After you use a Panorama template to push a
log export schedule that specifies an SCP server as the destination
( Device Scheduled
Log Export ), you must log in to each firewall
that receives the schedule and Test SCP server connection .
