---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-aiops/panorama-cloudconnector-plugin-2-1-0.html
fetched_at: 2026-09-16T12:59:22Z
source: palo-alto-main
---

# Panorama CloudConnector Plugin 2.1.0 Clear
...
Updated on 
 ...
 What's new for Panorama CloudConnector Plugin 2.1.0 (For AIOps and Cloud NGFW for
 AWS) 

 Panorama CloudConnector plugin 2.1.0 now
 supports proxy configuration settings from Panorama. These settings only take effect
 after a commit. Here are the scenarios: 

 Configuring proxy settings: When you configure proxy settings and
 perform a commit, the CloudConnector plugin won't recognize the new proxy
 settings during this commit. However, after the commit, the plugin will use the
 proxy configuration for all future interactions with the cloud. 

 Removing proxy settings: When you remove proxy settings and perform a
 commit, the CloudConnector plugin won't recognize the removed proxy settings
 during the commit. However, after the commit, the plugin will no longer use the
 proxy configuration for any future interactions with the cloud. 

 The following Panorama versions are supported: 

 Panorama Versions 10.2.x (10.2.3 and later) and 11.0.0 

 You can download this version of the CloudConnector Plugin on Panorama
 versions 10.2.3 or later from the Customer Support Portal or directly from Panorama Plugins . 

 Panorama Versions 11.0.1 and above 

 To help customers, we have preinstalled this plugin with the newer
 Panorama versions. 
 ...
 If you have already installed both the AIOps plugin and the CloudConnector
 plugin, uninstall the AIOps plugin, as they are identical and only the name has
 changed. Ensure that you have only one plugin installed, which should be the
 latest version of the CloudConnector plugin. 

 If you installed the AIOps plugin on PAN-OS 10.2.3 and then upgraded to PAN-OS 11.0.1 or
 later, a default version of the plugin will be installed with the new PAN-OS version.
 This results in both plugins being present on Panorama. In this case, follow these
 steps: 
 ...
 Fixed an issue where the root partition was reaching 99% capacity
 and causing the device to become non-functional. 

 ADI-30623 

 Fixed an issue where the CloudConnector plugin was using the
 incorrect user. 
