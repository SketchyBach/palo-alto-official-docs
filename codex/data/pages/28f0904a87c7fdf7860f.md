...
Account Protection for the Prisma Browser 
 ...
 browser-level credential salting to mobile devices and bringing iOS to parity
 with Windows. 

 Account Protection locks enterptise accounts by salting credentials a`t the
 browser level. When a protected account is accessed through Prisma Browser ,
 a secret element is added to the password before it reaches the website. The
 credential only works inside the browser — it cannot be reused from Chrome,
 Safari, or any other browser. If a user is offboarded, they lose access
 ...
 iOS Access only Access protected applications; enrollment must be performed on desktop 
