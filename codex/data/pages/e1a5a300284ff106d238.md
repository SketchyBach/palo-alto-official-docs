---
url: https://cortex-docs.paloaltonetworks.com/data-security-documentation/onboard-and-configure/csp-onboarding
fetched_at: 2026-09-06T10:51:11Z
source: cortex-platform
---

# Cloud service provider (CSP) onboarding | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center note-sticky
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Cortex Data Security provides a unified, normalized asset inventory for cloud assets. This capability provides deeper visibility into all your assets and superior context for incident investigation. 

 Cortex Data Security currently supports onboarding the following cloud service providers (CSPs): 
 ...
 The onboarding process has two main phases: configuring your template in Cortex Data Security and deploying it in your CSP environment. This topic describes the high-level process of these two phases and the considerations you should have in mind before you start onboarding. 

 Phase 1: Cortex CSP onboarding wizard 

 In this phase, you use the Cortex Data Security onboarding wizard to define the scope of your CSP environment and choose which security features you want to enable. Based on your selections, Cortex Data Security generates a ready-to-deploy configuration template in a format compatible with your CSP. 
 ...
 GCP, OCI: Currently only standard regions are supported. 
 ...
 Cortex Data Security supports two scan modes: 

 Cloud Scan (recommended): The scanning takes place within the Cortex Data Security environment. No additional setup is needed. 
 ...
 For a side-by-side comparison of the two modes—covering cost, management overhead, data residency, DSPM data access, and SaaS support—and guidance on when to choose Outpost Scan, see Outpost fundamentals and planning . 

 OCI: Outpost Scan is not supported. Use Cloud Scan mode for this CSP. 
 ...
 If you do not want to cover your entire environment, you can limit the scope by including or excluding specific regions, accounts, or organization units/folders/management groups. Exclusions apply to asset discovery and to all Cortex Cloud scanning capabilities that operate on discovered assets. Excluded accounts are not scanned and do not appear in the asset inventory, scan results, or alerts. Excluded accounts remain visible on the Cloud Instances page, marked as excluded, so you can review or re-include them at any time. 
 ...
 Both set to include : Cortex Cloud monitors all accounts under the included organizational units, folders, or management groups, plus any individually included accounts. 

 Both set to exclude: Cortex Cloud excludes any account that is either under an excluded organizational unit, folder, or management group, or in the excluded accounts list. 
 ...
 AWS: The CloudFormation StackSet deploys IAM roles to all accounts within the selected organization or OU scope, even if you exclude specific accounts from scanning. Exclusions only prevent Cortex Data Security from scanning or discovering the account; exclusions do not prevent role deployment. Additionally, audit log collection applies to all accounts in scope; exclusions do not apply to log collection. 

 Azure: When deploying at the tenant or management group scope, Azure Policy definitions are applied across all subscriptions in scope. Excluded subscriptions are not scanned or discovered by Cortex Data Security, but the policy definition may still be present. 

 GCP: When deploying at the organization or folder scope, the Terraform template provisions resources across all projects in scope. Excluded projects are not scanned or discovered by Cortex Data Security. 

 OCI: Tenancy-level deployment applies to all compartments. Compartment-level exclusions prevent scanning but the identity policy is applied at the tenancy level. 

 Step 6: Enable security capabilities 

 Select which security modulesCortex Data Security should activate for your connected accounts. Your selections determine the contents of the authentication template generated at the end of this wizard. 

 Every template includes the following: 

 Base deployment: The CSP-specific resources that enable Cortex Data Security to connect to your environment, discover the accounts in scope, and register them, along with the deployment logic that reports status back to Cortex Data Security. 
 ...
 Audit logs record activity in your CSP environment. When audit log collection is enabled, Cortex Data Security uses the log data for: 
 ...
 For CSPs that support custom log collection, you can choose how Cortex Data Security collects audit logs: 

 Custom (user defined) : If you already have audit log infrastructure in place, you can configure Cortex Data Security to use your existing setup. 

 Automated : Cortex Data Security sets up everything needed to collect audit logs on your behalf, including the log trail, storage, and notifications. 

 OCI: Audit log collection options may be limited compared to AWS, GCP, and Azure. Refer to the CSP-specific onboarding documentation for details. 

 Step 9: Template generation 

 Once you complete your selections, Cortex Data Security generates a customized configuration template tailored to your choices. The template format depends on your CSP. 
 ...
 After your authentication template is ready, deploy it in your CSP environment to provision the resources Cortex Data Security needs to connect.: 

 What happens during deployment 

 The authentication template automatically provisions everything Cortex Data Security needs to connect to your environment: 

 Secure access role: Grants Cortex Data Security read-only access to your cloud resources. 
 ...
 Secure notification channel: Reports deployment details back to Cortex Data Security over HTTPS. 

 After deployment completes, Cortex Data Security registers the account and the account appears as Connected in Cortex Data Security. 
 ...
 The notification mechanism executes and sends organization details to Cortex Data Security. 
 ...
 Cortex Data Security discovers member accounts as the role for each account becomes available. 

 All accounts appear as Connected in Cortex Data Security. 

 Phase 3: Post-deployment 

 After deployment completes, all included accounts display a status of Connected in Cortex Data Security. A Connected status means Cortex Data Security has established trust with your CSP and is starting work. A Connected status does not mean that every resource has been discovered yet. 

 What happens after deployment 

 After deployment completes, all included accounts display a status of Connected in Cortex Data Security. Note that a Connected status means Cortex Data Security has established trust with your CSP and is starting work. A Connected status does not mean that every resource has been discovered yet. 
 ...
 Validates that the access role works and that Cortex Data Security has the permissions it needs. 
 ...
 The capabilities you enabled (CSPM, vulnerability scanning, DSPM, and others) begin evaluating resources as Cortex Data Security discovers them. 
 ...
 Discovery starts immediately when your account reaches Connected status and continues in the background. Cortex Data Security works through your environment and begins posture evaluation as resources are inventoried. 
 ...
 Resource count: The discovered resources count grows steadily as Cortex Data Security works across your environment. 
