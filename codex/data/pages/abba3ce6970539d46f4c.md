---
url: https://cortex-docs.paloaltonetworks.com/application-security/application-security-posture-management-aspm/applications/business-application-assets
fetched_at: 2026-09-06T10:12:00Z
source: cortex-platform
---

# Business application assets | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center shield-alt
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 To access the Business Application asset inventory, use one of these methods: 

 Under Inventory , select All Assets → Business Applications 

 In the Application Security module, select Application Management → Business Applications 

 Understand the inventory 

 Widgets 

 The inventory includes these widgets: 

 Business Criticality: How is the application portfolio distributed across criticality tiers, and how many applications are rated Critical or High? 

 Business Unit: Which business units own the largest share of applications in the current result set? 

 When you filter a widget, the asset inventory automatically updates to reflect those specific filter conditions. 
 ...
 Right-click any application row to open its context menu and take action without leaving the inventory. All actions require the asset management permission; when that permission is absent, the actions are not shown. 
 ...
 Opens the application record for editing. 
 ...
 Sets the business unit on the selected applications. 

 Delete business unit 

 Clears the business unit from the selected applications. 

 Add assets 

 Adds assets to the application. 

 Remove assets 

 Removes assets from the application. 

 Caution: Deleting a business application is permanent and cannot be undone. If the application is referenced by scope-based access control or by policies, deletion changes what those controls enforce. The confirmation dialog reports whether the application is in use before you confirm. 

 Investigate application assets and security issues 

 Select an application in the inventory table to open its side card, providing in-depth information organized into several tabs. The Overview tab (default display) offers highlights and a general summary. Additional contextual tabs provide specific details, including a Topology tab (providing context on the application path to production), and tabs focusing on specific issue types detected within the asset, such as Secrets and Vulnerabilities . 
 ...
 Limitation : Only up to 4,000 issues will be displayed in the Issues inventory when redirecting from the graph, even if the count in a particular stage (such as Deployed) is higher. 
 ...
 Selecting a repository redirects you to the main Issues inventory. It filters for that repository's vulnerability issues. 

 Selecting the branch link opens the repository's asset side-card. You can view details without leaving the page. 
 ...
 Create business applications directly from the inventory page. The Create Applications control sits in the table header, above the widgets and the table. For more information, refer to Business applications . 
 ...
 Previous Scope user access to applications (Application SBAC) 
