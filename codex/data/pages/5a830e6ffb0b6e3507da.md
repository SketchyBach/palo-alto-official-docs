...
Updated on 
 ...
 The dashboard reports on the workspace you have selected. Under Tenant it covers the whole tenant, including the certificates held by every workspace; inside a workspace it covers that workspace. 
 ...
 My certificates : Displays only the certificates held by the workspace you have selected. Unassigned certificates are excluded, which is what distinguishes it from All certificates inside a workspace. 
 ...
 Note: Inside a workspace, All certificates , Retired certificates , and Revoked certificates include unassigned certificates only for users who hold the Assign Certificates to Tenants permission set. Without it, no filter shows unassigned certificates, All certificates and My certificates return the same results, and the Assign and Unassign actions don't appear. Under Tenant every filter includes unassigned certificates regardless. 
 ...
 Selecting Workspace as the column lets you narrow the inventory to the certificates a particular workspace holds, or to the unassigned ones. Under Tenant the value list offers the tenant and each of your workspaces; inside a workspace it offers that workspace. Unassigned is offered in both. 
