---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-30/vm-series-plugin-301/known-issues-in-vm-series-plugin-301.html
fetched_at: 2026-09-16T12:59:12Z
source: palo-alto-main
---

# Known Issues in VM-Series Plugin 3.0.1 Clear
...
cores are changed using CLI. 
 ...
 Workaround: When you see these errors in the logs, run
the following CLI command: 
 ...
 In an AWS environment, the command show plugins vm_series aws ha state does
not display valid outputs on the passive firewall. 

 Workaround: Use the command show high-availability state instead
of show plugins vm_series aws ha state . 

 This issue is addressed in VM-Series plugin version 3.0.2. 

 PLUG-10180 

 Bootstrapping by adding software, plugin,
and content together fails, due to limited disk space and larger
PAN-OS images for 10.2.0. 

 Workaround: Bootstrap with only plugin and content, and
then upgrade the PAN-OS software. 

 PLUG-9982 

 Traffic loss is observed in KVM PA-VM
in SRIOV mode with system setting at packet mmap. 
