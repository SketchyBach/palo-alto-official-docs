---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-vmware-vcenter/panorama-plugin-for-vmware-vcenter-101/addressed-issues-in-the-panorama-plugin-for-vmware-vcenter-101.html
fetched_at: 2026-09-16T13:00:32Z
source: palo-alto-main
---

# Addressed Issues in Panorama Plugin for VMware vCenter 1.0.1 Clear
...
in the Panorama plugin for VMware vCenter 1.0.1. 

 PLUG-3171 

 Fixed an issue where the VMware vCenter
plugin fails to validate vCenter credentials. 

 PLUG-8382 

 Fixed an issue where the op-vc_status.vcenter_login uses
externally untrusted values from XML request to generate the URL
and auth parameters for session.post . 

 PLUG-6676 

 Fixed an issue where the VMware vCenter
plugin fails to retrieve IP- tags when VLAN Trunking is configured
in the port group. 

 PAN-163387 

 Fixed an issue where the VMware vCenter
plugin is unable to parse tags with special XML characters. The
plugin now ignores such tags. 

 PAN-165744 

 The Panorama peer in HA using the VMware
vCenter plugin is unable to perform automatic retreival after failover.
This issue is resolved. 

 PLUG-2311 

 Fixed an issue where the VMware vCenter
plugin fails to retrieve vCenter tags with Panorama HA failover. 
