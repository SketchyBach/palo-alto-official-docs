---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/panorama-web-interface/panorama-managed-devices-summary/managed-firewall-information
...
firewall is assigned. 
 ...
 Default Master Key Enabled 

 Indicates whether the managed firewall is currently using the default
 master key. Possible values are: 

 Yes —The firewall is using the default master key. You
 must configure a custom master key before the grace period
 expires. 

 No —The firewall has a custom master key
 configured. 

 Unknown —Panorama cannot determine the master key
 status of the firewall (for example, if the firewall is
 disconnected or running a PAN-OS version earlier than
 12.1.2). 

 Days Remaining to Configure Custom Master Key 

 Displays the number of days remaining in the grace period for the
 firewall to replace the default master key with a custom master key.
 This column is relevant only when Default Master Key
 Enabled is Yes . 

 If the grace period expires before you configure a custom master key,
 the firewall fails all standard configuration commits and HA
 synchronization until you apply a custom master key. 

 Possible values are: 

 N days —The number of days remaining before enforcement
 begins. Zero days indicates that the grace period has
 elapsed. 

 Unknown —Panorama cannot determine the remaining grace
 period (for example, if the firewall is disconnected or
 running a PAN-OS version earlier than 12.1.2). 

 N/A —The firewall already has a custom master key
 configured. 
