---
url: https://cortex-docs.paloaltonetworks.com/cortex-xdr-agent/9.2/cortex-xdr-agent-for-windows/cortex-xdr-agent-for-virtual-environments-and-desktops
fetched_at: 2026-09-16T08:51:33Z
source: cortex-platform
---

# Cortex XDR agent for virtual environments and desktops | 9.2 | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Run the cytool imageprep scan command, with any of the following optional parameters: 
 ...
 After the scripts execute, the machine will shut down, and the App Layering service will proceed to take the snapshot and complete the publishing process automatically. 
 ...
 Install the Cortex XDR agent on the chosen layer (OS/Platform) during the preparation process of the App Layering image. 
 ...
 Before finalizing the layer, run imageprep for non-persistent VDI’s that utilize Citrix app layering and the perform the extra steps as necessary for MCS or PVS provisioning shown below. 
 ...
 Initiate the publishing process and watch the App Layering job details. Wait until the status reads, "Waiting for the virtual machine to be shutdown so a snapshot can be taken". 

 Connect to the running virtual machine. 

 Execute the following finalization commands in order: 

 Update VDI: cytool vdi update . 

 Run imageprep for Citrix non-persistent VDI's that utilize Citrix App Layering 

 Run the completion script as an administrator: 

 C:\Windows\setup\Scripts\kmsdir\CompleteDepoylment.cmd . 

 When the scripts have finished running, the machine will shut down. The App Layering service will automatically capture the snapshot and finalize the publishing process. 
 ...
 Boot a machine using the newly created virtual disk (vDisk). 

 Manually run imageprep for Citrix non-persistent VDI's that utilize Citrix App Layering 

 Create a new vDisk from this now-prepared machine. 

 PVS Provisioning (Cortex XDR agent on Platform layer and OS layer Installations) 

 To successfully execute imageprep on a PVS vDisk, the vDisk must first be booted on a VDI instance with read/write access enabled. 

 Run imageprep for Citrix non-persistent VDI's that utilize Citrix App Layering 

 When the the imageprep process is completed, revert the vDisk to read-only mode for normal use. 
 ...
 Return to complete the instruction depending on which provisioning process you are using. 
 ...
 For non-persistent VDI environments, after the vDisk is finalized, all automatic software updates (including operating system and user applications) should be disabled. If a software update is required, the image configuration procedure must be repeated, followed by rerunning the imageprep proceedure. Failure to follow this process may lead to performance degradation upon the initial launch of the updated application. 
 ...
 Previous Install the Cortex XDR Agent with Installer and Content Update Package 
