---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-0/pan-os-release-notes/pan-os-9-0-addressed-issues/pan-os-9-0-4-addressed-issues.html
fetched_at: 2026-09-16T13:55:59Z
source: palo-alto-main
---

# PAN-OS 9.0.4 Addressed Issues Clear
...
command to recover the RAID controller. 
 ...
 Fixed an issue where the timer system call
activated more frequently than expected, which caused higher than expected
CPU usage. 

 PAN-123371 

 Fixed an issue where the Wildfire ® Analysis
...
AOC cable. 
 ...
 Fixed an issue where Remove Config ( Panorama Plugins )
did not remove the configuration for any plugins you have set up
on Panorama. 

 PAN-121185 

 Fixed an intermittent issue where domains
...
(SCTP) connections due to incorrect session handling. 
 ...
 Fixed an intermittent issue where the dataplane stopped
...
the firewall from PAN-OS 8.0.16 to PAN-OS 8.1.8. 
 ...
 Fixed an intermittent issue with local high
availability (HA) status changes where a process ( mprelay )
failed to commit changes to the HA state. 

 PAN-118411 

 Fixed an issue where ARP entries took longer
than expected to age out in a single run. 

 PAN-118407 

 Fixed an issue where an internal path monitoring failure
due to a buffer leak caused the firewall to reboot. 

 PAN-117923 

 Fixed an issue where the management server stopped
...
fix, you must first upgrade ( Devices Dynamic Updates ) to GlobalProtect Clientless
VPN content release 79 or a later release. 

 PAN-116807 

 ( PA-7000, PA-5200, and PA-3200 Series
...
remained at 0% after a job was completed. 
 ...
 Fixed an issue where a process ( pan_comm )
...
validity checks. 
 ...
 Fixed an issue where the date in the GlobalProtect HTTP
...
the IPv6 address filter is based on netmask. 
 ...
 Fixed an issue where the test routing bgp virtual-router default restart peer Peer-v6 CLI
...
validation when the proxy ID contained 24 or more characters. 
 ...
 Fixed an issue where Dynamic IP and Port
(DIPP) NAT pools did not release used ports after all sessions were removed. 

 PAN-115852 

 Fixed an issue on VM-Series firewalls on
...
following validation error: plugins 'read-only' is not an allowed keyword . 
 ...
 Fixed an issue where after a refresh of
...
to reboot. 
 ...
 Fixed an issue where data logs were generated
...
to reboot. 
 ...
 Fixed an issue where log purging took longer
than expected, which prevented the firewall from capturing traffic
logs. 

 PAN-114567 

 Fixed an issue where the Eventid eq globalprotectportal-config-succ system query
caused the management server ( mgmtsrvr ) process to
stop responding. 

 PAN-114566 

 Fixed an issue where after a commit the
...
when the license was valid and successfully applied. 
 ...
 Fixed an issue where traffic was blocked
by the safe search enforcement instead of the intended allow rule. 

 PAN-114526 

 Fixed an issue where larger than expected
...
to ensure GTP-U traffic are captured. 
 ...
 Fixed an issue where Panorama in FIPS mode defaulted
...
page. 
 ...
 Fixed an issue where sessions were offloaded
...
VPN application. 
 ...
 Fixed an issue on a Panorama M-Series appliance where
the Summary ( Panorama Managed
Devices Summary )
web interface refreshes every 10 seconds when set to manually refresh. 

 PAN-114090 

 Fixed an issue on a Panorama virtual appliance
...
stopped responding and caused the firewall to reboot. 
 ...
 Fixed an issue where loading custom app
...
categories. 
 ...
 Fixed an issue where GlobalProtect configured
...
of the GlobalProtect pool. 
 ...
 Fixed an issue where the Panorama management server
returned a Security Copy (SCP) server connection error after you
created an SCP Scheduled Config Export profile ( Panorama Scheduled Config Export )
...
the following error message: Out-of-memory condition detected, kill process . 
 ...
 Fixed an issue where scheduled reports were
...
sent several benign critical content alerts daily. 
 ...
 Fixed an issue where obsolete IPv6 Neighbor Discovery
...
to reach full capacity and caused new IPv6 ND entries to fail. 
 ...
 Fixed an issue where hardware security module (HSM)
...
unable to log in when character spaces were used in usernames. 
 ...
 Fixed an issue where an incorrect SSH key initialization
caused a process ( pan_comm ) to stop responding every
15 minutes when you configured an SSH proxy on the firewall. 

 PAN-110990 

 Fixed an issue where a logical operation not configured
...
group object in the Include List ( Network Zone <zone-name> Include List ). 

 PAN-110839 

 Fixed a rare issue where a commit pushed
...
unencrypted TLS sessions due to an authentication policy match. 
 ...
 Fixed an issue where the VPN tunnel operational status
incorrectly displays up even though
the VPN tunnel is down. 

 PAN-106889 

 Fixed a rare issue on a firewall in an HA active/passive
...
generate a system log when the firewall detected a RAM issue. 
 ...
 Fixed an issue when you connected to an
...
a test mail that contained a data leak prevention (DLP) pattern. 
 ...
 Fixed an issue where the firewall incorrectly forwarded
traffic when you configured the ingress interface with a QoS policy
and the egress interface as a tunnel. 

 PAN-104808 

 Fixed an issue where scheduled SaaS reports generated
and emailed empty PDF reports. 

 PAN-104251 

 Fixed an issue where the syslog server TCP keep-alive
