---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-citrix-adc
fetched_at: 2026-09-16T07:25:19Z
source: palo-alto-main
---

# Provision to a Citrix ADC Clear
...
Updated on 
 ...
 Use this procedure to provision a certificate from Next-Gen Trust Security to a Citrix ADC machine. You can provision a certificate to an existing SSL endpoint or allow Next-Gen Trust Security to create a new endpoint during provisioning. 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Installations > Machines . 

 Select the Citrix ADC machine you want to provision a certificate to. 
 ...
 In Certificate Name , enter the name you want the certificate to use on the Citrix ADC. 

 Note: Certificate names must be unique on the Citrix ADC. If the name is already in use, provisioning fails. 

 In Chain Bundle Name , enter the name for the CA certificate bundle as it should appear on the Citrix ADC. 

 Note: Chain bundle names must be unique on the Citrix ADC. 

 (Optional) In Partition , enter an existing Citrix ADC partition name. 

 If left blank, the default partition is used. 

 Partition names are case-sensitive. 

 From Endpoint Type , select one of the following: 
 ...
 In Endpoint Name , enter the name of the endpoint. 

 If the name already exists in the selected partition, Next-Gen Trust Security provisions the certificate to that endpoint. 

 If the name does not exist, Next-Gen Trust Security creates a new endpoint using the specified name. 

 (Optional) In SNI , enter an alternative DNS name for Server Name Indication. 

 (Optional) To prevent the certificate from being pushed immediately, set Push upon saving to No . 

 Click Save . 

 After saving, Next-Gen Trust Security provisions the certificate to the specified Citrix ADC partition and creates an installation record on the Installations tab. If a new endpoint was created, it is ready to be assigned to a virtual server or HTTPS health monitor on the Citrix ADC. 
