...
Updated on 
 ...
 While a scan is running, the scan detail page (select View
 Progress in
 Scan )
 displays real-time progress indicators that help you assess scan health and plan around
 completion time: 
 Error
 Rate —Displays
 the percentage of attacks that resulted in errors (for example, rate limiting,
 connection timeouts, or content filtering). 

 Estimated
 Completion —Calculated
 from the number of remaining attacks and current attack throughput. The
 indicator shows Calculating... for the first five minutes
 after the attack execution
 starts,
 or if no successful attacks occurred in the past five minutes. 

 Scan Progress
 Tracker —A
 live activity details
 that
 shows what the scan is actively doing. The display varies by scan type: 
 For Attack Library scans , the attack categories (Security,
 Safety, and Brand) are shown with per-category progress counts and
 status (Complete, In Progress, or Pending). The Scan Progress Tracker
 shows the current category and subcategory. 

 For Agent scans , the Agent Scan Progress Tracker
 displays
 the current scan phase: Profiling, Goal Generation, or Attack
 Execution. 

 For Custom Prompt Set scans , the live indicator
 displays
 the current prompt set being processed. 

 Currently Attacking —Displays the scan's current phase.
 The number of phases varies by scan type. This indicator is available only for
 Attack Library (static) and Custom Prompt Set scans. For Attack Library scans,
 it shows the current subcategory under attack. For Custom Prompt Set scans, it
 shows the current prompt set. 
