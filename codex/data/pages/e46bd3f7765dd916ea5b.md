---
url: https://cortex-docs.paloaltonetworks.com/cortex-xdr-5.x/protect-your-endpoints/web-and-api-security-waas/secure-your-api-landscape
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 Discover, assess, monitor, and protect API traffic, endpoints, and specifications in Cortex XDR. 
 ...
 API specification name is obtained from the title field of the specification imported to Cortex XDR. 
 ...
 Number of requests or connections that have been analyzed and verified by Cortex XDR. 
 ...
 The Relations graph shows the connections between the API endpoint, API gateway, and VMs. This mirrors what's shown in the Graph Search . 
 ...
 The link from the number opens the page where you can review the details. Refer to Cases and issues for detailed information. 

 You can view all API security issues and cases detected by Cortex XDR. 
 ...
 If Cortex XDR detects security vulnerabilities or compliance issues in the posture of an API during scanning, an issue is generated. 

 Agentless 

 Detection Method : API Traffic Scanner 

 If Cortex XDR detects anomalies, suspicious activities, or potential security threats in the network traffic of the APIs, an issue is generated. 
 ...
 If Cortex XDR detects threats from cloud workloads, an issue is generated. 
 ...
 Number of requests scanned by Cortex XDR. 
 ...
 Cortex utilizes the DSPM engine to analyze API traffic and classify sensitive data into two categories: Data Profiles and Data Patterns . 
 ...
 Do the following to remove the integration of Azure APIM with Cortex XDR: 
 ...
 Token generated from Cortex XDR. 
 ...
 From File Name , select the panw_apisec_config.txt file that was extracted from the zip that was downloaded from Cortex XDR. 
 ...
 From Cortex XDR, go to Data Sources & Integrations and from F5 BIG_IP LTM , copy the API URL and paste it in the "cortex_collector_url" . 
 ...
 Navigate to Local Traffic → Virtual Servers → Virtual Server List . The virtual server functions as an API Gateway, handling all incoming and outgoing requests and responses, then forwarding that data to the Cortex XDR collector. 
 ...
 Test the request/response and verify that the logs are sent to Cortex XDR. This can be verified by checking that the counter has increased. The scanned API endpoint metadata from f5-bigip is ready for investigation in the API inventory. 
 ...
 When set to Enable , Cortex XDR performs the configured action for each of the options. 
 ...
 When Cortex XDR detects cross-site scripting (XSS) injection, it performs the configured action. 
 ...
 When Cortex XDR detects SQL injection (SQLi) attempts, it performs the configured action. 
 ...
 When Cortex XDR detects injection attacks, it performs the configured action. 
 ...
 When Cortex XDR detects known vulnerabilities (Common Vulnerabilities and Exposures (CVEs)), it performs the configured action. 
 ...
 When Cortex XDR protects workloads from exposing sensitive data, it performs the configured action. 
 ...
 When Cortex XDR detects attempts to bypass authentication controls, it performs the configured action. 
 ...
 When Cortex XDR detects evolving threats, it performs the configured action. 
 ...
 Cortex XDR can identify offensive tools that scan web applications for known security vulnerabilities and misconfiguration, and exploit them. When such tools are found, this module can block or report them. 
 ...
 When Cortex XDR detects HTTP requests with anomalies that are not expected from common web browsers, it performs the configured action. 
 ...
 When Cortex XDR detects automated tools, it performs the configured action. 
 ...
 When Cortex XDR detects known bots, it performs the configured action. 
 ...
 Cortex XDR offers the option to import API specifications that comply with the OpenAPI format, including format, file structure, and data types. 
 ...
 Even if you have already imported the specification, you can edit the API specification in Cortex XDR and add or update the server list. 
 ...
 Specification file name that was imported to Cortex XDR. 
