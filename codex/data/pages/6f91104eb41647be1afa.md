---
url: https://docs.paloaltonetworks.com/ngfw/help/10-2/device/device-certificate-management-certificates/manage-firewall-and-panorama-certificates
...
Manage Firewall and Panorama Certificates 
 ...
 Select Device Certificate Management Certificates Device Certificates or Panorama Certificate Management Certificates Device Certificates to display the certificates that the firewall or Panorama uses for tasks
 ...
 server certificate is not in the trusted CA list on the firewall. 
 ...
 Select the entity that generates the
 certificate: 

 Local —The firewall or
 ...
 ( Required ) Enter a name (up to 63 characters
 ...
 ( SCEP certificates only ) Select a
 SCEP Profile to define how the firewall
 or Panorama communicates with a SCEP server and to define settings
 for the SCEP certificate. For details, see Device > Certificate Management > SCEP . You can
 ...
 ( Required ) Enter the IP address or FQDN that
 will appear on the certificate. 

 Shared 

 On a firewall that has more than one virtual system
 ...
 Select this option if you want the firewall to issue
 the certificate. 

 Marking this certificate as a CA allows you
 to use this certificate to sign other certificates on the
 firewall. 

 Block Private Key Export 

 When you generate a certificate, select this option to
 block all administrators, including Superusers, from exporting the
 private key. 

 OCSP Responder 

 Select an OCSP responder profile from the drop-down
 ...
 ECDSA uses smaller key sizes than
 ...
 Firewalls running PAN-OS 6.1 or
 ...
 You cannot use a hardware security module (HSM) to store
 ...
 Select the Digest algorithm for
 ...
 Specify the number of days (default is 365) that the
