---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/provision-certificates/provision-certificates-microsoft-azure-key-vault
fetched_at: 2026-09-16T07:25:18Z
source: palo-alto-main
---

# Provision to a Microsoft Azure Private Key Vault Clear
...
Updated on 
 ...
 Use this procedure to provision a certificate from Next-Gen Trust Security to a Microsoft Azure Private Key Vault machine. 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Installations > Machines . 

 Select the Microsoft Azure Private Key Vault machine you want to provision a certificate to. 

 Open the More options menu for the machine and select Provision . 

 From Search a certificate by name, expiration, or fingerprint , search for and select the certificate you want to provision. 

 Verify that you selected the correct certificate by reviewing the Subject DN , Validity , and Fingerprint . 

 In Certificate Name , enter the name you want the certificate to use in Azure Key Vault. 

 What if the certificate name is already in use? 

 When provisioning a certificate, Next-Gen Trust Security checks whether the name already exists in the Azure Key Vault: 

 If the name is not in use, it is applied as entered. 

 If the name is already in use, provisioning fails and you must choose a different name. 

 (Optional) To prevent the certificate from being pushed immediately, set Push certificate on save to No . 

 Click Save . 

 After saving, Next-Gen Trust Security provisions the certificate to the specified Azure Key Vault and creates an installation record on the Installations tab. 
