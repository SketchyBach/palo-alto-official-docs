---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-1/pan-os-panorama-api/pan-os-xml-api-request-types/export-files-api/export-technical-support-data.html
...
and a content-disposition header with a suggested filename; for
example: 
...
an active job. 
 ...
 https://<firewall>/api/?key=apikey&type=export&category=tech-support
...
job-id parameter: 
 ...
 https://<firewall>/api/?key=apikey&type=export&category=tech-support&action=status&job-id=id
...
the data is ready to be retrieved. 
 ...
 https://<firewall>/api/?key=apikey&type=export&category=tech-support&action=get&job-id=id

 When using cURL, you can specify the output
file name as an option to cURL ( -o ). After
a successful retrieval of the job data, the job is automatically deleted
by the system. 
...
not complete. In cases like this, stop the active job: 
 ...
 https://<firewall>/api/?key=apikey&type=export&category=tech-support&action=finish&job-id=id
