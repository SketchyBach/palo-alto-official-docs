---
url: https://docs.koi.ai/guides/protect-ai-tools-with-koi/agent-enforcement
fetched_at: 2026-09-06T10:19:41.465Z
...
Visibility into AI coding agent behavior across your organization, starting with MCP tool usage.
...
Koi deploys an enforcement script alongside the existing hooks configuration via the MDM script package

The MDM writes a local policy configuration file containing the enabled guardrails and their blocklists, refreshed on every MDM run cycle

When the agent attempts an action, the hook pipes event data to the enforcement script

The script evaluates the action against active policies - matching file paths or parsing commands against the configured blocklists

The script returns allow or block with structured feedback

Blocked actions surface a message to both the developer and the agent; allowed actions proceed normally and are logged
...
File paths are extracted from event data (directly from tool arguments for file tools)

The ~ prefix is expanded to the user's home directory

Paths are matched against the blocklist using glob-style patterns
...
The command string is extracted from the hook event

Commands are tokenized — split by pipes, ;, &&, ||

Evasion prefixes are stripped: \, command, env, /usr/bin/, bash -c

The base command and its arguments are matched against the blocklist

SQL keywords use case-insensitive substring matching
...
preToolUse - Fires before a tool call executes. Used by both guardrails to intercept Read, Grep, file_search, and Shell tools.
...
Guardrail settings configured in the portal are delivered to the MDM

The MDM deploys the enforcement script as pre-compiled .pyc bytecode to the Koi configuration directory

The MDM writes the guardrails_config.json policy file alongside the script, containing the enabled guardrails and their blocklists

The MDM updates the hooks configuration to route agent events through the enforcement script
