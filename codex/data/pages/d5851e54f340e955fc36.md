---
url: https://docs.paloaltonetworks.com/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-10-addressed-issues
fetched_at: 2026-08-13T17:13:42Z
source: palo-alto-main
---

# PAN-OS 8.1.10 Addressed Issues Clear
...
a manual commit. With this fix, the manual commit is not required. 
 ...
 Fixed an issue where the ifAdminStatus object
...
were not used in an HA configuration. 
 ...
 Fixed an intermittent issue with local HA
...
ICMP 
 ...
 Fixed an issue where test routing bgp virtual-router default restart peer < peer-ID > CLI
...
from the web interface and displayed the following error message: Malformed Request . 
 ...
 Fixed an issue where the child session did
...
stop responding when you searched for common English words. 
 ...
 Fixed an issue where SaaS reports were not
generated due to report definitions not getting pushed to the log
collector. 

 PAN-115160 

 Fixed an issue where a UDP packet without
...
gateway with Security Assertion Markup Language (SAML) authentication. 
 ...
 Fixed an issue where a system query ( Eventideq globalprotectportal-config-succ )
...
to ensure GTP-U traffic are captured. 
 ...
 Fixed an issue where Panorama in FIPS mode
...
VPN application. 
 ...
 Fixed an issue on a Panorama M-Series appliance
where the Summary ( Panorama Managed Devices Summary )
...
you created an SCP Scheduled Config Export profile ( Panorama Scheduled Config Export )
due to the SCP server password exceeding 15 characters in length. 

 PAN-113356 

 Fixed an issue where the web interface did
not populate the Virtual System Name column ( Monitor Manage Custom Reports <monitor-name> Run Now ) when you generated
...
in an HA active/passive configuration was processing traffic. 
 ...
 Fixed an issue where incorrect serial numbers
...
alerts. 
 ...
 Fixed an issue where obsolete IPv6 Neighbor
...
to fail. 
 ...
 Fixed an issue where hardware security module
...
on KVM when you disabled DPDK. 
 ...
 Fixed an issue where an incorrect SSH key
...
the CLI. 
 ...
 Fixed an issue where BGP route attributes
were processed from BGP updates, which caused the firewall to stop
responding. 

 PAN-109457 

 Fixed an issue where the firewall duplicated
...
with a source region of Any . 
 ...
 Fixed an issue where Wildfire® signature
...
pattern. 
 ...
 Fixed an issue where scheduled SaaS reports
generated and emailed empty PDF reports. 

 PAN-104454 

 Fixed a memory leak issue with the User-ID ( useridd )
...
agent 8.1.11 or a later User-ID agent 8.1 release. 
 ...
 Fixed an issue where the syslog server TCP
...
to enable head-of-line (HOL) system mode. 
 ...
 Fixed an issue where the export function ( Panorama Managed Devices Summary Manage )
was not available for managed devices. 

 PAN-50031 

 Fixed an issue where the show wildfire local statistics CLI
