---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-10/vm-series-plugin-1010/addressed-issues-in-vm-series-plugin-1010.html
fetched_at: 2026-09-16T12:59:18Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 1.0.10 Clear
...
in the VM-Series plugin 1.0.10. 

 PAN-113460 

 On failover, the ENIs are attached in the incorrect
order for the now active HA peer on the VM-Series firewall on AWS. 

 ( This issue is fixed in VM-Series plugin 1.0.10 ) With
this fix, the logic for interface ordering is updated to use the
MAC address so that the ENI configuration is retained per your configuration. 

 PAN-120892 

 When the VM-Series firewall on AWS that is configured
in a high availability configuration fails over, the ENIs are attached
in the incorrect order on the now active peer. 

 ( This issue is fixed in VM-Series plugin 1.0.10 ) With
this fix, upon failover, the ENI configuration is retained per your
configuration. 
