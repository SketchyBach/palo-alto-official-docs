---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/service-providers/10-1/mobile-network-infrastructure-getting-started/gtp/mobile-network-protection-profile.html
...
GTP traffic. 
 ...
 The Mobile Network Protection profile ( Objects Security Profiles Mobile Network Protection )
...
enable protocol validation for GTPv1-C, GTPv2-C, and GTP-U, and
enable GTP-U content inspection to scan user data within GTP-U tunnels. 
...
or GTPv2-C messages with invalid IE length. 
 ...
 Reserved field in header —Checks for
malformed packets that use invalid values or reserved values in
a header. 
...
for outer GTP sessions is not supported with GTP stateful inspection. 
 ...
 GTP-U 
 Enabling Stateful Inspection for GTPv1-C and/or GTPv2-C automatically
...
unknown or incorrect message types. 
 ...
 End User IP Address Spoofing —When the
source IP address in a GTP-U packet from the subscriber user equipment
is not the same as the IP address in the corresponding GTP-C message
exchanged during tunnel set up. 
...
the IP traffic encapsulated in GTP-U packets. 
 ...
 Enabling 5G-HTTP2 disables
GTP-C for the profile. 

 PFCP 

 For Packet Forwarding Control Protocol (PFCP), enable Stateful
...
such as Denial of Service (DOS) or spoofing. 
...
Context Request, Delete PDP Context Response. The rest of the tunnel
management messages are logged as Others (explained below). 
