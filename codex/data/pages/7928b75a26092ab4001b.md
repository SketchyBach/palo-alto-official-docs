---
url: https://docs.paloaltonetworks.com/ngfw/help/11-1/device/device-setup-services/global-services-settings
...
authority. 
 ...
 Choose the type of DNS service for all DNS queries that the firewall initiates in support of FQDN
 address objects, logging, and firewall management: 

 Servers —Primary and secondary DNS servers provide domain name
 resolution. 

 DNS Proxy Object —Select a DNS Proxy configured on
 the firewall or create a new DNS Proxy to provide domain name
 resolution. If you enable a DNS proxy, you must enable
 Cache and EDNS Cache
 Responses ( Network DNS Proxy Advanced ). 

 Primary DNS Server 

 If you chose Servers , enter the IP address of the primary DNS server for
 DNS queries from the firewall. For example, to find the update
 server, to resolve DNS entries in logs, or resolve FDQN-based
 address objects. 

 Secondary DNS Server 

 ( Optional ) If you chose Servers , enter the IP address of a
