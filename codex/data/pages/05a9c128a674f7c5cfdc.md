...
deviceconfig setting ctd udp-bypass-exceed-queue 
 ...
 Advanced IP Defense Settings 

 AIPD Cloud Endpoint 

 The FQDN that the firewall uses for real-time Advanced IP Defense
 cloud lookups and direct-to-IP detection queries. The firewall
 connects to this endpoint to retrieve IP attribute verdicts when a
 local cache miss occurs. 

 The default endpoint connects to the nearest regional server
 automatically. You can override the default to specify a regional
 endpoint that meets your data residency or performance
 requirements. For available regional endpoints, see Update Advanced IP Defense
