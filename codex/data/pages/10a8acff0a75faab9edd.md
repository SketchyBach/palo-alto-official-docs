---
url: https://docs.paloaltonetworks.com/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-30/vm-series-plugin-301/addressed-issues-in-vm-series-plugin-301
fetched_at: 2026-09-16T07:45:10Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 3.0.1 Clear
...
fixed in the VM-Series plugin 3.0.1. 

 PLUG-10229 

 Fixed an issue where customers upgrading
PA-VM HA deployments from lower versions to PAN OS 10.2.0, with
Flexible vCPU license models and memory profile between 9 to 15G
were seeing traffic disruption for the existing sessions during
the upgrade. Now, the upgrade completes without any disruptions. 

 PLUG-9987 

 Fixed an issue where the validate
 button at
 Device VM-Series Azure HA Configuration Edit displayed an error
 resource-mgr-endpoint is invalid in an Azure HA environment. 

 PLUG-9983 

 Fixed an issue where the bootstrapped
firewall was not associating with the Collector-Group on Panorama,
when "cgname" was provided under the user-data in an AWS instance. 

 PLUG-9771 

 Fixed an issue where performance impact(high
utilization of the CPU) was observed on vmxnet3(Esxi) and ena(AWS)
driver based VM-Series Firewalls due to dpdk-rx-queue-num set
to 1 for PAYG images. 

 Workaround : Use the FLEX PAYG calculated license. 

 PLUG-9868 

 Fixed an issue in AWS FIPS-CC enabled
 PA-VMs, where the HA failover was impaired on interface-move mode
 that was observed in VM-Series plugin 2.1.2 and above. 
