---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/network/network-interfaces/pa-7000-series-layer-3-interface
fetched_at: 2026-09-16T07:31:50Z
source: palo-alto-main
---

# PA-7000 Series Layer 3 Interface Clear
...
remove the current NetFlow server assignment from the interface. 
 ...
 Select a virtual router, or click Virtual
Router to define a new one (see Network
> Virtual Routers ). Select None to remove
the current virtual router assignment from the interface. 
...
address type to the interface: 
 ...
 Static —You
must manually specify the IP address. 

 PPPoE —The firewall will use the interface
for Point-to-Point Protocol over Ethernet (PPPoE). 

 DHCP Client —Enables the interface
to act as a Dynamic Host Configuration Protocol (DHCP) client and
receive a dynamically assigned IP address. 

 Firewalls
that are in a high availability (HA) active/active configuration
do not support PPPoE or DHCP Client. 
...
number of IP addresses. 
 ...
 To delete an IP address, select the
address and click Delete . 
 ...
 Select to activate the interface for PPPoE
termination. 
...
connection. 
 ...
 Enter and then confirm the password for
the username. 

 Show PPPoE Client Runtime Info 

 ( Optional ) Opens a dialog that
displays parameters that the firewall negotiated with the Internet
service provider (ISP) to establish a connection. The specific information
depends on the ISP. 
 ...
 Select the authentication protocol for PPPoE communications: CHAP (Challenge-Handshake Authentication
...
current protocol assignment from the interface. 
 ...
 Perform one of the following steps to specify
the IP address that the Internet service provider assigned (no default value): 

 Type the entry in Classless Inter-Domain Routing (CIDR) notation: ip_address / mask (for
example, 192.168.2.0/24). 
...
object of type IP netmask . 
 ...
 Select None to remove the current
address assignment from the interface. 
...
the firewall and Internet service provider, enter a route metric
(priority level) to associate with the default route and to use
for path selection (range is 1 to 65,535). The priority level increases
as the numeric value decreases. 

 Access Concentrator 

 ( Optional ) Enter the name of the
access concentrator on the Internet service provider end to which
the firewall connects (no default). 

 Service 

 ( Optional ) Enter the service string
(no default). 

 Passive 

 Select to use passive mode. In passive mode,
...
lowercase letters, numbers, periods, hyphens, and underscores. 
 ...
 For the route between the firewall and DHCP
...
no default). The priority level increases as the numeric value decreases. 
 ...
 Select to display all settings received
...
Router Advertisement . 
 ...
 The remaining fields apply only
if you enable RA. 
...
(NDP) monitoring. When enabled, you can select NDP Monitor (  
 ...
  in Features
column) and view information about a neighbor that the firewall
...
and the User-ID (on a best-case basis). 
 ...
 To provide stateless address auto-configuration
(SLAAC) on IPv6 interfaces, select and configure the other fields
in this section. IPv6 DNS clients that receive the router advertisement
(RA) messages use this information. 

 RA enables the firewall
...
with this feature to provide DNS and other settings to clients. 
...
no retransmission time (range is 0 to 4,294,967,295; default is unspecified). 
 ...
 Specify how long the client will use the
...
via DHCPv6. 
 ...
 Select to enable the firewall to send DNS
information in NDP router advertisement (RA) messages from this
IPv6 Ethernet interface. The other DNS Support fields in this table
...
a series of DNS lookup requests to root DNS and authoritative DNS servers
...
client can use the RDNS servers to resolve domain names (range is Max Interval
...
Maximum length is 255 bytes. 
 ...
 A DNS search list is a list of
...
the resulting DNS query from the router is for the FQDN “quality.company.com”. 
...
to this Ethernet interface or add a new SD-WAN Interface Profile. 
 ...
 You
must Enable SD-WAN for the interface before you
can apply an SD-WAN Interface Profile. 
...
or determined automatically ( auto ). 
 ...
 Select a profile that defines the protocols
...
traffic that has a VLAN tag. 
 ...
 Specifies that all subinterfaces belonging
...
an IP address and its associated hardware (MAC) address. To delete
...
the IP address and MAC address of the neighbor. 

 Enable NDP Proxy 

 Ethernet Interface Advanced NDP Proxy 

 Select to enable the Neighbor Discovery
...
destined for those addresses. 
 ...
 It is recommended that you select Enable
NDP Proxy if you use Network Prefix Translation IPv6
(NPTv6). 

 If Enable NDP Proxy is selected,
...
and clicking Apply Filter ( 
 ...
 Click Add to enter
one or more IPv6 addresses, IP ranges, IPv6 subnets, or address
objects for which the firewall will act as the NDP proxy. Ideally,
...
translation in NPTv6. The order of addresses does not matter. 
 ...
 If
the address is a subnetwork, the firewall will send an ND response
for all addresses in the subnet, so we recommend that you also add
the IPv6 neighbors of the firewall and then select Negate to
instruct the firewall not to respond to these IP addresses. 
...
the firewall to use global defaults. 
 ...
 If LLDP is enabled, select to allow the
...
v1 
 ...
 Palo Alto Networks DDNS —You must use
this for SD-WAN AE interfaces and SD-WAN Layer 3 subinterfaces. 
