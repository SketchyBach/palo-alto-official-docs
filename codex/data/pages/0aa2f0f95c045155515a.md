---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/installations-overview/overview-machines/create-a-new-machine/vmware-nsx-avi
fetched_at: 2026-09-16T07:25:17Z
source: palo-alto-main
---

# Create a VMware NSX Advanced Load Balancer (AVI) Machine Clear
...
Updated on 
 ...
 Creating a machine enables Next-Gen Trust Security to connect directly to a VMware NSX Advanced Load Balancer (AVI) so certificates can be provisioned to its keystores. After creating the machine, you can provision certificates to it. 

 Before You Begin 

 Hostname or IP address of the VMware NSX Advanced Load Balancer (AVI). 

 Port number, if the load balancer uses a port other than 443. 

 Credentials with administrative permissions on the VMware NSX Advanced Load Balancer (AVI). 

 In VMware NSX ALB (AVI) Address/Hostname , enter the hostname or IP address of the load balancer. 

 If the load balancer uses a port other than 443, enter the port number in Port . 
 ...
 Enter the required VMware NSX Advanced Load Balancer (AVI) credentials. 
