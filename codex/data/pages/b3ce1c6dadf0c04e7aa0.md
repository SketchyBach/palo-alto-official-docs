---
url: https://docs.paloaltonetworks.com/ngfw/help/11-1/device/device-server-profiles-http
fetched_at: 2026-08-13T16:45:16Z
source: palo-alto-main
---

# Device > Server Profiles > HTTP Clear
...
Device > Server Profiles > HTTP 
 ...
 Select Device Server Profiles HTTP or Panorama Server Profiles HTTP to configure a server profile
...
a source or destination IP address on logs that a firewall generated. 
 ...
 See Device
> Log Settings for System, Config, User-ID, HIP Match, and
Correlation logs. 

 See Objects
> Log Forwarding for Traffic, Threat, WildFire, URL Filtering,
Data Filtering, Tunnel Inspection, Authentication, and GTP logs. 
...
delete all references to the profile from the Device Log settings or Objects Log Forwarding profile. 
...
zeros, alphanumeric characters, underscores, hyphens, dots, or spaces. 
 ...
 Select the scope in which the server profile
...
After you save the profile, you can’t change the Location . 
 ...
 Tag registration allows you to add or remove
...
enforce policy rules to an IP address based on tags. 

 Add the
connection details to enable HTTP(S) access to the User-ID agent
on a firewall. 

 To register tags to the User-ID agent on Panorama,
...
hyphens, dots, or spaces. 
 ...
 A server profile can include up
to four servers. 
...
as a User-ID agent. 
 ...
 Enter the port number on which to access
the server or firewall. The default port for HTTP is 80 and for
HTTPS is 443. 

 For tag registration, the firewall uses HTTP
or HTTPS to connect to the web server on the firewalls that are
configured as User-ID agents. 

 TLS Version 

 Select the TLS version supported for SSL
on the server. The default is 1.2 . 

 Certificate Profile 

 Select the certificate profile to use for
the TLS connection with the server. 
...
when establishing a secure connection to the server. 
 ...
 Select the HTTP method that the server supports.
The options are GET, PUT, POST (default), and DELETE. 
...
the User-ID agent, use the GET method. 
 ...
 Enter the username that has access privileges
to complete the HTTP method you selected. 
...
of an administrator with a superuser role. 
 ...
 Enter the password to authenticate to the
server or the firewall. 

 Test Server Connection 

 Select a server and Test Server Connection to
...
on the firewall or Panorama. 
 ...
 Specify the resource to which you want to
send logs using HTTP(S). 
...
and Value pairs, and the request payload. 
 ...
 Select the log attributes you want to include
as the payload in the HTTP message to the external web server. 

 Send Test Log 

 Click this button to validate that the external
