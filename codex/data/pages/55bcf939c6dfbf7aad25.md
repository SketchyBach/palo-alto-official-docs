...
# GCP Artifact Registry repositories are not encrypted with Customer Supplied Encryption Keys (CSEK) m | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center
...
For the complete documentation index, see llms.txt . This page is also available as Markdown . 
 ...
 GCP Artifact Registry repositories are not encrypted with Customer Supplied Encryption Keys (CSEK) m 
 ...
 resource "google_artifact_registry_repository" "pass" { provider = google-beta 

 location = "us-central1" repository_id = "my-repository" description = "example docker repository with cmek" format = "DOCKER" kms_key_name = google_kms_crypto_key.example.name } 

 Previous GCP Pub/Sub Topics are not encrypted with Customer Supplied Encryption Keys (CSEK) misconfiguration 

 Next GCP Big Table Instances are not encrypted with Customer Supplied Encryption Keys (CSEKs) misconfigur 
