---
url: https://docs.paloaltonetworks.com/ngfw/help/11-1/device/device-setup-management
fetched_at: 2026-08-13T16:45:22Z
...
users and groups from Master Device . 
 ...
 Enable Selective Push 

 Select this option (enabled by default) to make the options
 associated with selective push, such as Push Changes Made
 By or Push All Changes appear
 in the Panorama web interface. This allows you to choose the
 Push Changes Made By option to
 selectively push only the configuration changes made by specific
 administrators rather than pushing the entire configuration. 

 Disable (clear) this option to temporarily disable
 selective push and explicitly force a full configuration push every
 time. Consequently, administrators no longer have the option to push
 only their changes. When you disable selective push, the options
 associated with selective push, such as the Push Changes
 Made By or Push All Changes 
 options do not appear. 

 For more information about the options associated to selective push,
 see Panorama Commit
 Operations . 
...
through the firewall to stop when the log database is full. 
 ...
 Enable Threat Vault Access (enabled
by default)—Enables the firewall to access the Threat Vault to gather the latest information
about detected threats. This information is available for threat
logs and for top threat activity charted on the ACC. 
...
you to investigate for probable cause. 
 ...
 Enable High Speed Log Forwarding ( PA-5200 Series, PA-5450, PA-7000
 Series, and PA-7500 firewalls only ; only enabled in
 the PA-5450 by default)—As a best practice, select this
 option to forward logs to Panorama at up to a maximum rate
 of 120,000 logs per second. When disabled, the firewall
 forwards logs to Panorama at a maximum rate of only 80,000
 logs per second. 
...
the specified period, the firewall closes the connection. 
 ...
 Template ) to forward logs to Strata Logging Service . 
 After you configure Log Forwarding
 ( Objects
...
firewalls that are not managed by Panorama) You can enable firewalls that are not managed by Panorama to send logs
 to Strata Logging Service . To do this, you need to first generate a key in the Strata Logging Service app . This key enables
