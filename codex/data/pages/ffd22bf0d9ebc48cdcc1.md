---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/co-overview-issuing-certs/certificate-issuing-templates/defining-recommended-settings-for-an-issuing-template
fetched_at: 2026-09-16T07:25:03Z
source: palo-alto-main
---

# Defining Recommended Settings for a Request Policy Clear
...
Updated on 
 ...
 Recommended settings provide guidance when users create certificate requests using a specific request policy. These settings help reduce guesswork and save time by pre-populating commonly used values. 

 Info: Recommended settings are optional. If you need to require specific values for certificate requests, use issuing rules instead. 

 Important: Request Policies belong to the tenant. Defining or modifying recommended settings requires the Superuser role and Tenant selected in the workspace switcher; with a workspace selected the page is read only. See Workspace-Scoped Resources . 

 To Add Recommended Settings for a Request Policy 

 Sign in to Next-Gen Trust Security. 

 Click Configuration > Certificate Policies > Request Policies . 

 Do one of the following: 

 Click New to create a new request policy. 

 From the request policy list, click the name of an existing request policy. 

 In the Recommended Settings column on the right, click Modify . 

 Enter the values you want to recommend for this request policy. 

 Click Save . 

 The request policy is updated with the recommended settings you defined. 
