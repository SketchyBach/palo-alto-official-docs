...
# Manually Create Configuration Profiles (V3) for Prisma Agent Clear
...
Updated on 
 ...
 Create and deploy configuration profiles for Prisma Agents that define
 ...
 If your organization uses Endpoint DLP , you will need to create two
 configuration profiles: one for Prisma Agent , and the other for
 ...
 you only need to create the profile for Prisma Agent . 
 ...
 Create the configuration profile for Prisma Agent . 
 ...
 Configure the Content Filter . The Prisma Agent content filter suppresses the following
 ...
 Prisma Agent notifications are displayed on your end
 users' devices. 
 ...
 tab to specify access settings for Prisma Agent . This will
 provide Full Disk Access permissions for Prisma Agent 
 processes. 
 ...
 automatically load Prisma Agent system extensions on the
 ...
 extensions, configure removable system extensions for Prisma Agent by adding a second Allowed System Extensions and
 Team IDs section. 
...
<string>Connect</string>
 ...
 Save the Prisma Agent configuration
 profile. 

 Set the scope for the Prisma Agent configuration profile. 
 ...
 installation of the Prisma Agent configuration
 ...
 distribution of the Prisma Agent configuration
 profile. 

 Create the configuration profile with Endpoint DLP settings. 

 Deploy this profile to all
 managed endpoints so that the Endpoint DLP system extensions are
 pre-authorized by MDM before the
 PrismaAccessAgentInstaller_ <version> .pkg 
 installer runs. When the installer runs, it deploys Endpoint DLP
 automatically—the extensions must already be authorized or macOS will prompt
 users to approve them. 
 ...
 Double-click the Prisma Agent and Endpoint DLP profiles
 that you deployed. 
