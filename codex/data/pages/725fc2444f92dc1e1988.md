---
url: https://docs.koi.ai/guides/protect-applications-with-koi/software-inventory-preview
fetched_at: 2026-09-06T10:19:52.928Z
...
Discover and track all applications across your organization
...
Applications Inventory collects installed applications across macOS and Windows endpoints. Koi discovers applications differently on each OS platform, which affects what appears in the inventory.

macOS: Koi scans the Applications folder. Any application installed under this folder is discovered and added to the Inventory. Applications located outside the Applications folder will not appear in the inventory.

Windows: Koi scans the Windows registry for installed applications. Applications that are not registered in the registry will not be discovered. In particular, applications installed from the Microsoft Store are not written to the registry, so they will not appear in the inventory.

The inventory focuses on applications, it does not include every binary and process that may exist on a device, such as drivers, background services, CLI tools. In order to view these please see the Binaries Inventory.
...
Number of unique devices where the application was observed

Categories

The type of application (for example Security tools, Productivity, or Gaming)

Last seen

Most recent time Koi observed this application in the organization

First seen

The first time Koi observed this application on any endpoint

Last used

Last time a user in the org executed this application

Is Signed

Indicates whether the application’s binary is code-signed and the signature validation succeeded. Values: True, False.

Certificate Expiration Status

Status of the signing certificate based on its validity dates. Values: Valid, Expiring soon (60 days or less), Expired, Not yet valid (start date is in the future).

Certificate Expiration Date

The signing certificate valid-until date, the date after which the certificate is no longer valid.

Key Size

Public key length of the signing certificate, in bits, for example 2048-bit.

Certificate Issuer

The Certificate Authority (CA) that issued the signing certificate, used to validate the trust chain.

Certificate Signature Algorithm

The algorithm used to sign the certificate, for example SHA256_RSA, SHA1_RSA, MD5_RSA.

Signing Certificate Hash

Hash of the signing certificate (thumbprint).

SHA256

Cryptographic hash of the binary file, uniquely identifies a specific build or version of the app.

Team ID

Apple Developer Team identifier, a stable publisher identifier on macOS, shared across apps signed by the same developer account.

For example: 
Team ID (Google Chrome) = EQHXZ8M8AV


Signing ID

macOS code signing identifier for the application, formatted as <Team ID>:<App Identifier>, used to group all versions of the same app under the same publisher.

For example: 
Signing ID (Google Chrome)= EQHXZ8M8AV:com.google.Chrome
