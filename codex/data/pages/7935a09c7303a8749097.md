---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-cisco-trustsec/panorama-plugin-for-cisco-trustsec-2-0-4/addressed-issues-in-panorama-plugin-for-cisco-trustsec-2-0-4.html
fetched_at: 2026-09-16T13:00:39Z
source: palo-alto-main
---

# Addressed Issues in Panorama Plugin for Cisco TrustSec 2.0.4 Clear
...
Updated on 
 ...
 Learn about addressed issues in the Panorama plugin for Cisco TrustSec version
 2.0.4. 

 The following list describes issues addressed in the Panorama plugin for Cisco TrustSec
 2.0.4. 

 PLUG-22710 

 Fixed an issue where upgrading the plugin from version 2.0.0 to 2.0.2 or 2.0.3 caused
 intermittent " Credentials incorrect. Please update credentials and then
 commit " errors in the plugin log for one of the configured
 ISE/pxGrid nodes in HA during a force sync operation, even though credentials had
 not changed and the connection status reported success on both nodes. 

 PLUG-22865 

 Fixed an issue where a single IP address could intermittently be assigned two
 different IP tags simultaneously (one static, one dynamic) when two pxGrid servers
 were configured in pubsub mode. The duplicate tag condition arose because ISE nodes
 forwarded binding updates concurrently, causing the plugin to register conflicting
 tag associations for the same IP prefix. 
