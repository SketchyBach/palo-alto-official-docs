---
url: https://docs.paloaltonetworks.com/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-7-addressed-issues
fetched_at: 2026-08-13T17:13:53Z
source: palo-alto-main
---

# PAN-OS 8.1.7 Addressed Issues Clear
...
running PAN-OS 8.1.5 or an earlier PAN-OS 8.1 release. 
 ...
 Fixed an issue where source URLs ( Objects External Dynamic Lists <EDL-name> Create
List Source URL ),
...
when configured with HTTP Header Insertion ( Objects Security Profiles URL Filtering <Filter-name> HTTP
Header Insertion ). 

 PAN-111866 

 Fixed an issue where the push scope selection
...
encapsulation (GRE) version 1 traffic. 
 ...
 Fixed an issue where Captive Portal authentication
...
during a GTP-C update. 
 ...
 Fixed an issue on VM-Series firewalls Dynamic
...
responding during a local commit. 
 ...
 Fixed an intermittent issue on a firewall
where configuring Force Template Values ( Network Interfaces Commit Push to Devices Templates ) deleted the zone
assigned to an interface. 

 PAN-108642 

 Fixed an issue where P2MP OSPF static neighbor
did not display in the run-time neighbor table. 

 PAN-108542 

 Fixed an issue where the DHCP client interface
was configured with an incorrect subnet mask value instead of the
value provided by DHCP option 1. 

 PAN-108374 

 Fixed an issue on GlobalProtect™ where you
were unable to authenticate when the domain name included the ampersand
( & ) character. 

 PAN-108123 

 Fixed an issue where applications took longer
...
with this fix are not removed as expected (see known issue PAN-114041 ). 
 ...
 Fixed an issue where the test security-policy-match XML
...
search for service objects by destination port numbers. 
 ...
 Fixed an issue where the firewall did not
display the full URL information in the URL Filtering log ( Monitor URL Filtering )
...
/ PAN-SA-2019-0007). 
 ...
 Fixed an issue where DNS proxy memory leaks
...
which caused the firewall to stop responding. 
 ...
 Fixed an issue where MIB attributes caused
MIB compilation failures when using a third-party compiler. 

 PAN-106426 

 Fixed an issue where GlobalProtect did not
...
enabled with DPDK. 
 ...
 Fixed an intermittent issue where octet
values were incorrect for random flows in the NetFlow traffic. 

 PAN-104466 

 Fixed an issue on a VM-50 firewall where
...
(RTC) battery voltage exceeded the maximum threshold value. 
 ...
 Fixed an issue where BGP conditional advertisements
...
after you pushed a configuration to a firewall. 
 ...
 Fixed an issue where a newly deployed VM-Series
firewall in the VMware NSX environment did not display on the summary
web interface ( Panorama Summary )
...
to detect link-fault events on the transmission side. 
 ...
 Fixed an issue where SNMP queries displayed
incorrect values. 
...
ID did not clear when the session ID was set to 0. 
 ...
 Fixed an issue where administrators configured
...
message: Unauthorized request . 
 ...
 Fixed an intermittent issue on VM-Series
...
a development configuration file issue. 
 ...
 Fixed an issue where the IPv6 flow label
...
plane. 
 ...
 Fixed an issue where SYN-ACK packets with
...
traffic on the management interface. 
 ...
 Fixed an issue where a firewall stopped
...
(this command will persist across reboots). 
 ...
 Fixed an issue where the new connection
