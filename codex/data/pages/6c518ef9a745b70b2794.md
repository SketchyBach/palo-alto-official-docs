---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/vm-series-plugin/vm-series-plugin-21/vm-series-plugin-218/addressed-issues-in-vm-series-plugin-218.html
fetched_at: 2026-09-16T12:59:06Z
source: palo-alto-main
---

# Addressed Issues in VM-Series Plugin 2.1.8 Clear
...
fixed in the VM-Series plugin 2.1.8. 

 PLUG-11075 

 Fixed an issue that caused VM-Series
firewall bootstrap with userdata to fail on OCI. 

 PLUG-11052 

 Fixed an issue where resources do not appear in the OCI Metric Explorer even
 if OCI monitoring is enabled for them. 

 PLUG-10952 

 Fixed an issue where the plugin reports pkt_alloc_failure 
 when PA-VM-300 version 10.1.4 or later is deployed. 

 PLUG-10604 

 Fixed an issue where there was a mismatch in calculating license capacity
 and shared memory that was preventing the firewall from booting. 

 PLUG-10412 

 Fixed an issue in flex licensing such
as VM-FLEX-4, the HA4 status is displayed as not configured or not operational . 

 PLUG-10319 

 The validate function does not throw
errors when resource-mgr endpoint is empty and
does not verify if the URL entered in resource-mgr endpoint
field is valid. 
