---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/plugins/vm-series-and-panorama-plugins-release-notes/panorama-plugin-for-aws/aws-plugin-512.html
fetched_at: 2026-09-16T12:59:46Z
source: palo-alto-main
---

# Panorama Plugin for AWS 5.1.2 Clear
...
Updated on 
 ...
 The Panorama plugin for AWS version 5.1.1 introduces new features for Panorama
 integration with Cloud NGFW for AWS. 

 The Panorama plugin for AWS version 5.1.2 provides enhancements to
 Panorama-orchestrated deployments for the VM-Series on AWS. 
 ...
 The hierarchical tagging feature introduced on the vm-series
 firewall adds the IP tags to an individual IP address when the
 individual IP address has a different IP tag associated with it. During
 IP address computation for the DAG, the IP address also gets added to
 the DAG in addition to the CIDR. The firewall gets updated with all the
 IP addresses associated with a VPC. 

 When you upgrade or downgrade the AWS plugin from version
 5.1.2, you must reconfigure the tag pruning flag manually. 

 When you upgrade the AWS plugin to version 3.0.3, the
 previously configured custom tags get removed and select all 32
 tags option is selected by default on the Notify Group 
 dialogue box in Panorama. 

 After upgrading to AWS plugin version 5.1.2, the public IP
 addresses are not retrieved when you use the tag combination with your
 VPC. 

 Shared DAGs that are pushed from Panorama do not show those IP tags,
 which are not used in the security policy on the VM-Series firewall. 
