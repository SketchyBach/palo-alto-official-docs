---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/8-1/pan-os-release-notes/pan-os-8-1-addressed-issues/pan-os-8-1-2-addressed-issues.html
fetched_at: 2026-09-16T10:42:19Z
source: palo-alto-main
---

# PAN-OS 8.1.2 Addressed Issues Clear
...
for the FQDNs of those log receivers. 
 ...
 Fixed an issue where, after end users successfully
...
not specify. 
 ...
 Fixed an issue where syslog servers misrepresented
...
flows that needed a NetFlow update. 
 ...
 Fixed an issue where, after you upgraded
...
that specified a continue action ( Objects Security Profiles URL Filtering ) because the
...
policy rule cleared any existing firewall selections for that rule ( Panorama Policies <policy_type> {Pre
Rules | Post Rules | Default Rules} Target ). 

 PAN-95486 

 Fixed an issue with VM-Series firewalls
...
File when you scheduled the updates using the management interface. 
 ...
 This fix requires
the VMware NSX 2.0.4 or a later plugin. 
...
running the following CLI command on the KVM host: 
 ...
 Fixed an issue where mobile endpoints that
...
(PDP) context update. 
 ...
 Fixed an issue where, after you added group
...
management server where you could not assign tags that contained
a colon ( : ) to service or service
group objects. 
...
for an IKE gateway configuration ( Network Network Profiles IKE Gateways <IKE_gateway> General ). 
...
lost and recovered through successful WMI probing. 
 ...
 Fixed an issue where WildFire submissions
...
process to stop responding. 
 ...
 Fixed an issue where a Panorama management
...
to firewalls running PAN-OS 8.0 or an earlier release ( Objects GlobalProtect HIP Objects <HIP_object> Disk Encryption Criteria <encrypted_location> ). 

 PAN-94516 

 Fixed an issue on PA-500, PA-220, PA-220-R,
...
not support a hardware security module (HSM). 
 ...
 Fixed an issue where the total log storage
utilization that the firewall displayed did not account for IP
Tag storage that was set to less than two per cent ( Device Setup Management Logging and Reporting Settings Log Storage ). 

 PAN-94450 

 Fixed an issue where QSFP+ interfaces (13
...
| WildFire} <match_list_profile> ). 
 ...
 Fixed an issue where the firewall routed
...
in the matching rules ( Objects Address Groups <address_group> ). 
 ...
 Fixed an issue where a firewall forwarded
...
it advertised to External BGP (eBGP) peers in the BGP peer group. 
 ...
 Fixed an issue on firewalls deployed in
virtual wire mode where SSL decryption failed due to a memory pool
allocation failure. 

 PAN-94122 

 Fixed an issue where firewalls intermittently
...
to Block sessions on certificate status check timeout ( Objects Decryption Decryption Profile <Decryption_profile> SSL Decryption SSL Forward Proxy ). 

 PAN-94070 

 Fixed an issue where Bidirectional Forwarding
...
to that Layer 3 interface. 
 ...
 Fixed an issue where the request system external-list show type ip name <EDL_name> CLI
command did not display external dynamic list entries after you
restarted the management server ( mgmtsrvr ) process. 

 PAN-93937 

 Fixed an issue where the management server ( mgmtsrvr )
...
after you configured Traps log ingestion ( Panorama Log Ingestion Profile ) for
forwarding to a syslog server ( Panorama Server Profiles Syslog )
and committed configuration changes ( Commit Commit to Panorama ). 

 PAN-93755 

 Fixed an issue where SSL decrypted traffic
failed after you configured the firewall to Enforce Symmetric Return in
Policy Based Forwarding (PBF) policy rules ( Policies Policy Based Forwarding ). 
...
inbound perfect forward secrecy (PFS) sessions. 
 ...
 In certain customer environments, enhancements
...
of drive communication failure in PA-5200 Series firewalls. 
 ...
 Fixed an issue where configuring additional
...
a local Log Collector when you committed the configuration ( Panorama Setup Interfaces ),
...
mode. 
 ...
 Fixed an issue on firewalls in a high availability
...
on all firewall models except the PA-200 and VM-50 firewalls. 
 ...
 Fixed an issue where the Security policy
...
fix, the firewalls send logs only to the active Panorama. 
 ...
 PAN-93318 
 Fixed an issue where firewall CPU usage reached
100 per cent due to SNMP polling for logical interfaces based on
updates to the Link Layer Discovery Protocol (LLDP) MIB (LLDP-V2-MIB.my). 
...
administration page (CVE-2018-9337). 
 ...
 PAN-93233 
 Fixed an issue where PA-7000 Series firewalls
...
the incorrect hostname when responding to SNMP get requests. 

 PAN-93046 
 Fixed an issue where administrators whose roles
have the Privacy privilege disabled ( Device Admin Roles <role> Web UI )
can view details about source IP addresses and usernames in the
PDF reports exported from the firewall. 

 PAN-92958 
 Fixed an issue where disk utilization increased
unnecessarily because the firewall did not archive and rotate the /var/on file, which
therefore grew to over 40MB. 

 PAN-92892 

 ( VM-50 Lite firewalls only ) Fixed
...
in those fields. 
 ...
 Fixed an issue where an administrator whose
Admin Role profile had the Command Line privileges
set to superuser ( Device Admin Roles <role> Command Line ) could not request tech-support dump from
the CLI. 
...
firewall was in an initialization state after you rebooted it. 
 ...
 Fixed an issue on PA-3250 and PA-3260 firewalls
...
reference to the address objects before pushing configurations to firewalls. 
 ...
 Fixed an issue where the firewall dataplane
...
stopped responding while system resource usage was running high. 
 ...
 Fixed an issue where an Aggregate Ethernet
...
were in sync. 
 ...
 Fixed an issue where PA-7000 Series, PA-5200
...
assigned an invalid identifier. 
 ...
 Fixed an issue on PA-3050, PA-3060, PA-5000
...
tools to modify the input type for that field ( Panorama Managed Collectors <Log_Collector> Authentication ). 
...
the actual values were different ( Network Interfaces <interface> Advanced ). 
...
server displayed no output for the User Activity Report ( Monitor PDF Reports User Activity Report ). 
 ...
 PAN-87079 
 ( PA-3060, PA-3050, PA-5000 Series, PA-5200
Series, and PA-7000 Series firewalls only ) Fixed an issue where
Threat logs displayed an Other IP Flood message
instead of identifying the threat name of the correct protocol (such
as TCP Flood ) when traffic reached
the configured SYN flood max-rate threshold ( Objects Security Profiles DoS Protection <DoS_Protection_profile> Flood Protection SYN Flood ). 

 PAN-86672 

 Fixed an issue where in rare cases a commit
...
instead of the selected firewalls. 
 ...
 Fixed an issue with scheduled log exports
...
7.1 release and displayed the following error: 
...
bootup. 
 ...
 Fixed an issue where, after pushing the
high availability (HA) Group ID from a Panorama management server
to a firewall and overriding the value on the firewall ( Device High Availability General Setup ),
