---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/9-0/pan-os-release-notes/pan-os-9-0-addressed-issues/pan-os-9-0-3-addressed-issues.html
fetched_at: 2026-09-16T13:55:59Z
source: palo-alto-main
---

# PAN-OS 9.0.3 Addressed Issues Clear
...
to display in the inner session traffic and threat logs. 
 ...
 ( PA-5250, PA-5260, PA-5280, and PA-7000 Series
...
the impacted decoder(s). 
 ...
 Cortex Data Lake without Panorama —where
...
started with Cortex Data Lake now. 
 ...
 ( Firewalls with an AutoFocus license
...
in the logs when you configure a service route or proxy. 
 ...
 Fixed an issue where end users who don't
have REST API authentication roles were able to list and edit configuration
rules. 

 PAN-117149 

 Fixed an issue on firewalls configured with authentication
...
in the profile. 
 ...
 Fixed an issue where multiple device group administrators
...
through a proxy server was subjected to decryption. 
 ...
 Fixed an issue where communication between
...
the "in" and "not in" configurations ( Objects Log Forwarding Add Add Filter Filter Builder )
...
a NAT policy. 
 ...
 Fixed a rare issue where Traffic logs, Threat logs
and URL filtering logs stopped generating. 

 PAN-115012 

 Fixed an issue where a process ( appweb )
stopped responding, which caused the web interface to stop responding. 

 PAN-114867 

 Fixed an issue where GlobalProtect™ gateway
...
did not get queued when you configured more than 32 collector groups. 
 ...
 Fixed an issue where the setsystem setting layer4-checksum disable CLI command
...
configured the Device Group with large group hierarchies. 
 ...
 Fixed an issue where multiple dataplanes
...
cloud service firewalls. 
 ...
 Fixed an issue on a VM-Series firewall where
...
stopped responding, which caused the dataplane to restart. 
 ...
 Fixed an issue where the firewall dropped UpdatePDPContext reponse
...
a use-after-free (UAF) vulnerability in the Linux kernel ( PAN-SA-2019-0017 / CVE-2019-8912) 
 ...
 Fixed an issue with a memory leak on Panorama appliances
...
the User-ID™ ( useridd ) process to stop running. 
 ...
 Fixed an issue on Panorama VM-Series firewalls where
...
bootstrapped firewall. 
 ...
 ( PA-5200 Series firewalls only )Fixed
...
caused a split-brain condition. 
 ...
 Fixed an issue where a predefined report ( blocked
...
caused the report to incorrectly display logs for alerts. 
 ...
 Fixed an issue where the connection between
...
command. 
 ...
 ( PA-5200, PA-3200, and PA-7000 Series
...
generate a custom report ( Monitor Manage Custom Report <device-name> Report Setting ). 
 ...
 Fixed an issue where the dataplane stopped responding
due to an incorrectly calculated offset when you configured Exclude
video traffic from the tunnel ( Network GlobalProtect Gateways <gateway-name> Agent Video Traffic ). 
...
to stop responding and displayed the following error message: Dataplaneis down: controlplane exit failure . 
 ...
 Fixed an issue where Captive Portal authentication required
two log-in attempts when the authentication sequence was configured
as an authentication profile. 

 PAN-110293 

 Fixed an issue where GTP-U traffic dropped
...
restarted due to a race condition when generating custom reports. 
 ...
 Fixed an intermittent issue where the firewall dropped
...
with Trusted Root CA . 
 ...
 ( PA-500 and PA-800 Series firewalls
...
where configuring Force Template Values ( Network Interfaces Commit Push to Devices Templates ) deleted the zone assigned
to an interface. 
...
caused the dataplane to restart during a commit. 
 ...
 Fixed an issue where the Strict IP Address
Check incorrectly triggered when you enabled ECMP ( Network Virtual Routers Add Router settings ECMP ). 
...
send emails when you configured the email gateway with an FQDN. 
 ...
 Addressed an issue where in a slow network environment
...
this issue. 
 ...
 Fixed an issue where an API call ( show system disk details ),
...
where a commit and FQDN refresh took longer than expected. 
 ...
 Fixed an issue where the decode filter was
...
on a 20GQ NPC card took longer than expected to respond. 
 ...
 Fixed an issue Panorama M-Series and virtual appliances
...
to alert. 
 ...
 Fixed an issue where temporary files generated during
...
Rewrite Exclude Domain List ( Network GlobalProtect Portals <portal-name> Clientless
