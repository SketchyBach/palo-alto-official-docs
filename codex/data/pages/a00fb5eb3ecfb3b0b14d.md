---
url: https://docs.paloaltonetworks.com/ngfw/help/12-1/network/network-network-profiles/network-network-profiles-ike-gateways/ike-gateway-advanced-options-tab
...
that are lost when a peer is unavailable. 
 ...
 IKEv2 Advanced Options PQ QKD Tab 

 Enable Quantum Key Distribution 

 Enable Quantum Key Distribution to use Quantum
 Key Distribution, as described in the ETSI GS QKD 014 standard. 

 QKD Profile —Select the QKD profile to
 assign to the IKE gateway from the dropdown. 

 SAE ID —The local SAE ID is displayed
 for confirmation. This field is read only. 

 Peer SAE ID —Enter the SAE ID assigned
 to the IKE peer on the other side of the VPN tunnel. 

 Key Size (bits) —The key size, in bits,
 for the key returned from the KME QKD. The default value is
 256 bits. 

 A larger symmetric key provides
 higher resistance to a quantum attack using Grover's
 algorithm. A minimum of 256 bits should be used. 

 QKD Timeout (secs) —The amount of time,
 in seconds, that the firewall waits to terminate IKE
 peering after the KME cannot return a proper key. 
