---
url: https://docs.paloaltonetworks.com/content/techdocs/en_US/pan-os/10-2/pan-os-upgrade/upgrade-panorama/deploy-updates-to-firewalls-log-collectors-and-wildfire-appliances-using-panorama/panorama-log-collector-firewall-and-wildfire-version-compatibility.html
...
Appliances Using Panorama 
 ...
 PAN-OS® 10.2 version compatibility for Panorama™, Log
...
on both the Panorama management server and the Dedicated Log Collectors. 
 ...
 Before upgrading firewalls to PAN-OS 10.2,
you must first upgrade Panorama to 10.2. 

 ( PAN-OS 10.2.6 and earlier releases )
 Panorama must be running the same or a later PAN-OS version than the
 firewall it manages. See the Compatibility Matrix for more
 information. 
 ( PAN-OS 10.2.7 and later releases ) Panorama
 can manage a firewall running a higher version of PAN-OS within the same
 release train. For more information, see Changes to Default Behavior in PAN-OS
 10.2 

 ( PAN-OS 10.2.6 and earlier releases ) Dedicated Log Collectors must be
 running the same or later PAN-OS version than the managed firewalls forwarding logs. 

 ( PAN-OS 10.2.7 and later releases ) Dedicated Log Collectors can receive
 logs from firewalls running a higher version of PAN-OS within the same release
 train. However, it is a best practice that the Dedicated Log Collectors run the same
 or later PAN-OS version than the managed firewalls forwarding logs. 

 Panorama running PAN-OS 10.2 can manage WildFire® appliances and WildFire appliance clusters that
