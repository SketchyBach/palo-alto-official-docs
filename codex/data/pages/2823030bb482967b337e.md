---
url: https://docs.paloaltonetworks.com/prisma-browser/administration/manage-prisma-browser-policy-rules/manage-prisma-browser-access-and-data-control-rules
...
Updated on 
 ...
 when no other policy rule is applicable. Since this rule must be available
 ...
 Users/User Groups - Select the Users and User Groups 
 and that will be covered by the Rule. It's possible to select
 multiple Users and User Groups. The default is Any
 user . 
 ...
 refer to Manage Prisma Browser
 Applications . 
 ...
 Microsoft 365 Domain Name Standard domain format
 ...
 AWS Account ID 
 Region 
 Exactly 12 numeric (0-9)
 characters 
 Validated against AWS Supported
 Regions list, 

 Open AI chatGPT Domain 
 Workspace ID 
 Workspace ID found in local Storage 
 ...
 authentication factor used under Authentication Factor . 
 ...
 Add a webpage frame - Draw a colored frame
 around the webpage for every application, website, and URL in the
 rule's scope. Use the frame to indicate that the user is working in
 a regulated or otherwise sensitive environment. 

 Select a color for the frame, and optionally enter banner text of
 up to 255 characters. The banner text appears in a bar at the
 top of the webpage. When no banner text is entered, the frame is
 drawn as a border only. 

 The frame is applied on every page the user visits within the
 rule's scope, and is removed when the user navigates outside
 that scope. 

 This option will be ignored in the
 mobile rules. 
 ...
 * Partial support - RTF is a text format, but
 ...
 For information on configuring the individual controls, refer to Configure Data
 Controls . 
