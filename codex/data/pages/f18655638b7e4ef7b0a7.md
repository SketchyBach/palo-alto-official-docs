---
url: https://docs.paloaltonetworks.com/next-gen-trust-security/next-gen-trust-security/about-vaas/co-overview-issuing-certs/requesting-viewing-certificate
fetched_at: 2026-09-16T07:25:08Z
source: palo-alto-main
---

# Requesting and Viewing a Certificate Clear
...
Updated on 
 ...
 At least one request policy is configured and available for use 

 The workspace you have selected either owns that request policy or has been authorized to use it. Request policies are created under Tenant and shared with workspaces through the Authorized Workspaces field 

 You have the appropriate Strata Cloud Manager (SCM) role permissions to request certificates 

 Access and Permissions 

 Access to Certificate Requests is controlled by the workspace you have selected and by your SCM role: 

 Inventory Visibility 

 A certificate request belongs to the workspace it was made in. 

 Tenant : The inventory lists the requests from every workspace, and a Workspace column names the one each request came from. You can filter on that column to narrow the list to one workspace or several; the column can't be sorted. 

 A workspace : The inventory lists that workspace's requests only, and the Workspace column is dropped because every row would name the workspace you are already in. 
 ...
 Approving and rejecting requests : The Approve and Reject actions appear only when Tenant is selected. Inside a workspace they are hidden even for a user who holds the approver permission, so an approver has to switch to Tenant to act on a request 

 To Request a Certificate 

 Sign in to Next-Gen Trust Security. 

 Click Insights > Certificate Actions > Create Certificate Requests . 

 Select a Certificate Request Policy . 

 When you select a request policy, its details appear in the Certificate Request Policy section. 
 ...
 Complete the request fields. Some values may be pre-filled or locked based on the request policy. 
 ...
 The request is sent to the certificate authority defined by the request policy. 
 ...
 Click Insights > Certificate Requests > Certificate Requests . 
