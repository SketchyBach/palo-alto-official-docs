---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/objects/objects-security-profiles-vulnerability-protection
...
Updated on 
 ...
 For PAN-OS Shield vulnerability protection settings, see Objects > PAN-OS Shield > Vulnerability
Protection . 
...
for all analysis engines is alert. 
 ...
 Local Deep Learning 
 For each supported analysis engine, you can enable or disable Local Deep Learning to run the deep learning model directly on the firewall for faster threat verdicts and broader traffic inspection. 

 Enable —The firewall runs the deep learning model locally to analyze suspicious traffic. If a potential threat is identified, the traffic is forwarded to the cloud for a false-positive check. 

 Disable —Local Deep Learning is not used. Traffic analysis relies solely on Inline Cloud Analysis. 

 Local Deep Learning is enabled by default when you enable Inline Cloud Analysis on a new Vulnerability Protection profile. On platforms that do not support Local Deep Learning, this column is grayed out. 

 If Local Deep Learning is enabled in a profile that is applied to a device that does not support the feature, a commit warning indicates that the policy will not take effect on that device. The commit still succeeds. 

 Packet Capture 
 For the command injection analysis engine, you can enable local packet capture to retain forensic evidence when the cloud is non-responsive. 

 Enable single-packet —Captures a single packet locally when Local Deep Learning detects a threat and the cloud does not return a verdict. 

 Disable —No local packet capture is performed. 

 This setting applies only to scenarios where the cloud is unreachable or does not respond within the timeout period. When the cloud is responsive, packet captures are handled by the cloud. 

 Exclude from Inline Cloud Analysis and Local Deep Learning 
 Allows you to select a URL or IP address
exception list that bypasses the inline cloud analysis engines and Local Deep Learning.
