---
url: https://docs.paloaltonetworks.com/sd-wan/help/2-1/panorama-sd-wan-plugin/sd-wan-vpn-clusters
fetched_at: 2026-08-13T17:35:36Z
...
SD-WAN branch firewalls with each other (and optionally with SD-WAN hubs).
When you associate branches and hubs in an SD-WAN VPN cluster, the
firewall creates the required IKE and IPSec VPN connections between
the sites based on the type of VPN cluster you specify. 
...
to the target branch. 
 ...
 Mesh —SD-WAN topology that uses hubs and branches,
but allows branch devices to communicate with each other directly. 
...
to 4) of the hub. 
 ...
 The plugin internally maps the priority
to a BGP local preference value; the lower the priority value, the
higher the priority and local preference. 
...
of the hubs to the branches in the cluster. 
...
path. ECMP is enabled for the virtual router ( Network Virtual Routers ECMP )
and ECMP Multiple AS Support is enabled for
BGP ( Network Virtual Routers BGP Advanced ).
