---
url: https://docs.paloaltonetworks.com/pan-os/10-1/pan-os-release-notes/pan-os-10-1-4-known-and-addressed-issues/pan-os-10-1-4-known-issues
fetched_at: 2026-08-13T17:03:41Z
source: palo-alto-main
---

# PAN-OS 10.1.4 Known Issues Clear
...
specific to PAN-OS ® 10.1.4. This list includes issues
...
identified by an issue ID. 
 ...
 If you use Panorama to retrieve logs from Strata Logging Service , new log fields
...
the logs to Strata Logging Service and Panorama. This workaround does not support Panorama
...
is not available. 
 ...
 When the memory allocated is less
than 4.5GB, you cannot upgrade the firewall. The following error message
displays: Failed to install 9.0.0 with the following error: VM-50 in 9.0.0 requires 5.5GB memory, VM-50 Lite requires 4.5GB memory.Please configure this VM with enough memory before upgrading. 
...
eval or trial license expires and then install the production license. 
 ...
 When you rename a device group, template,
...
those objects. There is no impact to existing VM-Series firewalls. 
 ...
 An intermittent error while analyzing signed
PE samples on the WildFire appliance might cause analysis failures. 

 WF500-5471 

 After using the firewall CLI to add a WildFire
...
connecting after you restart the web server with the following command: debug software restart process web-server . 
 ...
 The Advanced WildFire Inline ML models OOXML 
 and Mach-O erroneously display as being
 ...
 On the M-600 appliance, closed ElasticSearch shards are not deleted
 ...
 The following NextGen firewalls are unable to automatically renew the
 device certificate ( Device Setup Management or Panorama Setup Management ). 
 ...
 Workaround: Log in to the firewall CLI and fetch the
 device certificate. 
 ...
 displayed when you configure the Panorama Settings ( Device Setup Management ) as part of a template or template stack
 configuration. 

 PAN-206243 

 The PA-220 firewall reaches the maximum disk usage capacity multiple
 a day that requires a disk cleanup. A critical system log ( Monitor Logs System ) is generated each time the firewall reaches maximum
 disk usage capacity. 
 ...
 On the Panorama management server, cloning any template ( Panorama Templates ) corrupts certificates ( Device Certificate Management Certificates ) with the Block Private Key
 ...
 On the Panorama management server, if you
create multiple device group Objects with the
same name in the Shared device group and any additional device groups ( Panorama Device Groups ) under
...
in a Security policy rule. 
 ...
 You
create a parent device group DG-A and
a child device group DG-B . 

 You create address objects called AddressObjA in
the Shared , DG-A and DG-B device
groups and add AddressObjA to a Security policy
rule under DG-A and DG-B . 

 Later, you change the AddressObjA name
in the Shared device group to AddressObjB . 
...
you Preview Changes ( Commit Push to Devices Edit Selections or Commit Commit and Push Edit Selections ). 
 ...
 This issue is
now resolved. See PAN-OS 10.1.9 Addressed Issues . 

 ( PA-5450 firewall only ) Trying
to configure a custom payload format under Device Server Profiles HTTP yields
a Javascript error. 

 PAN-194515 

 ( PA-5450 firewall only ) The Panorama
web interface does not display any predefined template stack variables
in the dropdown menu under Device Setup Log Interface IP Address . 
...
instead of on Panorama. 
 ...
 All logs ( Monitor Logs ) generated by a firewall running a PAN-OS 10.0
 release are not accessible if you downgrade from PAN-OS 10.1 to
 PAN-OS 10.0, and then upgrade back to PAN-OS 10.1. 

 Workaround: If you need to downgrade from PAN-OS 10.1 to
 ...
 This issue is
now resolved. See PAN-OS 10.1.6-h3 Addressed Issues . 

 ( PA-5450 firewall only ) There is
...
in the same subnetwork can cause routing and connectivity issues. 
 ...
 ( PA-5450 firewall only ) Documentation
for configuring the log interface is unavailable on the web interface and
in the PAN-OS Administrator’s Guide. 

 PAN-188052 

 Devices in FIPS-CC mode are unable to connect
to servers utilizing ECDSA-based host keys that impacts exporting logs ( Device Scheduled Log Export ), exporting
configurations ( Device Scheduled
...
to Devices . 
 ...
 The Panorama management server in Panorama
...
on. 
 ...
 ( PA-5400 Series firewalls only ) On the Panorama management
 server, the device health resources ( Panorama Managed Devices Health ) do not populate. 

 PAN-181116 

 This issue is
now resolved. See PAN-OS 10.1.5 Addressed Issues . 

 After upgrading to PAN-OS 10.1, some GlobalProtect tunnels
fall back to SSL instead of IPSec due to the inadvertent encapsulation
of the ICMP keepalive response from the firewall. 

 PAN-180661 

 This issue is
now resolved. See PAN-OS 10.1.6 Addressed Issues . 

 On the Panorama management server, pushing
an unsupported Minimum Password Complexity ( Device Setup Management )
...
the reason the commit failed. 
 ...
 This issue is
now resolved. See PAN-OS 10.1.7 Addressed Issues . 

 A UI issue in PAN-OS renders the contents
of the Inline ML tab in the URL Filtering
Profile inaccessible on firewalls licensed for Advanced
URL Filtering. Additionally, a message indicating that a License
required for URL filtering to function is unavailable
displays at the bottom of the UI. These errors do not affect the
operation of Advanced URL Filtering or URL Filtering Inline ML. 
...
CLI. The following configuration commands are available: 
 ...
 Define
URL exceptions for specific web sites— 
...
now resolved. See PAN-OS 10.1.6 Addressed Issues . 
 ...
 PAN-OS 10.1.2 is not supported on PA-7000
Series firewalls with HA (High Availability) clustering enabled
and using an HA4 communication link. Attempting to load PAN-OS 10.1.2
...
not affected. 
 ...
 Firewalls managed by a Panorama management server enter maintenance
 mode if: 

 Panorama is running PAN-OS 10.2 and managed firewalls are
 downgraded from PAN-OS 10.2 to PAN-OS 10.1.4 or earlier
 PAN-OS release. 

 Panorama is upgraded from PAN-OS 10.1 to PAN-OS 10.2 and
 managed firewalls are running PAN-OS 10.1.4 or earlier
 PAN-OS 10.1 release. 

 Workaround: When downgrading managed firewalls, downgrade to
 PAN-OS 10.1.5 first and then continue on your downgrade path. When
 upgrading Panorama, upgrade to PAN-OS 10.1.5 first and then continue
 on your upgrade path. 

 PAN-174982 

 In HA active/active configurations where,
...
now resolved. See PAN-OS 10.1.5 Addressed Issues . 
 ...
 Superuser administrators with read-only
privileges ( Device Administrators and Panorama Administrators ) are
unable to view the hardware ACL blocking setting and duration in
the CLI using the commands: 
 ...
 When you activate the advanced URL filtering
...
retrieve and update the licenses: license request fetch . 
 ...
 If you request a User Activity Report on
...
the User Activity Report again. 
 ...
 QoS fails to run on a tunnel interface (for
example, tunnel.1). 

 PAN-172067 

 When you configure an HTTP server profile ( Device Server Profiles HTTP or Panorama Server Profiles HTTP ),
the Username and Password fields
are always required regardless of whether Tag Registration is
enabled. 

 Workaround: When you configure an HTTP server
...
Filter for a Security policy rule ( Policies Security Application Value Show Application Filter ). 
 ...
 If you use Panorama to push a configuration
...
now resolved. See PAN-OS 10.1.7 Addressed Issues . 
 ...
 If you use the NetBIOS format ( domain\user )
...
does not successfully match the user to the correct group. 
 ...
 If you are using Panorama to manage firewalls
...
the commit to the firewall fails. 
 ...
 On the Panorama management server, the ACC returns
inaccurate results when you filter for New App-ID in
the Application usage widget. 

 PAN-171635 

 If you have an on-premise Active Directory
...
to remove the existing group mappings from the firewall. 
 ...
 On the Panorama management server, a custom
report ( Monitor Managed
Custom Reports ) with a high volume of unique
data objects is not generated when you click Run Now . 

 PAN-171145 

 If you edit or remove the value for the mail attribute
...
reflected on the firewall after it syncs with the Cloud Identity Engine. 
 ...
 In Policies Security Policy Optimizer New App Viewer , when you select
...
does not support AF_XDP when deployed in CentOS. 
 ...
 Connecting to the App-ID Cloud Engine (ACE)
...
Engine describes how to do this.) 
 ...
 On the Panorama management server, you are
unable to configure a master key ( Device Master Key and Diagnostics ) for
a managed firewall if an interface ( Network Interfaces Ethernet )
...
configure a master key. 
 ...
 If you issue the command opof stats ,
...
completes. 
 ...
 When a firewall or Panorama appliance configured
...
fails to connect to edge service. 
 ...
 This issue is
now resolved. See PAN-OS 10.1.6-h6 Addressed Issues . 

 PAN-OS reports the PA-5450 fan numbers incorrectly
by listing them in the opposite order. This does not affect fan operation.
For further information, contact Customer Support. 

 PAN-165669 

 If you configure a group that the firewall
...
reports. 
 ...
 On the Panorama management server, a context
switch to a managed firewall running a PAN-OS 8.1.0 to 8.1.19 release fails. 
 ...
 On the Panorama management server, pushes
to managed firewalls ( Commit Push to Devices or Commit
and Push ) may fail when an EDL ( Objects External Dynamic Lists ) is
configured to Check for updates every 5 minutes due
...
every 5 minutes. 
 ...
 A successful deployment of a Panorama virtual
...
release. 
 ...
 On the Panorama management server, activating
...
on managed firewalls in an HA configuration. 
 ...
 PAN-164618 The VM-Series firewall CLI and system logs
display the license name VM-SERIES-X ,
while the user interface displays VM-FLEX-X (in
both cases X is the number of vCPUs).
In future releases the user interface will use the VM-SERIES-X format. 

 PAN-164586 

 If you use a value other than mail for
...
the CLI output. 
 ...
 On the Panorama management server, the ACC and
on demand reports ( Monitor Manage Custom Reports ) are
...
a policy rule. 
 ...
 PAN-162836 
 On the VM-Series firewall, if you select Device Licenses Deactivate VM a popup window
opens and you can choose Subscriptions or Support and
...
now resolved. See PAN-OS 10.1.6 Addressed Issues . 
 ...
 When upgrading a multi-dataplane firewall
...
is not affected. 
 ...
 Workaround: Load the configuration
...
(HA) configuration, content updates ( Panorama Dynamic Updates ) manually uploaded
to the active HA peer are not synchronized to the passive HA peer when
you Install a content update and enable Sync
to HA Peer . 

 PAN-161666 

 The firewall includes any users configured
...
users as groups in the No. of Groups in
...
and has no impact on the existing session. 
 ...
 If you migrate traffic from a firewall running
...
Override Policy on the Palo Alto Networks Firewall . 
...
(TCI) policy. 
 ...
 As a result of a telemetry handling update,
...
DNS Analytics tab within AutoFocus) might not display correct results. 
 ...
 On downgrade to PAN-OS 9.1, Enterprise Data
Loss Prevention (DLP) filtering settings ( Device Setup DLP )
...
and complete the downgrade. 
 ...
 Downgrade your managed
firewall to PAN-OS 9.1 

 Log in to the firewall web interface and view the Tasks to
verify all auto commits related to the downgrade have completed
successfully. 

 Log in to the Panorama web interface and Commit Commit and Push to
your managed firewall downgraded to PAN-OS 9.1. 

 PAN-157103 

 Multi-channel functionality may not be properly
utilized on an VM-Series firewall deployed in VMware NSX-V after
the service is first deployed. 

 Workaround : Execute
the command debug dataplane pow status to view
...
It should now show multiple channels being utilized. 
 ...
 ( Panorama only ) If you configure
...
the CLI to update the combination signature. 
 ...
 On the Panorama management server, downgrading
from a PAN-OS 10.0 release to a PAN-OS 9.1 release causes Panorama
commit ( Commit Commit
to Panorama ) failures if a custom report ( Monitor Manage Custom Reports )
is configured to Group By Session ID . 
...
report. 
 ...
 On the Panorama management server, the Type
column in the System logs ( Monitor Logs System )
...
email PDF reports ( Monitor PDF Reports ) fail if a GIF
image is used in the header or footer. 
 ...
 Done despite reports generated from
 logs in the Strata Logging Service from the PODamericas
 Collector Group jobs are still in a
 Running state. 

 PAN-153068 

 The Bonjour Reflector option is supported
...
for the first 16 interfaces and ignored for any additional interfaces. 
 ...
 There is a known issue where M-100 appliances
...
to the hardware end-of-life dates .) 
 ...
 On a PA-7000 Series firewall chassis having
...
lookup that happens when HA cluster participation is enabled. 
 ...
 Automatic quarantine of a device based on
forwarding profile or log setting does not work on the PA-7000 Series firewalls. 

 PAN-150515 

 After you install the device certificate
...
Panorama to connect to the IoT Security edge service. 
 ...
 During updates to the Device Dictionary,
...
occurs. 
 ...
 Workaround: Perform a force commit to push
the attributes in the content update to the firewall. 

 PAN-150361 

 In an Active-Passive high availability (HA)
...
and perform a force commit to sync the devices. 
 ...
 If you enter a search term for Events that
are related to IoT in the System logs and apply the filter, the
page displays an Invalid term error. 

 Workaround: Specify iot as
...
For example: ( subtype eq iot ) and ( description contains 'gRPC connection' ) . 
 ...
 In an active-passive HA configuration, tags
...
during failover. 
 ...
 After downgrading a Panorama management
...
automatically restarts the VLD and logd processes. 
 ...
 Changing the device group configured in
...
tag mapping information. 
 ...
 On the Panorama management server, adding,
deleting, or modifying the upstream NAT configuration ( Panorama SD-WAN Devices )
does not display the branch template stack as out of sync . 

 Additionally,
adding, deleting, or modifying the BGP configuration ( Panorama SD-WAN Devices )
...
cluster containing the firewall with the modified configuration. 
 ...
 CN-MGMT pods fail to connect to the Panorama management
server when using the Kubernetes plugin. 

 Workaround: Commit the
...
Template Values ( Commit Push to Devices Edit Selections ). 
 ...
 Fetching the device certificate from the
...
fetching the device certificate from the Palo Alto Networks CSP. 
 ...
 Current performance limitation: single data
...
limits the firewall performance. 
 ...
 The Panorama management server allows you
...
and earlier releases where ZTP functionality is not supported. 
 ...
 ElasticSearch is forced to restart when
...
a delay in a log query and ingestion. 
 ...
 On the Panorama management server, managed
firewalls display as disconnected when
installing a PAN-OS software update ( Panorama Device Deployment Software )
but display as connected when you view your
managed firewalls Summary ( Panorama Managed Devices Summary )
and from the CLI. 

 Workaround: Log out and log back
in to the Panorama web interface. 

 PAN-135742 

 There is an issue in HTTP2 session decryption
where the App-ID in the decryption log is the App-ID of the parent
session (which is web-browsing). 

 PAN-134053 

 ACC does not filter WildFire logs from Dynamic
User Groups. 

 PAN-132598 

 The Panorama management server does not
check for duplicate addresses in address groups ( Objects Address Groups ) and
duplicate services in service groups ( Objects Service Groups ) when created
from the CLI. 

 PAN-130550 

 ( PA-3200 Series, PA-5220, PA-5250, PA-5260,
...
the hub. 
 ...
 Workaround: Add any specific prefixes for branches
to the hub advertise-list configuration. 

 PAN-127206 

 If you use the CLI to enable the cleartext
...
or time out. 
 ...
 Dynamic tags from other sources are accessible
using the CLI but do not display on the Panorama web interface. 

 PAN-123040 

 When you try to view network QoS statistics
...
contact Support for information about the workaround. 
 ...
 There is an issue on M-500 Panorama management servers
...
PAN-DB-URL connectivity only supports the following format: 2001:DB9:85A3:0:0:8A2E:370:2 . 
 ...
 PAN-OS 10.0.0 does not support the XML API
for GlobalProtect logs. 

 PAN-120303 

 There is an issue where the firewall remains
...
the Eth1/1 interface. 
 ...
 Workaround: Update the PAN-DB-URL
...
the PAN-DB Server IP address on the managed firewall. 
 ...
 On
the web interface, delete the PAN-DB Server IP
address ( Device Setup Content ID URL Filtering settings). 

 Commit your changes. 

 Add the new M-500 Eth1/1 IP PAN-DB IP address. 
...
in to the firewall CLI. 
 ...
 Restart the devsrvr process: debug software restart process device-server 
...
the firewall. 
 ...
 Alibaba Cloud runs on a KVM hypervisor and
...
a new one. 
 ...
 You can temporarily submit a change request
...
the change request are evaluated. 
 ...
 You cannot unregister tags for a subnet
...
an XML API request to unregister the tags for the subnet or range. 
 ...
 Invalid configuration errors are not displayed
...
errors. 
 ...
 The push scope selection on the Panorama
...
one of the following tasks. 
 ...
 Initiate a Commit
to Panorama operation followed by a Push
to Devices operation for the modified device group and
template configurations. 

 Manually select the devices that belong to the modified device
group and template configurations. 

 PAN-111729 

 If you disable DPDK mode and enable it again,
you must immediately reboot the firewall. 

 PAN-111670 

 Tagged VLAN traffic fails when sent through
an SR-IOV adapter. 

 PAN-110794 

 DGA-based threats shown in the firewall
threat log display the same name for all such instances. 

 PAN-109526 

 The system log does not correctly display
the URL for CRL files; instead, the URLs are displayed with encoded characters. 

 PAN-104780 

 If you configure a HIP object to match only
when a connecting endpoint is managed ( Objects GlobalProtect HIP Objects <hip-object> General Managed ), iOS and Android endpoints
...
these serial numbers do not appear in the HIP report. 
 ...
 Adding a disk to a virtual appliance running
...
the ESXi host to ESXi 6.5 update2 and add the disk again. 
 ...
 ( Panorama plugins ) The IP address-to-tag mapping
...
unregister the IP address-to-tag mappings: debug object registered-ip clear all . 

 PAN-101537 

 After you configure and push address and
address group objects in Shared and vsys-specific device groups
from the Panorama management server to managed firewalls, executing the show log <log-type> direction equal <direction> <dst> | <src> in <object-name> command
...
the vsys in the query string: 
 ...
 When booting or rebooting a PA-7000 Series
...
Speed section. The messages can be ignored. 
 ...
 GlobalProtect authentication fails with
...
administrator with read-only privileges performs a context switch. 
 ...
 A firewall that is not included in a Collector
...
mode. 
 ...
 On VM-Series firewalls that have Data Plane Development
...
command. 
 ...
 For administrator accounts that you created
in PAN-OS 8.0.8 and earlier releases, the firewall does not apply
password profile settings ( Device Password Profiles ) until after you
...
the passwords to apply password profile settings.) 
 ...
 When DPDK is enabled on the VM-Series firewall
...
of changes to the physical link state. 
 ...
 HTTP Header Insertion does not work when
jumbo frames are received out of order. 

 PAN-93968 

 The firewall and Panorama web interfaces
...
content update or check the Palo Alto Networks Threat Vault to see the
minimum PAN-OS release version for a threat signature. 

 PAN-93607 

 When you configure a VM-500
firewall with an SCTP Protection profile ( Objects Security Profiles SCTP Protection )
and you try to add the profile to an existing Security Profile Group ( Objects Security Profile Groups ),
...
make sure that you do not: 
 ...
 Switch to the firewall Context on
...
the firewall. 
 ...
 Use the Task Manager to verify that you are
...
firewall. 
 ...
 On a VM-Series firewall, the clear
...
command. 
 ...
 The VM-Series firewall on Google
Cloud Platform does not publish firewall metrics to Google Stack
Monitoring when you manually configure a DNS server IP address ( Device Setup Services ). 
...
the FQDN with the IP address in the Kerberos server profile. 
 ...
 PA-7000 Series, PA-5450, PA-5200
...
command. 
 ...
 In WildFire appliance clusters that have
...
and the cluster becomes unresponsive. 
 ...
 The firewall does not generate a packet
capture (pcap) when a Data Filtering profile blocks files. 

 PAN-73401 

 When you import a two-node WildFire appliance
...
exist: 
 ...
 You did not configure a worker list to add at
...
no 
 ...
 Both commands result in Panorama reporting
that the controller nodes are in sync. 

 PAN-70906 

 If the PAN-OS web interface and the GlobalProtect
...
access the GlobalProtect portal. 
 ...
 When viewing an external dynamic list that
...
the date when you configured the report ( Monitor Manage Custom Reports ). For
...
you configure the custom report. 
 ...
 PAN-38255 
 When you perform a factory reset on a Panorama
...
command. 
 ...
 The following issues apply when configuring
a firewall to use a hardware security module (HSM): 

 nCipher
nShield Connect —The firewall requires at least four minutes
...
from the show high-availability state and show hsm info commands
