---
url: https://docs.paloaltonetworks.com/ngfw/help/12-2/panorama-web-interface/panorama-device-deployment/manage-software-and-content-updates
...
finish until a reboot occurs. 
 ...
 HA Upgrade Orchestration —Select this feature to automatically
 upgrade an entire HA firewall pair from Panorama. 

 This option automates and unifies the upgrade process by orchestrating an upgrade of one
 peer, and then the other. This makes the upgrade experience as seamless as a single
 firewall upgrade while providing detailed visibility into the process. You can even
 upgrade multiple HA pairs in a single workflow. 

 With HA Upgrade Orchestration enabled, the system follows a careful, automated
 sequence: 

 It first upgrades the passive (or active-secondary) peer. 

 It then automatically reboots that firewall. 

 Once the first peer is back online and the HA status is synchronized, the system
 seamlessly upgrades the other peer. 

 Before initiating an orchestrated upgrade, ensure that you select HA Pair Status .
 This option filters out individual firewalls and displays only those configured in an HA
 pair, ensuring you are targeting the correct devices for the orchestrated process. 

 When you select HA Upgrade
 Orchestration , the system automatically enables two critical settings:
 Group HA Peers and Reboot device after
 install . 
 With the Group HA Peers option
 enabled, the system automatically selects both firewalls in the HA peer group. This is
 because the automated process requires both the firewalls to be managed as a single HA
 entity. With the Reboot Device After Install enabled, the system
 performs the necessary reboots automatically to complete HA pair upgrade. 
...
access the release notes for the desired content release. 
...
from file. 
 ...
 Preferred Releases Select the Preferred Releases checkbox to view the
