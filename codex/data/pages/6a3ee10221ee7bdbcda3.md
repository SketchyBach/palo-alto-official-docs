---
url: https://cortex-docs.paloaltonetworks.com/cortex-xdr-5.x/configure-cortex-xdr/cortex-xdr-data-sources/vendor-specific-data-sources/dropbox
fetched_at: 2026-09-16T08:41:23Z
source: cortex-platform
---

# Dropbox | Cortex Documentation Portal arrow-up-right-and-arrow-down-left-from-center

For the complete documentation index, see llms.txt . This page is also available as Markdown . 

 Ask 
 On this page 

 Guides 

 Cortex XDR 

 Cortex XDR 5.x Documentation 

 Configure Cortex XDR 

 Cortex XDR Data Sources and Connectors 

 Vendor-specific data sources and connectors 

 Dropbox 

 Configure the Dropbox data source and connector for Cortex XDR. 

 Here are the articles in this section: 

 Ingest logs and data from Dropbox 

 You can configure collecting Dropbox logs and data using a standard data source or content pack integration (onboarded prior to July 26, 2026): 

 Dropbox vendor 

 Description 

 Standard data source overview 

 Forward different types of data from Dropbox Business accounts to Cortex XDR using the Dropbox data source. 

 Link to standard data source instructions 

 The following types of data can be ingested from Dropbox: 

 Log collection 

 Events 

 Directory and metadata 

 Member Devices 

 Users 

 Groups 

 For more information, see Ingest logs and data from Dropbox . 

 Links to content pack/ integration details (onboarded prior to July 26, 2026) 

 The Dropbox content pack fetches and collects security events from Dropbox logs. It includes Correlation Rules, Modeling Rules, Parsing Rules, a Playbook, and a Cortex XDR Dashboard. It also includes the following integration: 

 Dropbox Event Collector : Use this integration to collect events from Dropbox logs. It contains commands such as dropbox-auth-start to initiate the authorization process, dropbox-auth-complete to finish authorization, dropbox-auth-test to check connectivity, dropbox-auth-reset to reset authentication, and dropbox-get-events to retrieve events. 

 Previous CyberArk 

 Next Ingest logs and data from Dropbox 

 Last updated 15 days ago 

 Was this helpful?
